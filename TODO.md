...
| Runtime v2 typed phase-status primary state | whilesunny | Active | `src/chemworld/foundation/state.py`, `src/chemworld/foundation/constitution.py`, `src/chemworld/runtime/phase_separation_services.py`, `src/chemworld/world/separation_kernel.py`, tests/docs | move phase-system readiness, settled status, and selected phase out of runtime metadata and into typed `PhaseLedger` records | pending |
