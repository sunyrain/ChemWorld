...
| Runtime v2 typed initial-amount ledger | whilesunny | Active | `src/chemworld/runtime/species.py`, `src/chemworld/runtime/primitive_services.py`, `src/chemworld/foundation/`, tests/docs | move runtime `initial_*_mol` and `initial_reactant_mol` bookkeeping out of metadata into typed `SpeciesLedger.initial_amounts_mol` | this commit |
