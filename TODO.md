...
本文件是 ChemWorld 唯一活跃任务板。所有开发、发布、专业物理模型深化、
notebook、站点和本地评测机任务都收束到这里。`docs/todo.md` 只保留公开摘要，
不作为第二份任务板维护。

## 当前结论

ChemWorld 已完成公开预发布所需的 P0/P1/P2/P4。当前主线进入 P3：专业物理化学
模块深化。D4 组已经完成；接下来暂停，先讨论后续优先级。
...
- 完成一个任务后立即更新本文件、运行对应门禁、提交并推送；
- `Done` 必须有代码、测试、文档或明确 no-test rationale；
- 不把 `proxy`、`lite`、`reference_validated` 或 `professional-candidate` 混写成
  `professional`。
...
还需完成 **24 项**，全部属于 P3 长期专业化深化。其中 3 项已由 `liyijun` 认领，
Codex 暂不处理。
...
目标：actions、ledgers、observations、spectra、scoring、logs、replay 和 docs 互相一致。
...
| DEEP-D4B | Codex | Done | LLE phase-split solver | Added TPD-style `LLEPhaseStabilityDiagnostic`, connected `liquid_liquid_split()`, `liquid_liquid_extraction()` and runtime `partition_split()`, recorded initialization policy and phase-stability metadata, and tested mass balance plus extraction integration. |
| DEEP-D4C | Codex | Done | Aqueous acid-base equilibrium | Added public pH-meter observation, charge-balance-compatible acid/base result, sequential Ksp precipitation hooks, model card evidence, and tests for pH visibility and multi-salt precipitation. |
| DEEP-D4D | Codex | Done | Gibbs-minimization toy solver hardening | Added `GibbsMinimizationDiagnostic`, recomputable diagnostics, KKT-style stationarity residual, constraint rank, degrees of freedom, stoichiometric formation case, solid/charge case, model card evidence, and documentation. |
