...
| Runtime v2 typed vessel-bound ledger | whilesunny | Active | `src/chemworld/foundation/state.py`, `src/chemworld/foundation/constitution.py`, tests/docs | stop deriving vessel max volume/temperature/pressure from `WorldState.metadata`; enforce typed `VesselLedger` as the only vessel-bound source | this commit |
