...
| Runtime v2 typed observation-cache ledger | whilesunny | Active | `src/chemworld/foundation/`, `src/chemworld/envs/`, `src/chemworld/runtime/observation_services.py`, tests/docs | move last public observation cache out of `WorldState.metadata` into typed `ProcessLedger`, and add constitution/tests blocking metadata fallback | this commit |
