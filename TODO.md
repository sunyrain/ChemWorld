...
| Runtime v2 typed pre-separation product metric | whilesunny | Active | `src/chemworld/runtime/`, `src/chemworld/world/separation_kernel.py`, `src/chemworld/foundation/`, tests/docs | move `pre_separation_product_mol` recovery denominator out of `WorldState.metadata` into typed `ProcessLedger.metrics` | this commit |
