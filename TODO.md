...
| Runtime v2 kernel/profile contract split | whilesunny | Done | `src/chemworld/runtime/kernels.py`, `src/chemworld/runtime/profiles.py`, `src/chemworld/runtime/kernel_contracts.py`, `src/chemworld/runtime/kernel_registry.py`, tests/docs | task profiles, kernel data contracts, service operation kernel, and registry plumbing now live in focused modules without changing transaction behavior | this commit |
| Runtime v2 domain-service registry split | whilesunny | Done | `src/chemworld/runtime/domain_services.py`, `src/chemworld/runtime/domain_service_registry.py`, `src/chemworld/runtime/constitution_factory.py`, tests/docs | service contracts, registry validation, and constitution factory now live outside the domain-service composition module | this commit |
| Foundation constitution checks split | whilesunny | Done | `src/chemworld/foundation/constitution.py`, `src/chemworld/foundation/constitution_reports.py`, `src/chemworld/foundation/constitution_state_checks.py`, `src/chemworld/foundation/constitution_preconditions.py`, tests/docs | report contracts, state invariant checks, and operation precondition checks now live outside the PhysicalConstitution facade | this commit |
| Foundation state ledger split | whilesunny | Done | `src/chemworld/foundation/state.py`, `src/chemworld/foundation/state_ledgers.py`, `src/chemworld/foundation/state_helpers.py`, tests/docs | typed ledger records and ledger helper functions now live outside the WorldState facade | this commit |
| Runtime v2 generic ontology cleanup | whilesunny | Done | `src/chemworld/world/ontology.py`, `src/chemworld/world/state_factory.py`, `src/chemworld/runtime/constitution_factory.py`, `src/chemworld/envs/chemworld_env.py`, tests/docs | fixed A/P/B/D/E no longer exist as generic world defaults; constitution substances now follow the compiled scenario mechanism | this commit |
| Runtime v2 role-based spectra cleanup | whilesunny | Done | `src/chemworld/world/spectra.py`, `src/chemworld/runtime/observation_services.py`, `src/chemworld/physchem/spectroscopy.py`, tests/docs | agent-visible raw signals now use role-based public aggregates and no longer expose fixed A/P/B/D/E labels or species-name heuristics | this commit |
| Runtime v2 action catalog world migration | whilesunny | Done | `src/chemworld/world/actions.py`, `src/chemworld/core/actions.py`, env/runtime/world imports, tests/docs | action constants, catalyst/solvent choices, and terminal-recipe vector helpers now live in `world.actions`; runtime/env/world/action_codec no longer import the core action facade | this commit |
| Runtime v2 remove core action facade | whilesunny | Done | `src/chemworld/agents/`, `src/chemworld/core/`, notebooks/tutorial helpers, tests/docs | agents and tutorials now import `world.actions` / `world.recipes` directly, `core/actions.py` has been removed, and architecture tests block reintroduction in source or notebooks | this commit |
| Runtime v2 remove core objective island | whilesunny | Done | `src/chemworld/core/`, `src/chemworld/world/scoring.py`, tests/docs | duplicated `core.objectives` and the remaining core package files have been removed; scoring is owned by `world.scoring`, and architecture tests block `chemworld.core` imports in source/tutorials | this commit |
| Runtime v2 typed observation-cache ledger | whilesunny | Done | `src/chemworld/foundation/`, `src/chemworld/envs/`, `src/chemworld/runtime/observation_services.py`, tests/docs | last public observation cache now lives in typed `ProcessLedger`; non-measurement observations read that cache, and constitution/tests block metadata fallback | this commit |
| Runtime v2 typed pre-separation product metric | whilesunny | Done | `src/chemworld/runtime/`, `src/chemworld/world/separation_kernel.py`, `src/chemworld/foundation/`, tests/docs | move `pre_separation_product_mol` recovery denominator out of `WorldState.metadata` into typed `ProcessLedger.metrics` | this commit |
| Runtime v2 typed solvent-loss metric | whilesunny | Done | `src/chemworld/runtime/`, `src/chemworld/world/separation_kernel.py`, `src/chemworld/foundation/`, tests/docs | move campaign-level `solvent_loss` out of `WorldState.metadata` into typed `ProcessLedger.metrics` while keeping phase-local loss summaries on phase records | this commit |
| Runtime v2 typed initial-amount ledger | whilesunny | Done | `src/chemworld/runtime/species.py`, `src/chemworld/runtime/primitive_services.py`, `src/chemworld/foundation/`, tests/docs | actual charged initial amounts now live in typed `SpeciesLedger.initial_amounts_mol`; constitution rejects `initial_*_mol`/`initial_reactant_mol` metadata | this commit |
| Runtime v2 typed downstream operation status | whilesunny | Done | `src/chemworld/runtime/crystallization_services.py`, `src/chemworld/runtime/distillation_services.py`, `src/chemworld/foundation/`, tests/docs | crystal filtering, fraction collection, and distillation diagnostics now live on typed equipment records; constitution/golden tests block metadata fallback | this commit |
| Runtime v2 typed vessel-bound ledger | whilesunny | Done | `src/chemworld/foundation/state.py`, `src/chemworld/foundation/constitution.py`, tests/docs | vessel bounds now come from typed `VesselLedger`; constitution and operation validator reject metadata vessel-bound state | this commit |
| Runtime v2 typed phase loss ledger cleanup | whilesunny | Done | `src/chemworld/runtime/phase_ledger_services.py`, `src/chemworld/runtime/crystallization_services.py`, `src/chemworld/runtime/distillation_services.py`, `src/chemworld/world/separation_kernel.py`, tests/docs | phase records no longer store `solvent_loss` metadata; process loss is read from typed `ProcessLedger.metrics` and constitution/golden tests block fallback | this commit |
| Runtime v2 typed phase material fallback cleanup | whilesunny | Done | `src/chemworld/world/separation_kernel.py`, `src/chemworld/foundation/`, tests/docs | phase product/impurity truth no longer reads `PhaseRecord.metadata`; typed phase species amounts are the only material source and constitution/golden tests block fallback | this commit |
