...
| Runtime v2 typed solvent-loss metric | whilesunny | Active | `src/chemworld/runtime/`, `src/chemworld/world/separation_kernel.py`, `src/chemworld/foundation/`, tests/docs | move campaign-level `solvent_loss` out of `WorldState.metadata` into typed `ProcessLedger.metrics` while keeping phase-local loss summaries on phase records | this commit |
