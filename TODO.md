...
下一步不应继续扩散新概念，而是沿着一个方向推进：

1. **长期专业化路线**：公开预发布包已经完成；现在继续 P3 专业物理化学模块深化，优先实现未被 `liyijun` 认领的条目。
...
| 全部可见路线图 | 26 | 完成 P3。包含长期专业物理化学模块深化。 |

## 4. 推荐执行顺序

公开预发布包装已经完成。后续按 P3 未认领条目继续推进；`liyijun` 已认领的 D1A/D1B/D1C 不重复实现，除非重新分配。
...
| DEEP-D4B | Codex | Done | LLE phase-split solver | Added `LLEPhaseStabilityDiagnostic` and `lle_phase_stability_diagnostic()`, connected `liquid_liquid_split()` / `liquid_liquid_extraction()` / runtime `partition_split()` to the same TPD-style diagnostic, records initialization policy and phase-stability metadata, and tests mass balance plus extraction-task integration. |
