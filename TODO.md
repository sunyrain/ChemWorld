...
| PRO-RUNTIME-M downstream truth role cleanup | whilesunny | Active | `src/chemworld/world/separation_kernel.py`, `src/chemworld/world/species_roles.py`, tests/docs | remove legacy species fallback from downstream truth calculations so separation observations depend on mechanism-role quantities and typed phase ledger summaries | in progress |
