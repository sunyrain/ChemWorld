...
| Chinese architecture UTF-8 restoration | whilesunny | Done | `docs/technical_architecture_zh.md`, docs | Chinese architecture report is clean UTF-8, updated with current Runtime v2 rollback semantics, task coverage, benchmark loop, and remaining gaps | this commit |
...
| PRO-RUNTIME-N process metric ledger | whilesunny | Active | `src/chemworld/foundation/state.py`, `src/chemworld/runtime/flow_services.py`, `src/chemworld/runtime/electrochemical_services.py`, tests/docs | move flow and electrochemical derived process metrics into typed ProcessLedger metrics while keeping observations and records deterministic | in progress |
