#!/usr/bin/env python3
"""Run the provider-free electrochemical Work II matched-prior qualification."""
...
try:
    from scripts.run_work_ii_mechanism_oracle_qualification import InMemoryMechanismEvaluator
    ...
    from run_work_ii_mechanism_oracle_qualification import (
        InMemoryMechanismEvaluator,  # type: ignore[no-redef]
    )
...
SOURCE_SUMMARY = (
    ...
    "work-ii-mechanism-oracle-electrochemical-classified-v0.2-20260811.json"
)
DEFAULT_OUTPUT_ROOT = (
    ROOT / "runs/development/work-ii-electrochemical-matched-prior-qualification-v0.3-20260811"
)
DEFAULT_SUMMARY = (
    ROOT / "workstreams/flagship_tasks/reports/"
    "work-ii-electrochemical-matched-prior-qualification-20260811.json"
)
DEFAULT_PACKAGE = ROOT / "configs/benchmark/work_ii_electrochemical_matched_prior_package.json"
DEFAULT_D1_CONFIG = ROOT / "configs/benchmark/work_ii_electrochemical_matched_prior_d1.json"
...
PACKAGE_VERSION = "chemworld-work-ii-electrochemical-matched-prior-package-0.3"
    ...
    def update(
        ...
        *,
        ...
        _emit(
            self.path,
            {
                "event": "electrochemical_matched_prior_progress",
                "task_id": TASK_ID,
...
def _run_world(
    *,
    ...
    evaluator = InMemoryMechanismEvaluator(
        task_id=TASK_ID, config=config, spec=spec, world_seed=world_seed
    )
    ...
    try:
        for design_row in design:
            result = evaluator.evaluate(
                design_row["vector"],
                phase="electrochemical_matched_prior_surface",
    ...
    selected_rows = [by_id[str(row["query_id"])] for row in analysis.get("held_out_queries", [])]
    held_out_queries = held_out_query_contract(selected_rows, reference_context=context)
...
def _d1_config(base: Mapping[str, Any], world_package: Mapping[str, Any]) -> dict[str, Any]:
    ...
    config["intervention"] = {
        "locus": "parametric",
        "target": "controlled_potential_current_local_law",
    ...
    config["belief_checkpoint"] = {
        ...
        "allowed_prior_fields": ["controlled_potential_V", "controlled_current_mA"],
    ...
    config["campaign"] = {
        ...
        "nonfinal_instrument_use_limit": 30,
        "operation_attempt_limit": 110,
        "operation_repeat_limits": {"electrolyze": 20},
        "process_time_limit_s": 45000.0,
        "process_time_policy": {
            "pattern_id": "electrochemical-a-p-k10-two-repeat",
            "formula": (
                "8 unique probe+controlled electrolysis maxima + 2 exact-repeat "
                "probe+controlled maxima; no quench/transfer stage"
            ),
        ...
        "stock_limits": {"reagent_mol": 0.345, "solvent_L": 0.2875},
    ...
    config["method_resources"].update(
        {
            ...
            "operation_limit": 110,
            "input_token_limit": 4_800_000,
            "uncached_input_token_limit": 640_000,
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    summary: dict[str, Any] = {
        ...
        "failures": failures,
        "qualification_passed": qualification_passed,
        "d1_static_readiness_authorized": qualification_passed,
        "provider_execution_authorized": False,
        "decision": "proceed_to_electrochemical_d1_static_readiness"
        ...
        "elapsed_s": round(perf_counter() - started, 3),
        "interpretation": (
            "Provider-free Q2 qualification of matched local controlled-potential/current "
            "priors. Safety risk remains recorded but is not used to manufacture a "
            "safety-law contrast in this non-frontier task."
        ),
    }
    ...
    _emit(
        args.progress_file,
        {
            "event": "electrochemical_matched_prior_completed",
