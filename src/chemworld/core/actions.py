"""Action space definitions and conversion utilities."""
...
def canonicalize_action(action: dict[str, Any], *, clip: bool = True) -> dict[str, float | int]:
    """Return a normalized action dictionary with valid scalar values.

    Gymnasium may provide arrays for scalar Box entries. Agents may provide
    Python floats or numpy scalars. This function accepts both and produces a
...
def sample_random_action(rng: np.random.Generator) -> dict[str, float | int]:
    """Sample one uniformly random action from the mixed action space."""
...
def action_to_vector(action: dict[str, Any]) -> np.ndarray:
    """Map a mixed action to normalized numeric features for surrogate models."""
...
def vector_to_action(vector: np.ndarray) -> dict[str, float | int]:
    """Map a normalized continuous vector to a valid action.

    The first four entries are continuous variables. The next two entries are
    interpreted as relaxed discrete coordinates and rounded to nearest index.
    """
