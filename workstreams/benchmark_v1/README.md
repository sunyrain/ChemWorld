...
约 -0.003。这说明类别表示影响搜索，却没有修复已声明的电化学能力。

publication 汇总器曾错误读取不存在的 `mean_risk`，导致紧凑报告把风险归零。修正后，600 条结果
均有非零 `mean_safety_risk`，六任务均值约为 0.09–0.27；但 safety violations 仍为 0，说明正式
安全阈值从未激活。structured safe GP 相对 structured GP 也没有一致降低风险或提高安全感知得分，
所以安全结论依然不成立，但原因是“约束不可辨识”，不是“风险信号不存在”。更正后的证据 schema
为 `chemworld-publication-evidence-0.2`。

`reports/agent-interaction-audit.json` 进一步确认：正式五方法都完成 40 次实验，三个 GP 方法都进入
36 次 acquisition；但这些方法只学习完整 recipe 到 final score 的映射，不消费中间谱图，也不会
在实验进行中根据观测改变动作。GP-PI/UCB、RF-EI、greedy 和真实 LLM 的历史运行没有保留正式、
可回放 artifact，不能进入方法排名。
...
- **安全评测**：连续风险存在，但约束从未激活，不能评价安全/约束方法。
