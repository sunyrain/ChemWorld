...
| GPT-5.6-sol C2 public participant | 终态 successor | 135/135 scheduled sessions，1,253/1,260 experiments，126 completed、3 failed、6 right-censored |
...
| GPT-5.6-sol current-composite evaluator | 已完成 | 420/420 truth、669/675 checkpoints、129/135 laws、756/810 scheduled blind replays；0 provider calls |
| 删失与资源敏感性分析 | 已完成当前 public 主分析 | 7 failed、7 right-censored 与 84 个未启动 blind 分母全部保留；另报 observed-point sensitivity |
| Matched evidence（A-P Study B + A-S B2） | DeepSeek + GPT matched formal 终态 | 两模型各完成 A-P `15/15` 与 B2 `15/15`、均 0 failures；A-P primary contrast 为 `0.0309/0.0602`，B2 为 `0.0645/0.0915`，且两模型 misindexed 均 `0/5` 表达 exact 1.75 law。B2 的 one-pair free-text surface 不具结构可识别性，结构检验由 B3 承担 |
...
| W2-61 four-condition action extension | 双模型 failure-aware 终态 | 两模型各 180、总计 360 scheduled slots；primary all-scheduled autonomy-minus-none 为 DeepSeek `-0.0913`、GPT `+0.1102`，区间均跨零；donor-eligible 42/26 仅为 post-treatment sensitivity，所有 failures 保留 |
| W2-62 GPT-5.6-sol C2 full-cohort successor | 双模型 135-coordinate 终态 | GPT 126/135 completed；与 DeepSeek 的 matched descriptive comparison 显示 GPT law MAE/compression loss 更低，但两模型 blind gain 均约为 0，三个 selective-correction loci 均未通过 |
| W2-63 DeepSeek B3 full-cohort successor | 双模型 30-cell failure-aware 终态 | DeepSeek 17 completed + 13 participant-schema failures；GPT 30 completed；joint law recovery 0/30 vs 5/30，Top-1 0/30 vs 2/30，eligible gain≥0.02 两者均为 0 |
...
| W2-64 publication reanalysis | provider-free 终态 | W2-61 primary 改为 all-scheduled failure-aware strategy estimand；DeepSeek W2-50 last-available law Top-1 `0/45`、participant Top-1 `11/45`、follow-law `12/42`，三条无 terminal action 的 cell 仍保留 earlier law |
...
| 跨 provider replication | A-P/B2、C2、B3 与四条件 successor 有双模型 scheduled surface | A-P/B2 为完整 matched formal；C2 各有 135 scheduled cells；B3 各有 30 scheduled cells；四条件 action extension 各有 180 slots。原 W2-50 纵向队列与 W2-64 decision-aligned diagnostic 仍为 DeepSeek-only；比较均为 block-specific matched descriptive，不作 model leaderboard |
| Manuscript story | 已整合至 W2-64，投稿包待最终冻结 | 不再等待 oracle；以 evidence acquisition、numerical revision、structural identification、artifact compression、action selection 与 evaluator validity 组织当前证据 |

### 1.1 模型与执行主体覆盖

结论先行：**A-P/B2、C2、B3 与四条件 successor 已有 DeepSeek 与 GPT-5.6-sol 的预定 scheduled surface。** A-P 与 A-S B2 拥有完整 matched formal 分母；C2 现在各有 135 scheduled cells；B3 各有 30 scheduled cells；四条件 action extension 各有 180 scheduled slots。W2-50 原始 45-cell 纵向队列及 W2-64 decision-aligned 派生诊断仍是 DeepSeek-only。Provider-free W2-51/52/53 不涉及 participant，因此不按模型复制。这里的“双模型补全”指冻结 scheduled denominator 已形成合法 failure-aware 终态，不表示每个 cell 都 completed，更不表示某个模型总体优于另一个模型。
...
| W2-50 + W2-61 four-condition successor | W2-50 45/45 autonomous records；W2-61 180 condition slots | 45 autonomous donors；180 condition slots | 共用冻结 candidate surface，无新 recipient truth | 是，primary 为各模型 45-strata all-scheduled estimand；donor-eligible 42/26 与 common 26 仅作 sensitivity |
...
最准确的总体表述是：**participant-bearing main evidence has dual-model scheduled coverage；A-P/B2 是完整 matched formal，C2、B3 与四条件 action successor 是 failure-aware matched descriptive。** 现有证据支持两个模型配置共同复现 B2 conditional numerical--exact-law-expression pattern，并由独立 B3 检验可识别结构；同时“更好的规律压缩仍不保证行动收益”扩展到完整 C2。不同 provider 的 schema/completion 差异以及非随机失败阻止模型优劣排序。
...
7. 固定高信息量反证可以使 A-P 错误参数方向在 5/5 worlds 被明确推翻；该 matched block 估计 conditional post-packet response，因为 packet 与额外 response turn 未被 turn-matched control 分开。
8. A-S B2 的 phase-process packet 后 misindexed prediction gain 平均高于 aligned（+0.0645），但仅 3/5 worlds 为正，且 0/5 表达 exact 1.75 law；其 one-pair free-text surface 不具结构可识别性。
9. 科学纠错不能压缩为 seeking/updating 二分：A-P 定位 conditional response，B2 定位 underidentifying surface 上的 numerical--expression dissociation，participant-identifiable structural law identification 由 B3 单独检验。
...
17. W2-59 的 matched replication 在 A-P 上得到 DeepSeek/GPT primary contrast `+0.0309/+0.0602`，在 A-S B2 上得到 `+0.0645/+0.0915`；后者两个模型的 misindexed exact 1.75-law expression 都是 `0/5`。由于 B2 participant-visible surface 存在精确 linear/power alias，这里复制的是条件性数值—表达模式，不是内部结构识别失败；B3 才是结构可识别检验。
18. W2-62 为 GPT-5.6-sol 补齐完整 C2 scheduled denominator。两模型 prediction improvement 为 `0.1198/0.1329`、law MAE 为 `0.2371 (135 laws)/0.1753 (129 laws)`、compression loss 为 `0.0686/0.0142`（DeepSeek/GPT），blind gain 为约 `-0.0010 (121 cells)/-0.0001 (126 cells)`；更准确的规律压缩没有带来行动增益。
19. W2-63 为 DeepSeek 补齐 B3 的 30-cell failure-aware denominator：17 completed、13 participant-schema failures。DeepSeek/GPT joint recovery 为 `0/30` 与 `5/30`，Top-1 为 `0/30` 与 `2/30`，但 eligible gain≥0.02 两者均为 0。
20. W2-61 在两模型各 180 个四条件槽上给出 action-aligned successor。primary all-scheduled autonomous-minus-no-evidence 为 DeepSeek/GPT `-0.0913/+0.1102`，区间均跨零且任务分层方向异质；donor-eligible `-0.1214/-0.1379` 仅为 post-treatment sensitivity。learned-law-only 没有一致收益，yoked recipient 的 retained failures 要与纯 experiment-selection effect 分开。
21. W2-64 对 DeepSeek W2-50 的 last-available laws 做零新执行诊断：law-implied Top-1 `0/45`，participant Top-1 `11/45`，follow-law `12/42`；三条无 terminal action ranking 的 cell 仍使用更早可执行 law。它是 DeepSeek-only 描述性分解，不是双模型 law-to-action effect。
22. 双模型 scheduled coverage 不等于 cross-provider causal effect，也不授权 model leaderboard；所有比较均保持 block、模型、failure-aware 分母与科学表面边界。
...
| DeepSeek + GPT C2 current | 两模型各 135 scheduled；DeepSeek 121 completed、GPT 126 completed；两边各 420 truth，laws 135/129，blind-evaluable cells 121/126 | 135-coordinate matched descriptive 闭环；两个模型均未通过 selective correction，GPT law 更准但 action gain 仍约为 0 |
...
| A-S Study B2 phase-process evidence | DeepSeek 与 GPT 各 5 clusters、15/15 fresh sessions、30/30 provider turns、80/80 provider-free truth、0 failures | primary contrast `+0.0645/+0.0915`；两模型 misindexed 均 `0/5` exact-law expression；participant-visible audit 拒绝 structural-family identifiability，因此仅支持条件性数值—表达分离 |
...
| W2-61 four-condition action successor | 两模型各 180 scheduled slots；primary all-scheduled DeepSeek/GPT autonomy-minus-none `-0.0913/+0.1102`；donor-eligible 42/26 仅为 sensitivity | 首次直接比较四种信息策略；primary 方向跨模型不一致且区间跨零，learned law 无一致收益，yoked 受 retained failures 主导 |
| W2-62 GPT C2 successor | 135/135 scheduled，126 completed、3 failed、6 right-censored；1,253/1,260 experiments；420 truth、669 checkpoints、129 laws、756 blind | 补齐 C2 双模型；规律压缩明显改善但未转化为 blind action gain |
| W2-63 DeepSeek B3 successor | 30 scheduled，17 completed、13 schema failures；与 GPT 30/30 matched descriptive | joint recovery 0/30 vs 5/30、Top-1 0/30 vs 2/30、eligible gain≥0.02 均 0；结构恢复与行动收益分离 |
| W2-64 publication reanalysis | 零新 provider/truth/物理执行；45/45 DeepSeek last-available laws evaluable | law Top-1 0/45、participant Top-1 11/45、follow-law 12/42；primary four-condition estimand 统一为 all-scheduled failure-aware |
...
结合 Study A 的 A-P suggestive signal，这把自由探索损失定位在取得反证之前或 packet 后响应之外的上游环节；由于额外 response turn 未单独控制，不把它写成纯证据包效应。
...
当前 participant-bearing 主证据已有 DeepSeek 与 GPT-5.6-sol 的 scheduled surface。A-P/B2 是完整 matched formal；W2-62 将 C2 补齐为两模型各 135 scheduled cells；W2-63 将 B3 补齐为两模型各 30 scheduled cells；W2-61 将 W2-50 后继补齐为两模型各 180 个四条件 action slots。W2-50 原始纵向队列及 W2-64 的 45-law decision-aligned 派生诊断仍是 DeepSeek-only。三类后继均保持 failure-aware 分母且不覆盖 W2-59 历史 stop boundary。W2-51/W2-52 已按原规则形成终态负/诊断结果，W2-53 进一步确认完整排序与动作充分性分离。当前投稿不再等待 oracle v0.5、W2-59 补跑或真正 thinking-off，下一步选择是：

1. **Study D：artifact transfer。** 在 context reset 后分别测试 typed law、evidence bundle 和更高保真 artifact 对 prediction/law/action 的增益。
2. **Additional-provider extension。** DeepSeek 与 GPT-5.6-sol 的主要 scheduled surfaces 已补齐；若要接入 Qwen、Kimi、WellAU，须另行冻结独立后继块，不能续跑 W2-59 的未启动分母。
