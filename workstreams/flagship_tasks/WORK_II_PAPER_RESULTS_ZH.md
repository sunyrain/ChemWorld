...
| 当前开发结果 | 完成情况 | 可以支持及限制 |
| --- | --- | --- |
| [EC新预测题与PA三臂入口](reports/work-ii-ec-pa-entry-20260919/REPORT.md) | 15/15固定参考批、99操作及全部精确重放通过，27.594秒，0 provider/重试；126项功能测试通过 | EC正负域各6题，低产率题2/12；PA匿名工具回复及X0/X3错配正确，同固定动作/读数一致。不是自由Agent三臂或正式资格，不升级旧暴露数据 |
| [EC六场核心及两场补充](reports/work-ii-ec-sol-20260918/REPORT.md) | 核心71/72终检、72中测、18后测；补充24终检、24中测、6后测；8条来源重放及修正建议复测通过 | 可读轨迹与候选失效；优化Aligned时间预算失败保留，后测单独补齐。单世界、低响应预测域不支持总体排序或系统性结论 |
| [PA12批](reports/work-ii-pa-sol-20260918/REPORT.md) | 12/12来源批、120操作、3后测；参考另12批，两条重放通过；MAE 0.06338、90%区间覆盖12/12、决策2/2 | 测8/16搭配；共享目录/标签暴露真实身份，不作严格匿名O；自主整批脚本符合开放规则 |
| [PA24批与比较](reports/work-ii-pa-sol-24-20260918/COMPARISON.md) | 一个新来源24/24批、240操作、3后测；重放通过、参考复用；MAE 0.11443、覆盖9/12、决策2/2 | 测16/16搭配及混合2×2，迁移仍较差；明确沿用身份暴露界面。单次结果不估计平均预算效应，token少不等于思考少 |

EC来源与后测核心实测61.62分钟，补充19.37分钟。PA12完整13.24分钟、24完整14.43分钟；后者复用参考，因此整链不是纯预算耗时对比。PA输入/输出分别1,544,640/15,829和929,019/13,930；缓存含于输入，后测按同线程累计差分。原数据与失败保持，新公共接口和文档修正不升级历史证据。

以下为更早资格修复和历史结果，描述当时协议，不替代新自主链与身份检查。

最新[W2-118修复汇总](reports/work-ii-gate-repair-summary-20260916.md)：新增可选定制委托/局部先验与公开状态读数，修正参考选点及PA分母合同。首块18/23条件完成；PA评估器动作前接入错误保留，修复后独立5/5条件、14/14批通过。当前主响应5/5、局部资料反证5/5、三臂入口15/15、世界分叉3/3，任务筛查4/5；FL仍无预设可行点（0/9）。两块累计23/28条件完成、75/89终检、474动作，23完整轨迹资源/精确重放均通过；2失败/3上游缺失位未移出分母，另存动作前导入错误。约3.44分钟，零provider、块内零重试。自由机制形式保留；局部档案接口资格不等于正式三臂内容或完整K1/Q/K2及自主研究链已通过。

此前[W2-117五体系gate实测](reports/work-ii-new-system-gates-20260916.md)：RX/EQ/BC/PA/FL共8个多批参考campaign、30/30批终检，246动作，资源/精确重放均8/8，约2.06分钟，零provider和补跑。旧三臂资料入口7/15可构造，仅PA三臂齐全；本轮预定公开响应筛查0/5、成对世界1/3（EQ）。RX另有明显时间效应，PA存在阶段/观测合同问题，FL无示例可行点；失败针对当时选点与接入，不是Agent失败率，不推翻旧显式合同资格。固定[说明](WORK_II_NEW_SYSTEM_GATE_NOTE.md)、全部失败及读数已保留；新九卡完整G-S/G-R仍未就绪。

W2-116在矩阵8.4/9.4/9.5节汇总首轮数量、gate与条件ETA：27来源核心162＋v模型工作块，有限扩展234＋v；从current绑定只读统计W2-100/101/103/105耗时，明确新协议尚无整链实测。属于排期设计，没有新增模型/物理实验、资格通过或科学结论；历史分母和结果不变。

W2-115整合[当前全部实验矩阵](WORK_II_EXPERIMENT_MATRIX.md)：只设Opaque/Aligned/Misindexed三臂，九张任务卡按体系定制委托、先验、终点与后测，不默认每体系探索/产率两任务。水相平衡、相间分配与有限预算表征分开；连续流采用配置相关可行窗口，产品任务各自核验实际交付。
本轮仅核对Paper 1及运行实现并修改设计，没有新实验、LLM评分或正式结果。来源任务交付及K1/预测在访谈K2前封存；认知核验与E-Q复用时去重，不强造配方或新增样本。接口和数值仍需逐卡资格，旧结果、分母及导出稿不变。
W2-114的三臂统一来源原则保留；其通用机理探索/产率优化框架与3(N+J)r通用计数已由实际任务—世界集合取代。目标配对只在适用子集。
W2-113曾整合独立先验与机理研究；其独立来源及候选引导/结构化提示默认对照已由W2-114替代，自由报告与证据/真值分别评价的原则保留。
W2-111/112的[旧提示译文](reports/work-ii-autonomous-prompt-example-zh-20260916.md)与[过程译读](reports/work-ii-autonomous-transcript-zh-20260916.md)仅为历史单例核查，不是新样本或系统性失效统计。

W2-110曾按用户当时要求撤回W2-109的强制来源复用，独立设计先验三臂与自主规律—预测；这一来源安排现已被最新统一方案替代。
以下W2-109/110只保留设计变更的历史说明，不定义当前矩阵；没有新实验，历史三臂与M1/M3的证据身份、数值和分母均不变。

W2-109此前仅完成三臂与自主规律—预测模块的重合分析和共享矩阵提案；其强制来源复用已撤回。
原三臂的预测/规律记录不因设计复用变成新的自主机理证据；无新增来源、预测结果、实验或独立样本，当前计划见矩阵第2.4/5.3节。

W2-108按用户纠正明确：M1/M3固定二次曲面只测局部响应面的建模/交付/决策，不能用作自主机理推断证据。
其原设计、结果、失败与分母保持；当前主设计改为发现Agent自主形成并封存K_src，开放模型结构与数学形式。
事后摘要/组件编码是支持诊断，自由模型接口尚待实现与验证；本轮没有新增实验或更改导出稿。

W2-107恢复先验三臂、实验修正和知识决策价值的当前设计，扩展候选机制覆盖；本次整理没有新科学数据。
旧C2三臂、M1/M3和完整流程开发的结论与分母均保持，计划条件只在[实验矩阵](WORK_II_EXPERIMENT_MATRIX.md)维护。

W2-106仅整合[完整实验矩阵](WORK_II_EXPERIMENT_MATRIX.md)，没有产生新数据或改写本索引中的统计。
M1/M3保留为主文中的局部能力与边界证据，F3保留表达保真度动机；新E0—E3的计划条件不计入任何已完成分母。

最新[W2-105完整流程迭代](reports/work-ii-full-process-iteration-20260915.md)：四个开发块142/142参考终检，
当前40个候选中有8个联合质量见证，覆盖纯化、结晶、蒸馏三个任务；8个历史语义失败单列保留。
完成选相扣样、粒径付费观测、完整粒子群、独立蒸馏切段、淬灭后溶解、晶种来源和热量费用记账修正。
Astra medium三个任务各一次：纯化25步，终检纯度30.47%、原投料回收4.53%；结晶20步，
纯度97.37%、扣晶种产率8.42%、细粉100%，均未联合达标；蒸馏13步后provider中断、无终检、token缺失。
零补跑、零失败事务、零工具错误；2544条动作和145条轨迹/已记录前缀精确重放、资源核对通过。
模型识别并如实报告已完成轨迹的质量失败；这仍是单世界、单批过程决策试跑，不是知识交付或系统性信息损失证据。

此前[W2-104完整流程诊断](reports/work-ii-full-process-diagnostic-20260914.md)：15个确定性条件各一次，
13终检、2个脚本合法范围错误，255条动作及全部轨迹/前缀精确重放，未新增模型调用。
纯化增加洗涤的真实终检纯度/回收下降；局部过程摘要与终检的组分、分母不同，已纠正分析混用。
蒸馏首段停止与追加混合的终检纯度35.9%→24.4%，回收96.4%→97.1%；结晶延长冷却仍未满足细粉要求。
该旧块无联合质量可达见证；当时后续的扣样对象及粒径观测修正已由W2-105实施，旧失败保持不变。

此前[W2-103完整流程单轮](reports/work-ii-astra-full-process-20260914.md)：Astra medium三个流程各尝试一次，
纯化22步及蒸馏17步完成终检，纯度分别28.40%/24.42%，回收10.65%/97.05%，均未达全部质量目标；
结晶在第一次加料后provider中断，未补跑。4次参考尝试含1次干燥时间记账失败及独立修正，3次终检。
共109条持久化动作与7条完整轨迹/前缀精确重放；另1次dry异常未形成轨迹行，费用不冒充完整。
无质量联合可达性参考见证，不宣称系统性信息损失；这一轮未进入稿件或正式证据。

以下2026-09-07历史结果的双模型正式主分析与一次补跑敏感性，已整合进当时两版稿件、图表和匿名复现包。

以下先列当时补充结果；原120会话正式主分析随后单列，分母与失败保持不变。
8/8额外会话有效、16/16轮，零新执行失败；未知/错误先验5/5恢复，正确先验2/3保持，另1例有效但科学答案错误，保留不再补跑。
虚拟一次重试敏感性中DeepSeek恢复5/20→20/20（+75个百分点），Top-1 7/20→5/20，regret 0.05551→0.09597；保持8/10→10/10。
两条方向保留，但该分析按原失败事后选择，不能替换原120会话主结果或被当作独立复现/新显著性主张。
实测日历20.10分钟、会话wall总和3404.030秒；16份usage完整，新增input2255824（cache1979776）、output450242（reasoning433005），35工具尝试、11拒绝。
8个输入逐字不变、16份实际提示词与对应GPT及冻结prompt一致、8个fresh thread及pre/post续接一致；8个恢复/Top-1/regret评分和累计usage独立复核通过。
结果见[一次补跑报告](reports/work-ii-information-failure-retry-20260907.md)，独立绑定current；无剩余会话，不重跑有效但错误的答案。
...
原GPT报告保留，累计usage去重不改科学评分。[双模型比较图](../../paper/figures/prior-discovery/figure-10-information-models.pdf)、
[GPT独立图](../../paper/figures/prior-discovery/figure-10-information-gpt.pdf)与[DeepSeek独立图](../../paper/figures/prior-discovery/figure-11-information-deepseek.pdf)均已交付。
两版稿件/PDF与匿名包已整合并通过复算和视觉检查，交付状态见[投稿清单](../../paper/ICLR_2027_SUBMISSION_CHECKLIST.md)；信息长度、配对噪声与同族参数覆盖继续限制解释范围。
...
| --- | --- | --- |
| [W2-105完整流程开发](reports/work-ii-full-process-iteration-20260915.md) | 142参考；当前8/40联合见证；Astra medium 3/3尝试，2终检/1服务中断 | 三个初始任务存在合法路径；两场完成的自主流程未达标，模型能识别问题；不推断系统性失效或摘要损失 |
...
| [W2-89一次补跑](reports/work-ii-information-failure-retry-20260907.md) | 原8失败各一次额外会话，8有效/16轮，7例结构正确、1例有效但答错；无新增独立world | 事后敏感性中DeepSeek恢复5/20→20/20、Top-1 7/20→5/20；总计保留128次尝试，不能替换原主结果或当作独立复现 |
...
| M1正式复核 | 10 worlds、40来源状态、120/120 sessions、160/160条件、200/200物理与replay；零失败 | 固定二次类对照：主差−0.00538，95%区间[−0.01630,+0.00061]，未支持实质收益；F-A/F-X 40/40一致；不测自主机理推断 |
| M3信息分离 | 复用M1的10 worlds与40来源；160/160接收会话、80/80新隐藏物理与replay，零失败 | 固定二次响应面交付：L−none −0.13723，95%区间[−0.15584,−0.12257]，支持实质收益；nearest10/10最优，不测自主机理推断或新机制迁移 |
...
当前先开发九张定制任务卡、自由报告、任务查询及评价，再完成各卡E0与E0-J资格。
E1-P按各卡委托运行三臂；E1-G仅为预选同世界增加另一委托。同源E-K/E-Q之后，E2按任务交付核验或复测、预选知识交付，E3按适用问题诊断；完整条件、计数和停止边界仅见
[实验矩阵](WORK_II_EXPERIMENT_MATRIX.md)，任务状态仅在[TODO](WORK_II_TODOLIST.md)更新。
旧M0—M4入口已退出当前执行顺序；已完成结果和未执行身份保持，不恢复历史补跑队列。本次没有启动新实验。
