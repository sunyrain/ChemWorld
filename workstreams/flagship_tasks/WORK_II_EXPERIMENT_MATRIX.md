...
不再为每个体系套用“机理探索＋产率优化”两份任务。W2-115替代W2-114的通用目标框架；三臂统一来源、自由报告及封存次序保留。历史任务的目标、结果和分母不追溯改写。

当前主任务集合为九张定制任务卡：电化学有效转化、反应/热过程辨识、水相平衡表征、有限预算表征、相间分配发现、连续流运行窗口、反应—纯化、反应—结晶、反应—蒸馏。
这是科学委托的分类，不宣称九种相互独立的底层物理机制；共享反应网络/组件的实例在统计中保留依赖。原先合并的“平衡/相/表征”拆开，避免不同对象共用一个模糊终点。

每张任务卡默认只有一个主要委托。产率、回收、粒径、预测误差、诊断质量或运行可行性分别按任务定义；共同的是三臂控制和研究后评价的原则，而非相同数字分数。
只有确实需要估计目标作用、且同一世界支持两种有意义委托时，才预选目标配对。可选反应辨识/受约束合成、结晶交付/路径辨识两个问题，不将其自动铺满主矩阵。
...
| 目标作用（可选支持） | 在同体系内，改变科学委托是否改变取证和知识？ | 事前选定的同世界目标配对；不能从跨体系成绩推断 |

| 块 | 当前设计 | 覆盖与地位 |
| --- | --- | --- |
| H | 旧M1/M3、C2/F3/F4及开发结果 | 保留原身份与局限，不并入新来源 |
| E0 / E0-J | 逐任务检查运行、可辨识性、终点/约束与评审有效性 | 全任务基础；无产品的任务不做产品质量资格 |
| E1-P | 九张定制任务卡各自的O/A/M自主研究 | 唯一默认来源；任务卡可因科学资格失败而标为待开发，不换成简单任务冒充 |
| E1-G | 预选同世界的另一明确委托 | 可选目标对照；没有默认第二任务，也不限于产率 |
...
具体科学委托、先验和后测统一见第2.4—2.8节的九张定制卡，不再保留与其竞争的旧七类任务表。候选机制是否实际存在、竞争解释是否可被公开实验辨别，须E0逐项验证。
...
下表是完整主任务清单。每行一个主要委托，均追加任务相关的机理讲述和预测。EC/RX/EQ/BC/PA/FL/P/C/D是本设计的简写，不是新注册runtime任务ID。

| 卡 | 体系与主委托 | 来源结束的任务交付 | 主要评价 | 是否默认第二任务 |
| --- | --- | --- | --- | --- |
| EC | 电化学：在电荷/能耗及选择性约束下建立有效转化方案 | 材料/介质与电位、电流上限、时长的方案或反馈策略 | 独立复测的有效目标产物量与约束达标；电荷/能耗分列 | 否；解释电化学响应是共同后测 |
...
| FL | 流量、停留时间、热边界与设备几何如何共同改变出口组成和约束 | 设置合法流量/停留时间组合，运行温度、时长和已开放的出口表征 | 当前接口由流量和停留时间构造体积/长度，不是固定同一反应器的两个独立旋钮 |
...
每行只选预先登记的一层先验内容，不默认与实体/参数/结构三层全交叉。O删除该层实例提示，A给正确或限定域内准确资料，M给结构、语气、精度匹配的错误关系；共同仪器合同、合法范围和计费始终正确且一致。

| 卡 | 建议干预的实例资料 | A/M可控差别及边界 |
| --- | --- | --- |
| EC | 材料/介质与有效响应的关联 | 正确关联与错配关联；公开材料标签/设备接口不变 |
...
- FL当前以V=Qτ构造设备体积/长度，名义throughput_mL不自动等于实际可交付产品量。当前任务评估配置相关的窗口与操作点；固定设备生产率优化若另立任务，先完善并核对进料/出口产品与资源账，不能把现有记录当作已实现的产量评价。
...
上述两项是支持实验候选，不是九张主卡各自必须再乘二；实际采用哪项在块note开始前固定。原主任务来源直接复用，每个配对世界只增加另一委托的O/A/M三个来源。
...
数值预算、查询量、实际world实例和正式样本量尚待逐任务资格，不能用旧任务卡的动作数或原生score阈值填满空白后称已冻结。开发允许改设计；每个已启动数据块仍固定覆盖和停止规则，旧失败不覆盖。
...
| --- | --- | --- | --- | --- |
| EC | 转化方案独立复测 | Raw/K对新控制组合的预测；预选新条件操作 | 材料关联的共同证据或真实材料响应分叉 | 额外机理目标来源、任意串联分离流程 |
...
每个任务—世界的三臂分别采用独立持久campaign，允许多批、批内反馈、测量、重复、修订与停止。E1-P执行第2.4节的九张定制任务卡；E1-G仅增加事前选定的另一委托。同一来源直接进入E-K/E-Q，不再另造机理来源。
...
| 时点 | 要求 | 记录和边界 |
| --- | --- | --- |
| 来源前与少量固定检查点 | 简短当前解释、证据与未决问题；合法操作照常推进 | 保存当时版本，不以最后回忆替代过程记录；所有条件同要求 |
...
| 第三轮：深入访谈 | 最难解释的观测、备选解释、反驳条件、一次下一实验、混杂、文字/方程/预测一致性 | 不泄露评价真值，不强迫凑解释；若修订保存K2，单列受提示反思效应 |
...
实验过程在批次决策/固定检查点保留简短目的、可检验预期与结果后的公开修订，不索取私有思维链，不强迫每个动作长篇解释。事后访谈只能补充解释，不能倒填为当时已经记录的动机。
若比较K0/K1对同一盲测的预测，用各自封存产物交给独立接收者；不能让已经学习的原Agent模拟回到初始状态。

### 5.4 E-Q：统一来源后的独立预测

每个三臂来源的K1先封存，再揭示预定问题；预测封存后才执行或揭示评价器结果，不给报告事后改写机会。原Agent预测为统一后测，fresh读出在预选子集实施，不默认所有来源×全部接收者全交叉。
...
令C_main为九张主任务卡实际登记的“任务—世界”单位集合，C_goal为预选目标对照新增的委托—世界单位；不包含已经在主任务中运行的那一侧。各单位c重复r_c次，来源总数为：
...
一张卡一个合格实例、每条件一次时，九卡主集合是27个来源；若在同一RX及C实例各增加一张配对委托，则新增6个、共33个来源。两张主卡若共享同一个物理世界，它们仍是不同委托会话，但统计只算一个世界簇。
...
| 顺序 | 工作 | 完成依据 |
| --- | --- | --- |
| 开发1 | 九张定制任务卡、共同三臂来源与自由报告/预测接口 | 委托明确，无默认双任务；交付类型与先验、观测权限匹配 |
...
九张主卡是设计覆盖，不宣称均已获得新协议资格或已经运行。开发实例可从互补的EQ、RX、C开始验证表征、机理和完整工艺三种交付形式，再覆盖其余卡；这是接口开发次序，不能用前三卡替代完整矩阵。
所有主卡各一个合格实例、r=1时是27个三臂来源；是否加入RX/C目标配对在对应块启动前决定，不自动从27变为54。该数字是设计实例，不是正式样本量承诺。
当前模型仍仅GPT-6 Astra / medium，每条件一次，允许多批/多轮。服务与科学失败保留，不因阴性补跑。W2-117/118执行无provider的固定参考筛查及接口修复验证；新自主来源与稿件导出未启动。
...
本表合并第3/9节已有要求，不增加独立readiness、证书或清单生成器。状态以当前TODO的W2-93—95和实际note/结果为准。W2-118已完成五体系局部修复验证；旧平台/单批或本轮局部通过均不自动替代新九卡完整资格。
...
| G-S：任务科学资格（科学） | 同公开信息参考验证可辨识性、预算内反证；按卡检查诊断、窗口或物料交付可达性；A/M操作权限相同 | W2-118当前公开响应5/5、局部资料反证5/5、成对世界3/3；任务筛查4/5，FL可行点0/9；完整资格仍未完成 | FL需独立论证可达前沿/任务规格；其余卡仍需完整机理辨识与留出预测。W2-93负责逐卡资格 |
| G-R：真实执行链（运行） | 当前输入→多批资源→持久轨迹→任务交付/K1/Q/K2隔离→评分/失败汇总贯通；精确重放与账本成立，无真值回流 | W2-118当前23/23参考campaign运行/资源/重放通过、15/15新资料入口实际送达；自由报告后测/provider链未测 | W2-94接入完整自主来源与后测；当前局部数字档案接口不等于正式机理先验已冻结 |
...
本轮资料为指定操作条件的一条可修订带噪档案估计，A来自同条件标定，M来自错配条件；只验证信息接口和反证可达性。它不取代正式实体/参数/结构先验设计，不约束Agent机制表达；已提示条件不得作为Q盲测。完整辨识、K1/Q/K2、评审及真实provider链仍待验证。W2-117与118选点/合同不同，不将通过率变化当成统计性能力提升。

当前启动判断：继续开发和按note开展资格工作；新27来源主比较尚未达到canary_ready/release_ready。此处只汇总六类既有边界，前后新增运行时gate、证书和审计生成器均为0。
...
新自由多批来源、语义评审和九卡查询没有整链实测。下表为单executor、单provider串行且无自动补跑的容量估算，不是统计置信区间、运行预算或硬timeout；必须先用合法参考与真实开发路径校准。已有单批runner的1800秒限制不能直接套到新研究会话。
...
当前可据此安排约一周量级的完整首轮开发，而非“27次调用当天跑完”。具体日历交付需在真实路径完成首批后更新；正式论文全实验ETA暂不确定，因为世界数、重复、模型范围及支持集合未定。这里不承诺完成日期，也不要求等全部任务才能逐卡看到结果。
