...
更新：2026-09-18，W2-130。**当前设计为development：三臂、每来源12批、12次可分配额外测量、终检另计，自主选择单批或成组实验。EC双目标、PA单目标；均先封存研究，再K1→Q→K2。EC与PA已有结果、修正及gate实际状态集中见第9.8节。PA的24批仅为已完成预算诊断，未改变主预算。新正式Participant样本量尚未冻结。**
...
| 后测 | K1自由报告→Q数值预测与区间→K2简短复盘；下一块查询域需平衡 | 同一顺序；Q为12条件×2互补比例及2项决策，K2三问；主域与迁移域分报 |
...
- **仍待实施/验证：** EC的平衡查询域、公开观测摘要及笔记/拟合能力仍需在新note前核定；现有Q仍为历史窄域，不能仅换资源就宣称新六场设计已就绪。PA公共匿名与O/A/M资料入口已有针对性测试，尚无修正后真实provider三臂；新Q需分清主域与迁移域。不要将定向测试写成新科学资格通过。

顺序为先完成所选块尚缺的运行修正和一份简短note，再由该块预定首场核对真实输入→工具→后测链；可以先PA或先EC，不恢复自动大队列。12批是主预算，不因结果不理想再增加次数。RX/C等后续体系仍按自己的科学任务推进，不复制PA问卷或EC双目标。
