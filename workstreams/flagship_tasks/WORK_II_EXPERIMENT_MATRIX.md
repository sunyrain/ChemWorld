...
[作者故事](../../paper/prior_discovery_story_zh.md)组织读者论证。W2-118曾修复五体系定制资料/观测接口并完成独立验证：当时主响应5/5、局部资料反证5/5、任务筛查4/5；FL窗口未建立，当时完整自主链尚未验证；EC/PA后续状态见9.8节。历史失败与当前版本分母分开保留，零模型调用，无论文导出；判定见第9.4节。
...
Opaque 必须相对于被干预的目标层 `L` 定义：保留公共操作、合法范围、仪器、评分和 `L` 之外的共同背景，但不提供 `L` 上任何实例特定知识。只有工作点而没有方向模型的资料属于 `ContextOnly`，不是第四个主臂，也不得记作 Opaque。
...
每次来源只使用预先登记的一层先验内容。EC对选定层展开两个目标与O/A/M，近期选E层；P/S是待选扩展，其他体系也不默认三层全交叉。O删除该层全部实例提示，包括实例参考中心、容差、方向、窗口、斜率、方程及由目标世界标定出的可行性/最优暗示；A给正确或限定域内准确资料，M给结构、语气、精度、锚点和置信度匹配的错误关系。共同仪器合同、合法范围、计费及目标层之外且不由目标结果选择的背景始终正确且一致。
...
| 臂 | 初始资料 | 保持不变 |
| --- | --- | --- |
| O：Opaque | 隐去指定层面的全部实例知识；保留公共实验室说明、合法范围和该层之外的共同科学背景 | 同世界、公开身份、操作/仪器、预算、评分、预定噪声配对；只给工作点而无关系规律者另标ContextOnly |
...
后续新数据块的提示原文、英文输出、封存顺序、任务预注册插槽、工具预算和失败修复，以[通用 K1–Q–K2 封存后测协议 v1.1](WORK_II_CANONICAL_K1_Q_K2_PROTOCOL_V1_1.md)为规范；当前五世界RX-P块的严格信息边界、具体十二题与评分见[RX-P任务协议v1.2](WORK_II_RX_P_K1_Q_K2_PROFILE_V1_2.md)，RX-S见[任务协议v1.0](WORK_II_RX_S_K1_Q_K2_PROFILE_V1_0.md)。本节说明实验设计角色，不另立一套提示。历史块仍按其实际执行版本解释。
...
| 第一轮：K1 | 使用协议 v1.1 的固定完整机理报告提示，以英文说明关键变量、关系/耦合、实验依据、竞争解释、适用范围和不确定性 | 中性提问，不暗示资料必错；保留“充分展开”，不设固定模型或硬字数；固定报告计算预算，无新物理证据 |
| 第二轮：E-Q | 使用固定外层提示，一次给出预注册的十二个独立新批次及任务相关指标；逐指标提交点估计和80%预测区间 | 查询动作、指标、参考、评分及工具预算在来源前冻结；先封存预测再验证，答案不反馈 |
| 第三轮：深入访谈K2 | 使用协议 v1.1 的七项固定复盘并以英文作答：初始主张、判断变化、竞争机理、一次判别实验、辨识/优化取舍、遗漏证据/不可靠预测、推荐或交付边界 | 引用K1和真实批号并避免重述全文；不泄露真值、不强迫凑解释；K2是受提示反思，不能覆盖K1/Q |
| 独立评价/应用 | 来源推荐复测、LLM评分、预选新部署或Raw/K读出 | 各自资源和分母独立，结果不能倒流来源或覆盖K1/预测 |

K1逐字提示见协议第3节；有操作建议时统一以“实验阶段已结束，操作建议已经封存”开头。按定义没有合法操作建议的任务，只有在数据块开始前声明后才可将该短语替换为“任务交付”，其余提示不变。
任何任务都可如实说明没有研究某些关系；按任务相关知识维度记录不足，不将任务外关系事后加入主要成败标准。正确但无实验支持、诚实未识别与确定错误分别评价。
深入访谈对所有条件使用协议第5节相同的七项问题；依据报告追加的特定追问只作预选诊断，不与统一主测试分数混合。七问中的“一次合法完整实验”只是假设性提案，不免费执行、不替换来源交付，也不计为新的行动成功。无操作推荐的任务按协议声明不适用，并评价对应任务交付的推广边界，不强造配方。
实验过程保留实际公开的目的、预期与修订，不要求Agent额外生成固定检查点，不索取私有思维链或逐动作长解释。事后访谈只能补充解释，不能倒填当时动机。K1保留“充分展开，不必压成短摘要”，但不设置强制字数上限或预定机制模板；K2以引用K1、避免重复整表和整篇报告来控制冗余，不删除复杂规律所需表达。
...
| G-R：真实执行链（运行） | 当前输入→多批资源→持久轨迹→任务交付/K1/Q/K2隔离→评分/失败汇总贯通；精确重放与账本成立，无真值回流 | EC W2-122已执行8来源、95终检、24后测，来源重放及修正建议复测均8/8；失败来源后测通过独立续接补齐，主runner已前瞻修复失败来源后测，尚无新provider复核。PA12/24各一来源已贯通但严格匿名不成立 | W2-124修复运行边界，再逐卡验证；不因EC接入成功宣称九卡齐备 |
...
| G-C：终态与证据结案（科学/运行） | 全部计划位成功/科学失败/服务失败/未执行有明确终态；分母、usage缺失、资源、replay和汇总可核对 | W2-118两块累计23/28条件完成、75/89终检、474/572动作；2接入失败/3上游缺失保留，零provider/块内重试；W2-117另列 | 这是历史W2-118分母；EC/PA新块已结案，计数和限制见9.8节。新正式块按实际输出结案 |

这些名称区分检查职责，不是每次讨论或开发都要依次取得六张通过证书。开发块先固定问题和范围，核对所涉及的操作、观测、资源与实际输入；尚未取得的科学资格明确标注，可以运行用于查明该缺口的开发试验，不能称已合格比较。G-R的完整路径也可由预定首场验证，不先要求另一场相同来源。G-J只约束正式语义评分，G-F只在正式生产边界，G-C在结案；无需等待不相关任务。EC/PA当前逐项状态以第9.8.3节为准，不能把下述旧通过率当作新界面的全面放行。
...
W2-118资料为指定操作条件的一条可修订带噪档案估计，A来自同条件标定，M来自错配条件；当时只验证信息接口和反证可达性。它不取代后续正式实体/参数/结构先验设计，不约束Agent机制表达；已提示条件不得作为Q盲测。EC后续K1/Q/K2和真实provider链由W2-122运行，PA由W2-128/129运行但有身份暴露；其他任务仍需核对。W2-117与118选点/合同不同，不将通过率变化当成统计性能力提升。

当前判断：EC及PA两轮开发试跑均已结案；本次W2-130只修实现、分析和文档，没有新来源。下一块依据第9.8节核对实际相关缺口并固定简短note，不新增证书或审计生成器，也不要求Agent事前按固定机制表打支持/反驳标签。Experiment 1正式confirmation/release按自身冻结协议独立处理，不把其全部发布条件变成普通开发的前置门槛。
...
本节旧排期表为W2-116单目标历史基准，不覆盖EC双目标十二批新设计，也不定义当前待跑队列。EC W2-122已实测：六场核心来源21.70分钟、后测39.07分钟，逐场整链合计61.62分钟；两场补充另19.37分钟。PA已有单来源13.24/14.43分钟的开发实测；其他体系和修正后的完整三臂尚无同条件实测，不因修订自动加倍；近期排期见第9.6节。
...
本节保留W2-123基于EC记录形成的改进设计；它不是自动执行队列。其后PA已先行完成12批与24批试跑，最新选择和实现状态见第9.8节，不再要求先重跑EC。旧提示、原回答、终检、查询和失败不变；EC六场核心的71/72终检、两场补充及全部接入失败保留在[评估报告](reports/work-ii-ec-sol-20260918/ANALYSIS.md)。

Remote RX protocol development also corrected locus-specific opacity and expanded its earlier suggested RX subset to two goals. Those changes describe the RX block; they do not authorize extra EC/PA runs or rewrite their saved prompts. The older RX-P canary and EC-P supplemental pilot exposed a parameter reference region and are ContextOnly, not strict P-Opaque. Historical planning totals are not current execution totals.
...
| --- | --- | --- |
| 原队列自动扩成18场；用户收束到6场时另2场P已启动 | W2-130已修显式--units，无默认全因子队列 | 新design记录单位顺序与对应分母；旧18场设计和缩减记录保留，下一块需独立note |
| 144000 s不足以支持优化Aligned十二批，原失败保留 | W2-130前瞻修正：时间只记账，库存按操作上限供给 | 12批/12额外测量为主资源；保留操作范围、安全、损耗。新资源卡不能追溯用于宣称旧来源成功 |
| 原runner只对完成12终检者问后测，旧块另脚本补齐失败者 | W2-130按有效终态回执/存活上下文决定后测，并保留source_failure/posttest_failure | 不按高分或完成批数筛选；无完整上下文记不可用，provider错误不伪造续接，仍需真实首场验证 |
...
| RX-P所谓Opaque收到`420 K / 3300 s`近似参考区域；EC-P补充Opaque收到完整P参考配方 | 目标层信息泄漏/命名错误；两者保留为`ContextOnly`历史数据，不能进入strict P三臂。EC-E的P控制不属于E层，未发现同类问题；RX-S Opaque为`None`且可见面检查通过 | 新RX-P六格全部fresh运行：P-Opaque不含参考中心/容差/方向/窗口/方程，A/M保持schema、锚点、置信度匹配且只反转目标关系。W2-124加序列化payload和RX-S可见面回归测试。Q中的420 K只在K1封存后共同揭示，不是来源先验 |
...
| 初始大合同和每步返回混入大量重复字段；旧official_budget=48与本轮budget=360同时出现；K1/K2长篇重复 | 上下文成本与有效预算表意问题；需要统一提示与去重复规则，而非临时删题或增加重试 | 面向Agent明确本轮有效12批/360操作，旧任务默认数不作为竞争指令；精简单位、合法动作、资源变化、观测/不确定性和终态，完整账本留宿主。K1保留完整展开，K2固定七问并要求引用前文、避免重复；无固定模型或硬字数门槛 |
...
**来源仍自由。** 12批、12次非终检测量、12次终检；测量按需分配，重复占批次位，采样损耗照实记。允许自主单批或成组编排，不要求先提交组计划，不强制每次测量/批末触发反思，不预设必须做某个反证实验或每批填目的。物理串行、结果送达、控制权返回及计划修改分别记录；不能由其中一项推断其他项。优化组与探索组同世界、同工具、同资源，仅主要委托不同。

**封存顺序保持：任务交付→K1→Q→K2。** 具体提示、英文输出与边界由[通用 K1–Q–K2 封存后测协议 v1.1](WORK_II_CANONICAL_K1_Q_K2_PROTOCOL_V1_1.md)冻结。K1不看具体题，Q后不反馈答案，K2不能改写原预测。后测只读来源已有信息，允许本块事前声明的公共计算；不以是否找到好配方或凑满十二批作为后测准入。认知任务不强造物理推荐。

| 阶段 | 下一版具体要求 | 本轮经验与约束 |
| --- | --- | --- |
| K1 | 逐字使用协议第3节的完整机理报告提示；用英文及自选数学或伪代码说明关键变量、关系/耦合、关键实验、竞争解释、适用范围和未知 | 保留“充分展开，不必压成短摘要”；不强制字数、候选机制或固定系数。无操作建议的任务只可使用事前声明的“任务交付”替代句 |
| Q | 逐字使用固定外层提示，一次列出预注册的十二个独立新批次；逐指标给点估计和80%预测区间，逐题及总体rationale为英文 | 12题不等于12次调用；查询、指标、参考、评分和计算预算在来源前固定。EC可用6指标，其他体系按卡选指标，不统一六个终点数值 |
| K2 | 逐字使用协议第5节七问并以英文作答，覆盖初始主张、判断变化、竞争解释、一次判别实验、辨识/优化取舍、遗漏证据/不可靠预测及推荐/交付边界 | 引用K1和真实批号，避免重述完整实验表和整篇报告；无真值反馈，不改写K1/Q。全部受评来源使用相同七问 |
...
| PA：已完成两轮单臂开发 | 旧E/P/S资格按其合同；本次12/24批身份暴露不作严格O，当前只考虑E | 自主机理、两相比例预测和选相，不增加独立优化来源 | 公共S/X目录与动态资料已修；新三臂核对实际输入、读数及定制Q，旧数据保持历史身份 |
...
**2026-09-19授权覆盖：** 用户已将原“RX-P单世界六格”扩展并明确授权为 [RX-P/S五世界双目标执行块 v1.0.1](WORK_II_RX_PS_FIVE_WORLD_DUAL_GOAL_PROTOCOL_V1_0_1.md)。当前实际启动分母为 `2 loci × 5 worlds × 3 arms × 2 goals = 60` 个独立source sessions，每场12批，即720 source batches；每场K1/Q/K2，即180 posttests。此授权只覆盖RX-P/S，不自动启动下表EC、PA或C计划。旧的18来源近期四块合计继续作为历史排期，不是本次运行分母。v1.0在0次provider调用时发现S gate继承helper的accumulation符号反向并fail-closed；失败目录保留，v1.0.1只恢复资格设计已使用的`parent - reversible child`方向，未改条件、重复或阈值。

**2026-09-19恢复边界：** 首次正式进程在第16格启动前因共享provider连接失败停止；停止时15格完成12批source（180批），12格完整封存K1/Q/K2。恢复严格按 [recovery v3](WORK_II_RX_PS_FIVE_WORLD_DUAL_GOAL_RECOVERY_V3.md)：两个`tool_budget_exceeded`格在原thread补Q后重做K2，一个provider受损格在原thread补K1/Q/K2；后测MCP与外层tool-attempt监控均设为程序允许的最大值256，题目、schema、query、模型、reasoning effort和信息边界不变。recovery v1误设512并在MCP初始化时停止；recovery v2合法启动MCP，但外层继承的8次上限在保留15次计算、尚无最终Q时终止。两次均无实验、无真值，记录保留；v3始终从原source封存thread而非不完整修复历史恢复。第16格原记录为0动作，故在另一个write-once目录重跑相同冻结cell及确定性seed；其后只运行44个从未启动的格。原`result.json`与失败receipt不覆盖，600次reference truth仍须等60条有效K2全部封存。

**2026-09-19阶段结果：** [recovery v4–v6](WORK_II_RX_PS_FIVE_WORLD_DUAL_GOAL_RECOVERY_V4_V6.md)修复了SSH反向代理生命周期，使第25格在原thread补齐Q/K2并封存；随后第26格完成12批、K1和K2，但Q发生provider failure且无payload，运行再次fail-closed。当前25/60整链有效、300/720来源批进入有效分母；若计入第26格已保留的12批，物理来源完成312/720，任务27–60尚未启动。W01/W02的24条整链、288批和72项后测已形成[逐任务完整可读报告](reports/work-ii-rx-ps-five-world-dual-goal-20260919-w01-w02/README.md)。这只是truth前阶段结果，不产生三臂效应、预测评分、参考真值或推荐复测结论。

RX-P使用[profile v1.2](WORK_II_RX_P_K1_Q_K2_PROFILE_V1_2.md)，RX-S使用[profile v1.0](WORK_II_RX_S_K1_Q_K2_PROFILE_V1_0.md)。在首个provider source前先运行P/S各12批工程路径以及90次RX-S provider-free repeated-noise gate；S gate必须在五world都具备三个独立噪声坐标并通过冻结的accumulation、information-gain和budget-window门。全部60个K2封存后才生成`2×5×12×5=600`次共享参考执行并评分，防止真值污染仍可续接的会话。
...
| 3：RX-P | strict P-Opaque与匹配A/M、早期观测、真实热路径和干预预测题 | 当前专用协议为两目标六格：6来源、72计划批、18后测，共24模型阶段 | 检验竞争过程、终止推断及P层正确/错误先验影响；ContextOnly历史canary不并入 |
| 4：C-P | 十二批完整路径预算、粒径/化学测量、种子扣除及合格交付验证 | 3来源、36计划批、9后测，共12模型阶段 | 检验路径知识能否支持有后果的操作，建立论文深度实例 |
| 5：定向诊断/机制评审 | 依据上述开发经验，在新块前固定分支、对照、节点和评审锚定 | 单独计数，不默认全来源全诊断；不在当前18来源中暗加 | 决定能否从行为相关推进到特定机制解释 |
...
按当前RX双目标六格协议，近期四块合计是**18来源、216个计划完整实验位、54项后测、72个来源/后测模型阶段**：EC 6、PA-E 3、RX-P 6、C-P 3，按顺序逐块执行，不是一键全开授权。此前15来源/180批/45后测的数字是RX仍按单目标三臂时的旧排期，不再作为当前分母。后测题、数值字段和工具调用不增加独立来源。参考答案、校准、建议复测、经典方法、诊断分支与LLM评审尚未全部定量，必须另列，不把216称全部物理成本。

当前EC六场旧题量整链实测61.6分钟；下一版暂按60—90分钟安排已就绪执行，不能承诺精简一定节省某个百分比。PA三臂暂留30—60分钟，RX双目标六格暂留60—120分钟，C三臂暂留60—180分钟，均为工程排期假设、非实测保证。因此这18来源先暂留3.5—7.5小时机器时间，不含实现、公共参考/资格、评审和诊断；C路径可能超出，第一场用实测更新。正式总ETA仍待样本设计，不沿用旧18场剩余时间。
...
### 9.7 W2-127：PA相分配的轻量三臂设计

本节定义PA当前任务，末尾记录随后完成的两轮单臂试跑。PA研究**相间分配**，不是新增一个自发相分离/相图任务；保持一个自主机理探索委托，接预测与选相决策后测，不复制EC的双目标来源，也不自动扩展到参数/结构先验。推进不以重跑EC为前提。

#### 9.7.1 任务及真实能力边界

给Agent的主要委托：在12次独立实验内，自主研究匿名介质、萃取剂、两相用量与混合条件如何影响目标物在两相中的分配，形成可检验的解释，预测新条件，并说明何时应保留哪一相。实验顺序、重复、对照和模型形式由Agent决定，不规定探索/利用比例，不要求还原实现代码或提交预定分配方程。

初期沿用同一已知初始混合物、固定温度的PA切片，保留四种介质与四种萃取剂、体积、混合时长及强度等现有合法操作。这里研究分配与选择，不另加上游反应、洗涤、纯化或全流程回收任务；这些由P覆盖。原生偏重有机相的score保留后台记录，不作为本任务目标、主要评价或必交最佳配方；公开任务提示与结束接口须与此一致。

核对现有实现后需明确：

- [资料生成](../../src/chemworld/materials.py)公开的是跨匿名搭配面板的边际性质，不是完整4×4搭配表，也不保证某材料在每一种介质下都更好。A是正确范围的名义资料，不是每个条件的精确答案。
- [分配实现](../../src/chemworld/world/phase_kernel.py)含材料搭配、体积、混合响应与夹带。当前PA父世界未启用P的新组成耦合族；不能把组成依赖或分子级机理当成本轮必有现象。
- [静置实现](../../src/chemworld/runtime/phase_separation_services.py)主要更新时间/费用和settled状态，不实现随静置时长持续消除夹带的动力学。查询固定合法静置条件，不设计“静置越久夹带越少”的预设正确答案；多次mix也不能未经核对便解释成连续动力学积分。
- `separate_phase`保留目标相并移除另一相，不是单纯切换测量对象。分相后另一通道归零不能解释成此前没有分配到该相。

#### 9.7.2 一个世界、三臂、相同资源

建议首块选PA-W01 / E层，GPT-5.6 Sol / medium，每臂一次、独立会话；开发使用旧W01不算独立确认。三臂同世界、初态、操作、观测、12实验位、12次非终检测量额度和12次终检额度，仅实例资料不同：

| Arm | 实例资料 | 本轮观察的问题 |
| --- | --- | --- |
| Opaque | 匿名操作标签与共同操作/观测说明，无实例性质档案 | 无资料时如何组织取证、建立分配解释 |
| Aligned | 正确对应的名义介质/萃取剂边际资料 | 是否正确利用资料，并检验其对具体搭配的适用范围 |
| MisIndexed | 沿用E层的X0/X3资料交换，其他资料与A相同 | 错配是否改变实验覆盖；实验出现冲突后是否修订 |

三臂均不告知臂名或哪份资料被交换；资料适用范围和可检验性质公开。E层当前有开发资格及challenge支持，P/S不因开发15/15而自动进入本块。该选择不改写Experiment 1正式confirmation/release协议。

#### 9.7.3 一次实验与观测

典型操作为：新鲜初态→选介质与加入量→加水相/萃取剂→自主混合→静置→按需HPLC→选保留相并结束→终检。允许合法调整与重复；这是操作示例，不是替Agent预排12个配方。每次都重置物料，实验之间只保留研究上下文。

明确使用已核对的`partition-s0-extraction-efficiency-v3`观测语义。最关键的科学读数是**丢弃任何一相前的HPLC**；每次HPLC按现有公开合同同时返回两相通道，不虚构为无成本双仪器或后台额外真值。12次非终检额度允许自由分配，推荐测量位置说明属于仪器使用说明，不要求每批必测或强制反证。测量用在别处或重复用尽后，如实保留缺测。

| 公开字段 | 明确定义 | 不应误读为 |
| --- | --- | --- |
| `product_in_organic` | 有机相目标量/本批固定初始目标量 | 有机相浓度、分配系数 |
| `product_in_aqueous` | 水相目标量/本批固定初始目标量 | 被保留相的纯度 |
| `phase_ratio` | 有机相体积/两相总体积 | 有机相体积/水相体积 |

观测摘要保留时点、分相前后、采样损耗与实际读数；分相后终检只解释保留物，不补写此前缺失的两相测量。守恒检查区分仪器噪声、采样与弃相，不要求两个带噪比例精确加到1。评价后台保存真值，不能通过摘要补给Agent。辅助过程时间、库存与操作额度须支持12批，不沿用旧八批合同或隐性时间截断。

#### 9.7.4 后测：三次提交即可

来源结束后K1作为主要知识交付，不再先索要另一篇完整总结或强制最高分配方。依次K1→Q→K2，使用原会话，关闭新增实验访问；Q封存后不反馈答案，K2不改写此前预测。

| 阶段 | 提问/输出 | 规模 |
| --- | --- | --- |
| K1 | 你的分配规律是什么，哪些实验支持它；资料与实验各贡献什么；哪些关系或适用范围尚不能确定？自由文字/方程等，不重抄实验表 | 1次提交 |
| Q | 给定12个操作条件，预测分相前两相目标量比例，各给点估计及90%区间；再回答下述2个决策问题，统一简短说明依据 | 1次批量提交；每条件仅2个核心数值终点 |
| K2 | ①最可能出错或未验证的一条资料/自建规律？②若再有一次实验，怎样区分你的主要解释及替代解释？③哪些证据未使用，哪些预测最不可靠？引用批号即可 | 1次提交，3个问题 |

Q建议固定4个体积/相比例条件、4个材料搭配条件、4个混合条件；成对只改关键因素，固定初始库存和静置条件，避免把材料、体积和过程变化混成同一种外推。主评价范围和操作变量事先告知Agent，12个具体配方在启动前确定且三臂完全共用，不能看来源结果后挑反例。实际被Agent做过的题保留并标注，不冒称严格留出。

同一组Q复用两个决策题，不另开来源或物理试验：①在指定条件保留哪一相以获得更多目标物，并预测保留比例；②在相同总用量约束下，两套指定条件哪套目标物保留量更大。选项须在独立开发时检查不恒为同一答案；比较无实质差异时接受近似相同/证据不足，不强造有利反转。它们检验知识的决策用途，不称完成自主全局优化。

主题尽量落在少数可研究的搭配和连续条件变化上。16个搭配的未知相互作用不能靠12批全部唯一恢复：未测搭配单列外推及校准，允许合理不确定；不把“未恢复整张隐藏表”定义为失败。未来只有要单独研究探索与回收目标的冲突时，才增加独立优化三臂。

评价只分三层：公开轨迹中的取证/修订，K1的正确性与证据边界，Q的两相预测/区间和2项决策。两相比例常近似互补，不能充作两个独立科学样本。先报告连续误差、区间覆盖/宽度、决策及守恒一致性，不设置强行全通过的总分；可用同一实际可见数据的简单均值/近邻参考，不暗给基线完整物理公式或搭配表。未校准的LLM判断只作可追溯定性分析，不阻塞数值评价。

#### 9.7.5 执行组织与最小检查

计划仅**3个来源、36个物理实验位、最多36次额外测量和36次终检、9个后测提交**；含来源共12个模型阶段。12个Q条件一次预计算、三臂共用；参考真值、观测噪声检查和精确重放另计成本，不计为Agent新增实验。每臂24个标量预测、三臂72个，区间端点与两个决策不增加独立来源。就绪后三臂暂留30—60分钟，属工程估计；接入开发另计，首场按实测更新ETA。

启动前只做三项针对性检查：①三臂资料与物理世界分开，公共提示不把原生score当任务目标；②两相HPLC真实可见、字段/分母/弃相/采样解释正确，12批资源可用；③来源结束或失败后的K1/Q/K2与报告贯通，固定题在公共操作域内且不全饱和或决策恒定。复用已有物理资格，只补新接口和查询所涉及的验证，不重建全体系审核包。

首个真实模型会话计入获授权的数据块；合法但低分、没发现规律或没利用证据均是保留的结果，不据此追加重跑。数据送达故障单独标为运行问题，修复另开受影响的新版本块。预期可以看到总体资料被过度推广、物料比例被误读或证据到预测不一致，但出现成功与合理未知同样是有效结果；单世界每臂一次不支持系统性失效结论。

**W2-128执行更新：** 用户只授权一臂，按[试跑note](WORK_II_PA_SINGLE_ARM_TRIAL_NOTE.md)完成来源12批与K1/Q/K2，13.24分钟，[结果](reports/work-ii-pa-sol-20260918/REPORT.md)保留。共享材料目录与操作标签暴露真实名称，严格匿名Opaque无效。第1批逐操作返回，第2—12批各提交完整10操作脚本，批末接收摘要；第2批取错字段，随后用history找回。自主组批符合开放研究设计，不是协议违规。撤回此前“测量/批末必须交回模型”的建议；不把step并发1说成逐操作自适应。

**W2-129预算诊断：** 用户进一步授权一个24批新来源，见[24批note](WORK_II_PA_24_BUDGET_TRIAL_NOTE.md)。12批仍是主预算；本次保留旧公共身份及脚本权限、同世界/模型/后测，复用既有参考；未接续已见过Q的旧会话，未扩三臂。单次对比只作描述，记录增量token、覆盖及预测变化。匿名缺陷需要修正，自主调度继续保留，二者不再并称“接口偏差”。

W2-129现已完成：[12/24结果及token](reports/work-ii-pa-sol-24-20260918/COMPARISON.md)。新来源24/24批、240操作及3/3后测完成，重放误差0；覆盖16/16搭配及时间×转速2×2对照，但两相MAE由0.06338升至0.11443，区间覆盖由12/12降至9/12，决策均2/2。第1批后依次提交7/8/4/4批脚本，每组完成才提交下一组，等待中也有摘要流入；组内配方预写且无读数自适应分支。不能说全部方案在开始时已定，也不能证明反馈改变了后组计划。添加水相的迁移题出现广泛低估；保留不利结果，不据此认定24批有害、模型思考变少或预算已充分。

### 9.8 W2-130：EC与PA当前收束及实际检查边界

#### 9.8.1 保留的设计与已完成证据

| 项目 | EC电化学 | PA相间分配 |
| --- | --- | --- |
| 主要任务 | 机理探索、公开综合得分优化，两份独立委托；优化不等于只追求产率 | 一个机理探索委托，预测分配及选相；暂不增加独立回收优化来源 |
| 下一块核心设计 | 同一开发世界、E层、两目标×O/A/M，6来源/72计划批/18后测 | 同一开发世界、E层、O/A/M，3来源/36计划批/9后测 |
| 来源接口 | 两体系均12批、12次自由分配额外测量、12次终检；自主单批/组批、重复、模型与修订 | 同左；无必做搭配表、组计划、逐批反思或机制模板 |
| 后测 | K1自由报告→Q数值预测与区间→K2简短复盘；新Q已固定6组正/负电位配对，见9.8.4 | 同一顺序；Q为12条件×2互补比例及2项决策，K2三问；主域与迁移域分报 |
| 已完成来源 | 核心6场71/72终检、72中测、18后测；补充2场24终检、24中测、6后测 | 12批和24批各1个新会话，共36来源批、36中测、36终检、6后测；旧参考另12批 |
| 当前证据地位 | 单世界开发，优化A少一批及原失败保留；不能推出稳定三臂排序 | 两来源均暴露真实名称，不是严格匿名O；只用于链路与预算/策略诊断 |

本表是设计规模，不授权启动9场新来源。EC历史排除块、参考、建议复测和重放另计，不能把本表称为全项目累计物理成本。PA参考12批只生成一次、第二轮复用；36来源批与36来源重放批分别计，参考重放另12批。旧文件及机器分母保持原值。

#### 9.8.2 结论纠正与下一步

- **EC：** 可分析不同委托/先验下的实验选择与具体失效；不能根据单世界每格一次确认普遍先验损害、目标冲突或系统性信息损失。小产率MAE受9/12题低于0.02的窄域影响；得分误差、机理依据与建议复测分别解释。
- **PA：** 12批覆盖8/16搭配且混合因素混杂；24批覆盖16/16并做2×2混合对照，仍有体积/操作条件迁移问题。它说明“材料覆盖增加不保证预测改善”，没有区分预算不足、选点、解释/拟合和随机性各自的作用。原先声称“主要是利用率不高”应收窄为待检验假说；不要求少量实验恢复任意未知16格与连续响应的完整真值。
- **可比较性：** 同工具、预算、信息规则下，自主组批属于Agent策略，可以比较总表现和轨迹。不能据两次单来源差异估计纯数据量效应；工具脚本数、token、物理步数都不是内部思考次数。若以后专门研究逐证据修订，可另设统一反馈对照，当前不增加该实验或gate。
- **观测与知识：** 平台必须可靠提供真实购买的读数、定义、时点与历史检索；Agent自行只打印部分结果也需要保留。真实生成、进入上下文、在报告引用和用于预测是不同事实。PA保留相回收不是两相总量；EC/P的丢读数案例不能自动证明普遍信息压缩损失。
- **预算提示一致性：** PA24旧公共instruction残留“完成12批”，与该场系统提示/资源卡的24批不一致；实际完成24批不证明该句无影响。W2-130已移除自由研究公共提示的固定12文字，引用本轮预算；旧24批输入保留，不能升级为严格预算因果对照。
- **当前修正：** PA公共目录与操作标签改为独立S/X匿名ID，并补上动态task_info原先未送达的A/M边际资料；三臂物理操作能力相同。EC改为显式单位清单、过程时间仅记账、库存覆盖操作范围，完整终态失败来源也可后测，K2精简三问；旧暴露身份的数据不升级。报告生成器不再输出强制反馈建议，24批人工分析独立保存，重导出不丢失。无新provider实验、无旧来源重跑。
- **W2-131补齐：** EC平衡查询域已实现并完成固定参考，PA运行入口已支持显式O/A/M与独立三臂串行执行，实际材料工具的匿名/资料送达检查通过，见9.8.4。历史Q保持原窄域身份。仍无修正后真实provider三臂；预定首场须核对实际输入、购买读数、工具使用与同会话后测。当前只开放公共计算器支持的计算/拟合，不将其宣称为无限制笔记或通用代码能力。正式范围、重复数与冻结仍须按所选块固定，功能检查不冒充科学比较。

顺序为固定所选来源块的一份简短note，再由该块预定首场核对真实输入→工具→后测链；可以先PA或先EC，不恢复自动大队列。首场计入该块，不额外要求不断追加smoke。12批是主预算，不因结果不理想再增加次数。RX/C等后续体系仍按自己的科学任务推进，不复制PA问卷或EC双目标。

#### 9.8.3 为什么讨论没有反复提gate

讨论实验策略不产生数据，无需门禁；试跑实际上做了相关检查，但此前未持续使用G-D等名称。另一方面，PA匿名检查确有遗漏，不能用参考与重放通过来掩盖。最小核对如下：

| 检查 | 本轮实际状态 | 什么时候处理 |
| --- | --- | --- |
| 本块设计、分母、停止规则（G-D） | EC范围修改留痕；PA12/24各有事前note；均保留失败，无按成绩补跑 | 每个新数据块开始前，固定该块即可 |
| 问题可研究、操作与观测含义成立（G-S相关部分） | 旧EC资格和PA物料/观测参考可复用其覆盖范围；不保证新自由探索一定学会或唯一识别全部机制 | 新任务/观测/规律变化时补相关证据；不重复无关资格 |
| 三臂实际输入与身份隔离（科学＋G-R） | EC旧目录泄漏块已排除；PA12漏查真实名称，PA24明确沿用旧界面；现已修公共S/X目录和标签 | 严格三臂的新块必须核对实际输入；旧PA不改名为合格O |
| 资源、测量、后测、重放与计数（G-R/G-C） | EC8条来源重放及修正建议复测通过；PA两来源/参考重放通过，后测齐备，token按累计差分；EC主runner基础分支已修，尚有查询/真实路径待验证项 | 开发用定向检查，预定首场验证真实路径；结案报告全部终态 |
| LLM机理评分有效性（G-J） | 目前是可引用原文的定性复核，尚无已校准LLM主终点 | 正式使用LLM分数前校准，不阻止读报告或客观预测评分 |
| 清洁提交、来源绑定、正式资格/发布（G-F） | 当前为开发；旧冻结证据仍描述其旧版本 | 稳定且获正式生产授权后一次冻结；不作为讨论/修文档门槛 |

预测差、错误先验没造成损害、Agent选择组批、没做某个反证实验、12批不覆盖全部组合，都是研究结果或解释局限。只有实际合同错误、信息泄漏、观测错误、记录缺失等才触发相应修复。分析/渲染错误从旧数据重算；修正已污染的执行后另开版本，正式资格块遵守其事前整块重做规则，不静默替换历史结果。

#### 9.8.4 W2-131：已实现的新预测题与PA执行入口

2026-09-19完成[固定验证](reports/work-ii-ec-pa-entry-20260919/REPORT.md)，事前设计见[简短说明](WORK_II_EC_PA_EXECUTION_UPDATE_NOTE.md)。本次新增15个固定参考批、99操作，另15批/99操作精确重放；全部通过，约27.59秒，0模型调用、0重试。126项相关功能测试通过。它验证执行与信息边界，不是新Agent来源或正式三臂结果。

**EC新Q：** 每组分别−0.8 V和+0.8 V，12题依次成对编号。每题独立从同一初始状态开始，追加介质25 mL、试剂0.012 mol，再设电位、电解、终止、终检。以下体积为追加量，不是整批总液体体积。

| 题号 | 条件组 | 电解质ID | 介质ID | 电流上限/mA | 电解时间/s |
| --- | --- | ---: | ---: | ---: | ---: |
| Q01/Q02 | 短时基准 | 0 | 0 | 100 | 600 |
| Q03/Q04 | 长时基准 | 0 | 0 | 100 | 7200 |
| Q05/Q06 | 电解质变化一 | 1 | 0 | 100 | 7200 |
| Q07/Q08 | 电解质变化二 | 3 | 0 | 100 | 7200 |
| Q09/Q10 | 介质变化 | 0 | 2 | 100 | 7200 |
| Q11/Q12 | 电流上限变化 | 0 | 0 | 500 | 7200 |

每组内只改变电位；其余组相对长时基准只改变表列因素。电流是上限，不保证实际值。共覆盖3种电解质、2种介质，不称全材料覆盖或完整规律辨识。来源提示只提前公开控制轴，具体12题在K1封存后一次给出；不预告答案、不要求预定方程。Q继续预测终检的产率、选择性、法拉第效率、传质效率、能效和公开综合得分，共72个标量点估计及80%区间，一段共用依据；K2仍只三问。

新题固定参考的产率范围为0.00202—0.23459，2/12低于0.02；旧题是9/12。新题保留短时低响应，不按结果剔题。总体及正/负电位、各上下文分别报告误差与区间，避免一个平均数掩盖跨条件差异。这是参考域诊断，不能推断新模型表现一定更差或三臂差异更大。旧设计、旧题和旧后测文本保持原值；续接旧记录从其保存的design取题，不调用当前题目覆盖旧证据。

**PA入口：** `--arm Opaque`可选单臂；`--arms Opaque Aligned MisIndexed`只运行显式列出的三臂。每臂使用新的中性工作目录和独立模型会话，物理世界、噪声规则、任务和12批/12额外测量/12终检预算相同。O只有匿名S/X目录；A增加名义边际资料；M只交换X0/X3的资料，其他资料不变。臂名留在宿主账本，不进入公共研究目标。自由单批/组批及自主解释形式保持。
...
### 9.11 2026-09-20：结晶正式实验优化

用户已授权全面优化结晶并组织正式实验，当前执行说明为
[结晶校准与正式实验](WORK_II_C_FORMAL_NOTE.md)。该新块与仍在运行的单世界预实验分开；
旧题目、旧失败和旧结果不改写，也不转为正式样本。

准备采用五世界 × 12/24批 × E三臂，共30个独立来源、540个来源批次、90组K1/Q/K2，
另计30次推荐复测。模型仍为GPT-5.6 Sol / medium，仅一个“质量约束下提高扣晶种净回收”任务。
预算选项已向用户询问，未另行调整前按30场准备；不自动增加P/S或机理探索第二任务。

旧预实验的19批参考已全部完成并精确重放；它同时暴露出旧预测题12题均不合格、四对效应近似持平。
新块先在独立开发世界按事前固定的24个候选校准，从六个因素各选一对，共12条不同配方；
选题依据为物理覆盖和质量边界，不使用Agent成绩。全部候选和失败保留。
选题冻结后，五个正式世界逐一执行12条题目及O/A/M相同动作探针，共75批参考，重放另计。
每世界必须同时有可行见证、至少3条质量正例和3条负例、至少4对有实质效应；不删不利世界或临时换题。

主终点、三臂资料干预、自由机理形式和自由实验顺序不变。粒径使用明确的无量纲指标；
“是否有颗粒”因终检要求已有分离晶体，只作状态记录，不再作为主要准确率。
连续预测报告MAE、80%区间覆盖、宽度和interval score；质量判断报告混淆计数与平衡准确率；
效应量误差、可分辨方向和近似持平分别计分。用每个来源已公开终检数据建立均值与近邻参考，
不把后台真值拟合器包装成模型可用基线。K2仍为结晶的三问，不套用RX七问。

开发期间只做定向检查；稳定后一次提交执行面并冻结，随后执行固定资格块和正式来源。
实时报告区分启动、批次完成、整链封存与科学达标，保留失败、资源和原会话后测。
开发校准24/24批及重放已完成：选定题集有3条质量正例、9条负例、2条有效回收可行见证，
六组因素对照均至少有一个指标超过预设分辨尺度。61项综合功能检查通过，执行面已在
`c0ba5647`提交并完成首次冻结。首次资格进程在62次操作/3次终检后消失，没有完成世界验证，
正式provider仍为0/30。2026-09-20修复只改变推荐配方/公开参考方法对拒绝动作的处理以及后台启动、状态显示；
原预实验36批/9后测保持不变，单独补做MisIndexed推荐复测，得到净回收0，旧失败保留。
新执行版本沿用原冻结题集、提示、阈值和30场规模；重新从首个单元执行75批固定资格验证，
通过后单执行者接入正式来源。原3次终检/62次操作另记资源，不计入新验证完成量。
实际队列状态以[正式运行报告](reports/work-ii-c-formal-20260920/REPORT.md)为准。

### 9.12 跨体系共用执行优化与当前结晶状态（2026-09-20）

W2-139已完成共用执行层优化：批量计算动作合法性、设备字段按需复制、同次只读请求复用资源快照、复用公开报告。
后续EC、PA、结晶、蒸馏、流动反应等任务直接使用这一实现，不需要各体系重复开发或增加一套资格证书。
15个注册任务配置、31个状态的公开内容/动作验证一致；这表示软件覆盖，不代表15个独立科学体系通过资格。
结晶同一201次操作/12批参考连同精确重放，从历史1,357.672秒降至19.141秒；逐步输出、真值与资源消耗一致。
132项定向回归通过。物理模型、求解精度、动作约束、资源记账、失败保留、原实验预算均不变。
测量范围与耗时边界见[英文报告](reports/work-ii-runtime-optimization-20260920/REPORT.md)。

新体系沿用轻量流程：先确认公开资料/指标定义和预算实际送达，再执行有代表性的完整配方，
检查合法性、可行见证、预测题覆盖与重放；一次开发验收后再冻结正式块。运行时保留进度与资源；
不因文档或无关代码变更反复重跑全局审计。后台资格计算不调用模型，模型来源研究及后测耗时单独统计。

当前C正式块不能直接接入模型。旧v2保留646次操作/40次终检：W01通过；W02只有2条质量正例，
低于事前3条；W03/W04固定冷却目标高于实际温度而被拒；W05完成4批后主动停止。
v1另有62次操作/3次终检。正式模型仍为0/30；未改资格阈值、删世界或抹去失败。
下一块应明确重设计可跨世界执行的冷却路径及题目覆盖后重新冻结，不机械重跑已知有缺陷的配方。

另已纠正未来C提示：`crystal_yield`为扣除晶种后的结晶阶段回收率，分母为分离前已有目标产物，
不是原始反应物投料。历史预实验措辞错误保留并披露；不能将其定量预测失误全部归因于模型。

### 9.13 结晶世界设定与参考域诊断（2026-09-20）

W2-140完成第一轮修正及单独的可行域诊断，见[执行说明](WORK_II_C_WORLD_REDESIGN_NOTE.md)、
[初次验收](reports/work-ii-c-world-redesign-20260920/REPORT.md)及
[全材料/浓度域结果](reports/work-ii-c-world-redesign-20260920/domain/REPORT.md)。
先将参考配方的300 K起始条件落实为计时计费的实际操作，并统一回收率分母说明；
108批开发校准通过，随后75批验收完整完成，但仅4/5世界满足原十二题覆盖标准。
W04动作与重放均正常，失败原因为12题全超过细粉上限，不能据此认定世界无可行工艺。

另行事前固定四溶剂×三投料量×三体积，在五个既有世界和两个新增开发世界执行252批，
全部执行与重放通过。各世界有11、7、9、8、10、10、10条可行配方（每世界36条）。
存在同一公共可执行配方在七世界均合格，阶段回收率20.6%–29.6%；它是后台见证，不给agent规定路线。
因此保留现有物理/材料分布、三臂资料干预、12/24批预算和质量阈值；下一块重设计六对参考题，
围绕已确认的可行域保留材料、播种、冷却、复热、继续生长和上游投料对照，再从第一单元重新资格验证。
旧十二题失败保留，正式模型仍未启动。此次开发数据不能自动晋升正式证据。

当前纯度阈值大多不活跃，细粉与中位粒径来源于同一颗粒分布；不把四个输出称为四种独立困难。
阶段回收率也不等于总反应产率或新增产物质量。252批含重放累计约32.9分钟，中位每配方1.95秒，
最长61.64秒；S2占主要计算时间。共用执行层已加速，但颗粒群求解耗时仍依工况变化。
本项未改变物理精度，也未放宽失败规则。共享优化已单独推送main，世界诊断保持独立开发记录。

### 9.14 新题集收束与正式来源启动（2026-09-20）

用户授权快速推进。执行以[固定v3说明](WORK_II_C_REFERENCE_V3_NOTE.md)为准；
