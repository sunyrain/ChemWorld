...
| W2-51 | DESIGN + PROVIDER-FREE QUALIFICATION IN PROGRESS / PROVIDER NOT AUTHORIZED | 新 evidence-to-action 五条件因果分解固定为 3 tasks × 5 fresh worlds × 3 priors × 5 conditions：225 fresh sessions；仅 45 autonomous donors 执行 12 轮，共 540 participant experiments。No-evidence 首次见候选即直接排序；artifact-only fresh context 同时接收规律与候选；autonomous/yoked 完成证据阶段后揭示候选。旧 hash-split 开发块完成 240/240 truth/replay，但候选门仅 11/15；不读真值、跨 world 固定的 public-feature maximin 重分组为 15/15，三任务 raw score range 最低分别 0.598/0.275/0.195。8-point oracle 仅 3/15 通过；修正 categorical coverage 后的 96-point global grid 完成 1,440/1,440 provider-free truth，但仍仅 7/15 通过（electrochemical 3/5、crystallization 0/5、reaction-safety 4/5）。该失败冻结为 locality/expressiveness 证据；新块维持 96 点总量，固定 32 global + 64 outcome-blind candidate-neighborhood，并扩展同一 typed schema 的 cubic 与 categorical-conditional 项后从 0 开始。首次 hybrid launch 因 verbose query ID 触发 Windows 路径上限而在首 cluster 停止、无可用科学结果；仅缩短 ID 后在新输出根从 0 重启。recipient reveal/hidden-field/terminal runtime、donor dependency、yoked checkpoints、clustered analysis 与 law-action agreement 均已实现；provider execution 未授权。 |
