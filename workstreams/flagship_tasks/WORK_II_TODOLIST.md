...
具体范围和停止规则见[收束矩阵](WORK_II_EXPERIMENT_MATRIX.md)；开发12/12终止（10有效完成、2失败），正式120会话待冻结后启动。
...
| W2-77 | DOING | Claim: Codex /root — 开发12/12尝试、24调用，10有效完成、2输出上限后重连失败，均保留；3次数值工具计算成功。固定GPT/DeepSeek正式120 sessions、600秒/轮、1200秒/session、18小时整块、内部重试0；冻结一次后完成整块。第三模型暂缓，不延续旧B3分母 |
...
新增执行面仍处development；已启动独立开发校准，正式执行须在校准完成后冻结预算和执行面。
