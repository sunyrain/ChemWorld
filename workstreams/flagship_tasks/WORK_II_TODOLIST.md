...
用户最新要求恢复原设计：Flash/low原正式块已于09:40恢复，第005会话运行中；
此前2完成、1工具预算失败、1用户中断原样保留，从第005个会话续跑，不重发已尝试单位。
原12小时日历截止仍为2026-09-07 11:39:38（+08:00），不扣除暂停或重置预算；
56个按既有速度需6.25–6.53小时，预计原窗口内无法全部尝试，到点保留未启动分母。
使用原冻结源码与low配置；none独立6会话资格0/6通过（3格式失败、3输出上限）单独保留，none正式不启动。
两种披露条件沿用相同low配置，不声称与GPT/medium等算力；累计usage修正在退出后的分析中应用。
...
| ID | 状态 | 工作与完成标准 |
| --- | --- | --- |
| W2-87 | DOING (GPT DONE; DeepSeek low RESUMED) | Claim: Codex /root — [信息完整性干预](WORK_II_INFORMATION_COMPLETENESS_EXPERIMENT_NOTE.md)：GPT 6开发+60正式完成，恢复6/20→20/20、主差+70个百分点，报告/图件完成。用户恢复Flash/low原设计：保留2完成、1工具预算失败、1用户中断，09:40已从005恢复（2完成、2失败、1运行、55未启动），原日历截止9月7日11:39:38不变；到点不补跑。原冻结执行源码，none的0/6资格失败及早期失败块单独保留。累计usage修正与原停止报告保留；退出后新导出报告，未完整终态前不合并双模型主效应，稿件整合待后续 |
