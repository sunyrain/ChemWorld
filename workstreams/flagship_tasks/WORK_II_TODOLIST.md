...
最后更新：2026-09-08。执行者：Codex /root；main 分支。科学实验终态保留；W2-90实验讲解、图件改版与论文交付已验收。
...
W2-87双模型已于2026-09-07 13:37终态：120/120计划会话均已尝试，112有效完成、8失败/历史中断，原正式块无未启动或替换。
GPT 60有效；DeepSeek 52有效、7执行失败、1历史用户中断。未知/错误先验恢复分别6/20→20/20、3/20→17/20，
两配置均+70个百分点；共享10世界合并主差+70，近似95%[+52.5,+85]，正确先验保持分别8/10→10/10、6/10→10/10。
结构恢复不保证行动改善：GPT Top-1 5/20→12/20、失败计入regret 0.07097→0.00748；
DeepSeek Top-1 6/20→4/20、regret 0.15090→0.24263，完整合同有效恢复17/17仍未带来同方向行动收益。
原60分母、失败、none资格失败及用户取消日历截止/开启3并发的修订全部保留。三并发段39单位实测99.44分钟，未触发429降档。
两模型机器/可读报告及共享世界合并报告已绑定current，累计usage去重；60条DeepSeek评分、119份实际提示词和60个独立thread复核通过。
用户随后授权的W2-89一次补跑已于15:06完成：8/8有效、16/16轮，实测20.10分钟；7例结构恢复/保持、1例有效但答错，全部保留。
事后选择的敏感性分析中，DeepSeek恢复5/20→20/20、Top-1 7/20→5/20；原120会话主分析与8个失败不变，总计保留128次尝试。
双模型比较图、DeepSeek独立图、两份稿件、PDF及匿名复现包均已整合并验收；实验和图文余项0。
W2-90已补充四层实验的对照、分母、解释边界和两个实测案例，更新核心图表并完成两版导出。
下一步为作者终审和投稿账户事项；阅读顺序、当前页数、包内容与验证范围只在[投稿清单](../../paper/ICLR_2027_SUBMISSION_CHECKLIST.md)维护。
...
M0/M1开发结果独立保留；当前交付已纳入双模型信息完整性干预与单列补跑分析。
历史各轮验收数字保留在原任务行，当前交付以投稿清单为准；M2/M4暂缓。
...
> “四层断裂”保留为研究起点；当前结论是四个转换问题及其不同读出，不是四个均已证实的能力故障。
...
M1/M3提供观察到的成功设置及边界，没有识别跨协议差异的单一原因；是否超过nearest不作为诊断论文成立或结束实验的门槛。
...
- W2-87：固定v3环境下，仅补全post观测映射，GPT未知/错误先验恢复6/20→20/20，
  DeepSeek为3/20→17/20；共享10世界合并提升70个百分点，近似95%区间[+52.5,+85]。
  GPT行动Top-1由5/20升至12/20，DeepSeek由6/20降至4/20；结构恢复与行动收益须分开解释。
  结论限于两配置和同族参数覆盖；全部失败计入原分母，新结果未回填旧B3。

## 3. 当前任务

2026-09-06独立评阅已完成，用户随后授权优先尝试解释成功/失败条件的单因素干预。
Claim: Codex /root — W2-90 — DONE；实验讲解、关键图表和两版论文导出已完成。科学实验余量0，进入作者终审。
...
W2-79–81最终收尾：21项相关回归通过；桌面/手机浏览器无脚本错误或页面横向溢出。
手机图表保留12px字号并支持触摸/键盘横向滚动；[场景和证据预览](../../apps/blender_lab/README.md#explore-in-a-browser)
已保存。当前探索地址为本地服务的`/explore`，启动方式见同一README。

历史W2-77正式终态：GPT为59有效/1失败，DeepSeek为58有效/2失败，各计划60；全部240轮已尝试。
失败为DeepSeek post提交校验、GPT传输中断、DeepSeek输出上限各1例，均保留。会话wall合计9.34小时。
GPT在0/30工具开放会话调用，DeepSeek在20/30调用；工具可用性主效应不等于强制计算的效果。
无剩余必做实验，W2-78也已完成；下一步为作者审读和投稿账户事项，不恢复暂缓扩展或补齐历史失败分母。
...
新增执行面已完成独立开发校准，并在正式块首单元前冻结预算和执行面；完整正式块按该设计终止。
...
- 主证据涉及GPT-5.6-sol/medium与DeepSeek-v4-flash，Codex为共用harness；DeepSeek按研究块保留high/low设置，W2-87及W2-89使用low。
...
上述记录描述M3历史发布段；W2-87信息完整性干预和W2-89一次补跑均已交付，当前进入作者终审。
