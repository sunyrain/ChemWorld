...
WellAU/Qwen/Kimi 未启动新的 replication。W2-61/62/63 已把 C2、B3 与 oracle-free 四条件 action
successor 补为双模型 scheduled coverage：C2 各 `135` cells、B3 各 `30` cells、四条件 successor 各 `180`
slots；A-P/B2 另有完整双模型 matched formal 分母。原 W2-50 纵向队列与 W2-64 decision-aligned
diagnostic 仍为 DeepSeek-only。所有失败和删失保留，因此该补全不等于全 cell completed、
...
反证明确定推翻，支持 conditional post-packet response。A-S B2 的 misindexed-minus-aligned update-gain contrast 为
`+0.0645`、3/5 worlds 为正、exact one-sided sign-flip `p=0.125`，misindexed 0/5 表达 exact 1.75 law。
participant-visible audit 显示 B2 的 one-pair free-text surface 存在精确 linear/power alias，结构族不可识别；
因此 B2 收束为 numerical--exact-law-expression diagnostic，participant-identifiable structural test 由 B3 承担。
...
- **B — matched-evidence falsification**：2 loci × 1 task × 5 worlds = 10 clusters、30 fresh sessions。
  ...
  A-S B2 15 sessions：A-P 支持 conditional post-packet response；B2 给出 mixed prediction contrast，
  同时 0/5 exact-law expression。B2 表面不具结构可识别性；原 Study B A-S 分支因 evaluator truth 缺陷降为 historical。
...
当前 B/B2 已全部执行并终态。C3 的二元跨-locus 强主张不受支持；允许的结论是 A-P conditional response 与
A-S B2 underidentifying-surface numerical--expression dissociation 并存。结构识别由独立 B3 检验。该结果是终态科学
...
- [x] Matched-evidence 机制块已终态：A-P Study B 15/15 current；原 A-S Study B 15-session branch historical；
  A-S B2 15/15 current。B2 主对比 `+0.0645`、3/5 positive、`p=0.125`，misindexed 0/5 exact-law expression；
  one-pair free-text surface 不具 structural-family identifiability。
...
| W2-19 | TERMINAL / MECHANISM-BOUNDARY CLOSURE | 当前有效 matched-evidence 30 sessions terminal；A-P 支持 conditional post-packet response，A-S B2 为 mixed prediction signal + `0/5` exact-law expression on an underidentifying surface；participant-identifiable structural test 由 B3 提供，二元 C3 强主张不支持，不再追加同类 block |
...
| W2-51 | DONE / TERMINAL SCIENTIFIC REJECTION RETAINED; 96-GRID ROUTE CLOSED | 工作包已经完成，完成不等于科学门槛通过。原始 W2-51 事实不变：五条件设计计划 3 tasks × 5 fresh worlds × 3 priors × 5 conditions，即 225 sessions、45 autonomous donors 和 540 participant experiments；restart 的前 8 个 formal clusters 完成 896/896 provider-free truth/replay，candidate gates 8/8、oracle gates 7/8。`reaction-to-crystallization / seed836245547` 的 oracle Spearman `rho=0.738095<0.80`、Top-1 disagreement、fit/candidate overlap=0，触发冻结 stop rule；余下 7 clusters、operational canary 及全部 participant execution 均未启动，provider calls=0、result replacement=0。该失败永久保留，不回写或复活。其后三个独立升级也在全新 held-out world 科学失败：v0.2 为 `electrochemical / seed762707071 / rho=0.785714`；v0.3 在精确 typed-law 蒸馏后为 `reaction-to-crystallization / seed468887863 / rho=0.595238`；最终 v0.4 固定 ExtraTrees 在 25/25 exposed construction worlds 回放通过，但在 `electrochemical / seed241995082` 为 `rho=0.785714`，计划 15 clusters 中仅完成 5、通过 4、失败 1、10 未启动，总 truth/replay 560/560，candidate outcomes read=0、provider calls=0。v0.4 失败单元 design rank=8、蒸馏误差 `2.13e-14`，故当前瓶颈是 96-grid 对新 world 的候选排序外推，不是 typed-law 表达；当前路线关闭，不建立 v0.5、不删失败 world、不降 `rho>=0.80`，participant/formal 均不授权。它不再阻断当前 ICLR 写稿或其他独立实验。原始收口见 `reports/WORK_II_EVIDENCE_TO_ACTION_FORMAL_CLOSEOUT_ZH.md`；升级收口见 `reports/WORK_II_EVIDENCE_TO_ACTION_ORACLE_V2_QUALIFICATION_CLOSEOUT_ZH.md`、`reports/WORK_II_EVIDENCE_TO_ACTION_ORACLE_V3_QUALIFICATION_CLOSEOUT_ZH.md` 与 `reports/WORK_II_EVIDENCE_TO_ACTION_ORACLE_V4_QUALIFICATION_CLOSEOUT_ZH.md`。 |
| W2-52 | DONE / TERMINAL LARGE-GRID CONSTRUCTION PASS + PROSPECTIVE RANK REJECTION | 工作包已经完成，完成不等于把首个 prospective 失败改成通过。用户授权将 oracle grid 从 `96` 扩为 `320`（64 global + 256 candidate-neighborhood），ExtraTrees 保持固定以隔离 coverage 效果。首次运行因 typed-law `evidence_ids` 超过 128 项 schema 上限形成 platform-defective partial：1 个完整单元、1 个 partial，共 `496/496` truth/replay，原样保留且不复用；修复为顶层保留全部 fit provenance、law 均匀引用 128 IDs 后，从新 root 完整重启。exposed construction `7/7` 通过、总 truth/replay `2352/2352`，四个历史失败均修复，最低 `rho=0.857143`。但独立 prospective qualification 的首个新 world `electrochemical / seed799649867` 完成 `336/336` truth/replay 后仅 `rho=0.714286<0.80`；candidate gate passed、Top-1 agreement=true、design rank=8、蒸馏误差 `3.55e-14`、fit/candidate overlap=0、candidate outcomes read=0。按冻结 stop rule，余下 14 clusters 未启动，provider calls=0，formal reserved seeds 未触碰。结论为增加 grid 能修复已知 construction failures，但完整排序门与动作目标不等价；不恢复 W2-51，不授权原 participant/formal cohort，也不继续在同一 qualification seed 上调参。该终态结果不再阻断当前 ICLR 写稿或其他独立实验。收口见 `reports/WORK_II_EVIDENCE_TO_ACTION_LARGE_GRID_V1_CONSTRUCTION_CLOSEOUT_ZH.md` 与 `reports/WORK_II_EVIDENCE_TO_ACTION_LARGE_GRID_V1_QUALIFICATION_CLOSEOUT_ZH.md`。 |
| W2-53 | DONE / TERMINAL ORACLE GATE–ACTION ALIGNMENT DIAGNOSTIC | 对 W2-51/W2-52 全部 `16/16` 已完成 unit-version 做冻结回顾分析，原 Spearman 与 Top-1 `16/16` 精确复现，fit/candidate overlap=0，新增 truth/provider/物理实验均为 0。W2-51 的 96-grid fresh formal-preparation 单元为 rank gate `7/8`、Top-1 `1/8`、距最优 `<=0.01` 为 `3/8`，其中 `6` 个 rank 通过但 Top-1 错；W2-52 的首个 fresh 320-grid prospective 单元则 rank gate 失败但 Top-1 正确、normalized regret=0。结论是完整排序泛化与动作充分性分离；不改变 W2-51/W2-52 历史结果、不降低原 `rho>=0.80`，但两工作包按终态结果关闭并移出当前 ICLR 阻断项。未来新 control 必须 prospectively 以 regret、近最优选择和 near-tie-aware ordering 对齐动作目标。见 `reports/WORK_II_EVIDENCE_TO_ACTION_GATE_ALIGNMENT_ZH.md`。 |
| W2-54 | DONE / TERMINAL DEVELOPMENT PILOT WITH YOKED RIGHT-CENSORING | 按 W2-50 原 input-manifest 首 cell 固定 `electrochemical-conversion / seed0 / opaque`，只读复用 autonomous Top-1 donor，并执行三条件试点。`no_evidence` 完成 1 session：真实 rank 2、normalized regret `0.012386`、距最优 `<0.01`、pairwise `0.846154`；`learned_law_only` 完成 1 session：Top-1、regret `0`、pairwise `0.653846`，低于 autonomous 的 `0.730769`，方向上再次显示 action quality 与完整排序分离。yoked 首次真实路径暴露 provider 不支持 `oneOf`，第二次暴露 opaque schema 与 validator 漂移，均保留；最小 contract repair 在同一新线程保留 `5/5` belief turns，final snapshot 引用全部 `85` 个公共事件，当前 canonical validator 在 128-ID 上限下对五个 raw turns 零 provider 重验证通过。进程退出后尝试恢复第 6 个 terminal turn，但原 executor 的临时 Codex home 已清理本地 rollout；CLI 在 `1.502 s` 内、provider 前失败，receipt 为 `thread_id=null`、usage 空、tool events `0`，故不计新增 provider call，也不以新线程拼接上下文冒充 same-thread session。最终新 recipient sessions 为 `2/3` completed，yoked 为 `5/6` turns 后右删失；累计 provider attempts 仍为 `9`，另有 1 次本地 resume-launch failure，0 tool contamination、0 新 truth/物理实验。旧失败、五个 snapshots 与 terminal-resume failure 全部保留，不补跑、不替换。该单-stratum development pilot 只支持 apparatus/方向性观察，不作因果或 arm-level 结论，工作包按右删失终态收束。 |
| W2-55 | DONE / REVIEWER CONTROL ANALYSES INTEGRATED | 对原 W2-50 `45` 个 scheduled cells 做连续与阈值敏感性分析，固定保留 3 个历史 failures，只让原本合格的 `42` cells 进入关联分析。final-law MAE 与 selected rank 的 pooled Spearman 为 `-0.073`（task-world cluster bootstrap 95% interval `[-0.380,0.256]`），与 normalized regret 为 `-0.133`（`[-0.452,0.217]`）；0.05--0.30 七阈值表与 task-stratified 结果均已输出。另在 current-composite `135/135` cells 上执行 production parser/executor schema-capacity control：full registered basis 对 participant final prediction 的 mean MAE 为 `4.25e-13`，participant law 为 `0.1539`，participant-term-matched 为 `0.0114`，leave-one-query-out 为 `0.0788`。因此主要损失定位为 participant 的稀疏规律蒸馏，而不是 typed-law 接口容量；该 control 是同域 capacity analysis，不宣称全局机制恢复。新增 participant/provider/物理实验均为 0。见 `reports/WORK_II_REVIEWER_CONTROL_ANALYSES_ZH.md`。 |
| W2-56 | DONE / DEEPSEEK CANARY REJECTED + GPT FORMAL 30/30 COMPLETED | replicated B3 provider-free preparation 完成 5 development worlds、`1280` candidate-grid truth executions、`480` 新 exponent-grid executions + `40` reused targets、5 public worlds / `160` truth executions及 30-cell manifest；action-opportunity eligible worlds 为 `3/5`。DeepSeek 三 session canary 原样收束为 1 completed + 2 participant-schema failures，formal `0/30`，不得补跑或把单个 canary 当科学分母。随后用户独立授权完全匹配 science surface 的 `gpt-5.6-sol / medium` replication：canary `3/3` 通过，formal `30/30` cells、5 worlds × 3 arms × 2 sessions 全部 completed，失败 `0`。opaque/aligned/misindexed 的 post MAE 为 `0.0367/0.0215/0.0378`；family+1.75±0.10 recovery 为 `0/10`、`5/10`、`0/10`，aligned exponent error 相对两个对照均在 `5/5` worlds 更低。Top-1 为 `0/10`、`2/10`、`0/10`，两次 Top-1 均来自 action-opportunity 不成立的 world；eligible gain≥0.02 为 `0/18`。资源为 33 provider attempts、66/66 completed turn receipts、0 retries、0 tools、0 physical experiments；1 个 transient provider error event 在同一 post turn 内最终 completed 并保留。GPT 结果支持 provider-specific 三臂结论，不支持与无 formal 分母的 DeepSeek 做 leaderboard。见 `reports/WORK_II_AS_STUDY_B3_REPLICATED_CANARY_CLOSEOUT_ZH.md`、`reports/WORK_II_AS_STUDY_B3_GPT56_SOL_MEDIUM_PREPARATION_ZH.md` 与 `reports/WORK_II_AS_STUDY_B3_GPT56_SOL_MEDIUM_FORMAL_CLOSEOUT_ZH.md`。 |
| W2-57 | TERMINAL / DEEPSEEK CANARY 2/3 + GPT UNSTARTED | shared-index v2 保持相同 5 worlds × 3 arms × 2 replicates 与公开 `action_index=0..7`。provider-free preparation 完成 `1280/1280` candidate、`520/520` exponent、`160/160` public truth；双模型 qualification/public truth/roster/public-packet hashes 一致。DeepSeek 三 session canary 的 `6/6` provider turns 均 completed，0 retries、0 tools、0 infrastructure failures；aligned 与 misindexed 完成，opaque 的 post payload 保留合法 `selected_action_index=3`，但冗余 status 错写为 `pre_submission_complete`，形成 1 个 participant-schema failure。故 canary 终态 `2/3`、不补跑；按冻结顺序 GPT canary `0/3`，DeepSeek/GPT formal 均 `0/30`，outcome replacement=0。旧 W2-56 结果继续保留且不混入。见 `WORK_II_AS_STUDY_B3_SHARED_INDEX_CROSS_MODEL_EXPERIMENT_NOTE.md` 与 `reports/WORK_II_AS_STUDY_B3_SHARED_INDEX_DEEPSEEK_CANARY_CLOSEOUT_ZH.md`。 |
...
| W2-59 | DONE / TERMINAL BLOCK-SPECIFIC CROSS-MODEL COVERAGE | 冻结的 `270` 个 formal sessions 中终态观察 `36`、合格/可评分 `34`、保留失败 `2`、未启动 `234`；另有 `12/12` 排除式 canary 已观察，`11` completed、`1` participant-schema failure。A-P 与 A-S B2 首次形成完整 DeepSeek + `gpt-5.6-sol`/medium matched formal 分母：两模型均各完成 `15/15 + 15/15`、0 failures；A-P primary contrast 为 `0.0309` 与 `0.0602`，A-S B2 为 `0.0645` 与 `0.0915`，且两模型 misindexed exact 1.75-law expression 均为 `0/5`。B2 的 one-pair free-text surface 不具 structural-family identifiability，故不把该计数解释为结构恢复。GPT C2 在分母内 canary 为 `3/135` terminal、`2` qualified、`1` provider/session qualification failure，余 `132` 未启动；GPT W2-50 为 `3/45` terminal、`2` eligible，misindexed 在 `5/12` experiments 后 session interruption，余 `42` 未启动；B3 的 DeepSeek/GPT canary 为 `2/3` 与 `3/3`，因共同门要求双方通过，formal 均 `0/30`。首次 GPT B3 零 provider 平台缺陷保留，修复后从 canary 第一单元重启；其余失败均不补跑、不替换。W2-55 对 GPT 新输出零 provider 检查后因 stop-boundary 分母不足而不可估计，不与 DeepSeek 拼接；provider-free W2-51/52/53 未重复。W2-59 无合法可继续 provider session。见 `WORK_II_CROSS_MODEL_MAIN_EVIDENCE_COMPLETION_EXPERIMENT_NOTE.md` 与 `reports/WORK_II_CROSS_MODEL_MAIN_EVIDENCE_COMPLETION_CLOSEOUT_ZH.md`。 |
| W2-60 | DONE / TERMINAL B2 LOW-REASONING ROBUSTNESS; A-P PLATFORM-DEFECTIVE PARTIAL | 为保持 agent harness 不变，冻结为 `deepseek-v4-flash` + Codex `reasoning_effort=low`，不是会换 direct controller 的 provider `thinking=off`。A-S B2 canary `3/3` 通过，formal `15/15`、30/30 turns、same-thread `15/15`、0 failures、80/80 provider-free truth；opaque/aligned/misindexed post error 为 `0.0067/0.0069/0.0069`，全部 15 cells 低于 0.02，misindexed exact 1.75-law expression 仍为 `0/5`。注册 primary contrast 反向为 `-0.0405`，`2/5` worlds 为正，exact one-sided `p=0.8125`；provider-reported reasoning output 相对 high 从 `506,637` 降到 `400,639`（-20.9%）。结论是 numerical--expression pattern 对该 reasoning budget 稳健，但 selective-update contrast 方向配置敏感，不作 reasoning/model leaderboard。A-P low canary 的外层前台监督被超时截断：三臂有进度与 terminal-failed 事件，但无 cell receipts/canary summary，故 root 按 platform-defective partial 保留，formal `0/15`，不补跑、不作 A-P low 科学估计。见 `WORK_II_DEEPSEEK_LOW_REASONING_MATCHED_EVIDENCE_EXPERIMENT_NOTE.md`、`reports/WORK_II_AS_STUDY_B2_DEEPSEEK_V4_FLASH_LOW_RESULTS_ZH.md` 与对应机器结果。 |
| W2-61 | DONE / TERMINAL FOUR-CONDITION CROSS-MODEL ACTION EXTENSION | 在 W2-50 冻结的 45 task--world--prior strata 上移除 oracle，形成 DeepSeek 与 GPT-5.6-sol 各 `45 × 4=180`、总计 `360` 个 failure-aware 条件槽：`no_evidence`、`yoked_evidence`、`learned_law_only` 与 `autonomous_exploration`。primary all-scheduled failure-aware strategy estimand 保留每个模型全部 45 strata：autonomous-minus-no-evidence regret 差为 DeepSeek `-0.0913`（95% cluster interval `[-0.2124,0.0388]`）与 GPT `+0.1102`（`[-0.0533,0.2794]`），方向不同且均跨零；learned-law-minus-none 为 `-0.0028/+0.2459`，没有一致 benefit。DeepSeek/GPT donor-eligible 分母 `42/26` 及其 autonomy 估计 `-0.1214/-0.1379` 仅作 post-treatment availability sensitivity；共同可配对为 `26` strata（13 clusters）。yoked completion 为 `10/42` 与 `24/26`，故 autonomous-minus-yoked 的 failure-aware 优势不能与纯 experiment-selection effect 等同。所有 donor、recipient、schema 与 session failures 均保留；不作 provider 因果效应或模型排序。见 `WORK_II_W250_ACTION_ALIGNED_CAUSAL_EXTENSION_EXPERIMENT_NOTE.md`、`reports/work-ii-w2-61-cross-model-action-aligned-causal-extension-v0.1.json` 与 W2-64 publication reanalysis。 |
| W2-62 | DONE / TERMINAL GPT-5.6-SOL C2 FULL-COHORT REPLICATION | `gpt-5.6-sol`/medium 在与 DeepSeek 完全相同的 45 task--world clusters、9 tasks、3 arms 上形成全 `135` scheduled-cell 分母：`126 completed + 3 failed + 6 right-censored`，完成 `1,253/1,260` participant experiments。provider-free current-composite evaluator 完成 `420/420` truth、`669/675` checkpoints、`129/135` laws、`756/810` scheduled blind executions；54 个未启动 blind executions保留。GPT-5.6-sol 的 A-E/A-P/A-S selective-correction gates 均失败，overall gate `p=1.0`。与 DeepSeek-v4-flash 的 135-coordinate matched descriptive analysis 得到 prediction improvement `0.1329/0.1198`、law MAE `0.1753/0.2371`、compression loss `0.0142/0.0686`、blind gain `-0.0001/-0.0010`（GPT/DeepSeek）；规律压缩改善没有转化为行动增益。Codex 是共同 session harness；不作 provider effect 或 leaderboard。见 `WORK_II_W262_CODEX_C2_FULL_REPLICATION_EXPERIMENT_NOTE.md` 及 W2-62 两份机器/中文报告。 |
| W2-63 | DONE / TERMINAL DEEPSEEK B3 FAILURE-AWARE REPLICATION | 在与 GPT-5.6-sol B3 相同的五个 partition worlds、三臂、每臂/世界两 session 科学表面上，DeepSeek high 完成冻结的 `30/30` scheduled slots：`17 completed + 13 participant-schema failures`；失败不得删除或替换。DeepSeek/GPT failure-aware joint family+1.75 recovery 为 `0/30` 与 `5/30`，Top-1 为 `0/30` 与 `2/30`，mean regret 为 `0.9579/0.7594`，post MAE 为 `0.0928/0.0320`；两模型在 eligible action opportunity 上 gain≥0.02 均为 0。结果支持“结构恢复与行动收益分离”，但 differential schema failure 阻止模型优越性解释。见 `WORK_II_W263_DEEPSEEK_B3_FULL_REPLICATION_EXPERIMENT_NOTE.md` 与 `reports/work-ii-w2-63-b3-failure-aware-cross-model-v0.1.json`。 |
| W2-64 | DONE / TERMINAL PROVIDER-FREE PUBLICATION REANALYSIS | 不新增 provider、truth 或物理执行。四条件 primary estimand 固定为 all-scheduled failure-aware strategy estimand，donor-eligible 降为敏感性。DeepSeek W2-50 的 45 个冻结 cell 均取最后一条可用 executable law；三条无 terminal action ranking 的失败 cell 保留 earlier law。law-implied Top-1 `0/45`、participant failure-aware Top-1 `11/45`、有效 participant ranking 中 follow-law `12/42`，平均 law-implied/participant regret `0.438/0.344`。B2 另纳入 45 条 sanitized public summaries、三配置全臂 expression coding 与 participant-visible identifiability audit；精确 alias、constant baseline 和 failed positive control 将 B2 限定为 underidentifying expression diagnostic，B3 承担结构识别。该 decision-aligned 分解仅来自 DeepSeek cohort，law quality 与 law use 均未随机化，不作双模型或因果 law-to-action claim。见 `WORK_II_W264_PUBLICATION_REANALYSIS_EXPERIMENT_NOTE.md` 与 `reports/work-ii-w2-64-publication-reanalysis-v0.1.json`。 |
...
W2-51/52 的“完成”采用终态定义：固定分母、真实失败、未启动分母与 stop rule 均已完整处置并进入论文边界，
不要求把科学拒绝改写成通过。两项不再阻断 ICLR 稿件、图表或其他独立实验；原 225-session cohort 仍未获授权。
...
- W2-55 reviewer controls：
  `workstreams/flagship_tasks/WORK_II_REVIEWER_CONTROL_ANALYSES_EXPERIMENT_NOTE.md`、
...
- W2-60 DeepSeek low-reasoning B2 robustness：
  `workstreams/flagship_tasks/WORK_II_DEEPSEEK_LOW_REASONING_MATCHED_EVIDENCE_EXPERIMENT_NOTE.md`、
  `workstreams/flagship_tasks/reports/work-ii-as-study-b2-deepseek-v4-flash-low-results-v0.1.json`、
...
- W2-62 GPT-5.6-sol C2 full-cohort（经 Codex harness）与双模型 current-composite：
...
- W2-56 replicated B3 DeepSeek canary、GPT-5.6-sol medium provider-free preparation 与 formal closeout：
  ...
  `workstreams/flagship_tasks/reports/work-ii-as-study-b3-replicated-canary-closeout-v0.1.json`、
  `workstreams/flagship_tasks/reports/WORK_II_AS_STUDY_B3_REPLICATED_CANARY_CLOSEOUT_ZH.md`、
  `workstreams/flagship_tasks/reports/work-ii-as-study-b3-gpt56-sol-medium-preparation-v0.1.json`、
  `workstreams/flagship_tasks/reports/WORK_II_AS_STUDY_B3_GPT56_SOL_MEDIUM_PREPARATION_ZH.md`、
  `workstreams/flagship_tasks/reports/work-ii-as-study-b3-gpt56-sol-medium-formal-closeout-v0.1.json`、
  `workstreams/flagship_tasks/reports/WORK_II_AS_STUDY_B3_GPT56_SOL_MEDIUM_FORMAL_CLOSEOUT_ZH.md`
