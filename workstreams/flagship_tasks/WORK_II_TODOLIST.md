...
本轮继续执行M1独立世界正式复核（用户已授权继续推进）；M0/M1开发实验及六张图重构已完成。
真实路径42/42执行与重放、12/12 provider sessions和16/16条件均完成；后续矩阵按实际结果及成本组织。
正式证据使用另行稳定、冻结的执行面；开发结果明确标注，不自动升级为投稿证据。
...
当前论文报告描述性证据与受限信息策略比较；表示/决策器开发canary已完成，正式复核与新条件迁移尚未完成。
不能把未来方法或预期改善写进现有 Results。
...
| W2-67 | DONE (development) | M1 runner及canary：2 task-worlds，12/12 sessions、16/16 slots；失败感知读出、简单基线和成本汇总；正式矩阵尚未运行 |
| W2-68 | DEFERRED | M2：同预算取证对照；仅当论文保留acquisition因果主张时进入必做集合 |
| W2-69 | DEFERRED | M3：fresh-context新条件迁移；方法泛化的优先扩展，独立于旧C2 selective gate |
| W2-70 | DEFERRED | M4：独立模型/参考数据复核；加强外部效度，不作为近期全面数字孪生任务 |
| W2-71 | DONE | 六张图重构、配对分母与缺失值呈现修正；长稿17页，ICLR正文9页/总19页，PDF视觉及28项定向检查通过 |
| W2-72 | DOING | M1独立世界复核：10 worlds、40来源状态、120会话机会、160条件、200物理执行及重放；固定次序平衡和主对比，完成运行器验收后冻结一次并执行；见[实验说明](WORK_II_M1_REPLICATION_EXPERIMENT_NOTE.md) |
...
2026-09-05收束记录：当前TODO、结果索引、claim map和投稿清单去除重复任务队列；
标题/摘要/引言/讨论及概念图统一为experimental knowledge与decision quality，补充既有
decision-focused文献边界。31项定向集成检查通过；最终Work II图文修改后16项受影响检查再次通过，
Ruff和diff检查通过，37个本地Markdown链接有效。匿名稿9页正文/19页总长，长稿18页，均渲染检查。
既有机器报告、数值source data和current evidence bindings未改；没有provider调用或新科学执行。
控制处置：保留公开/私有边界、资源账本、exact replay及固定实验规则（K0/K1）；
release hash/完整审计留到最终freeze（K2）；重复执行队列和状态投影合并到上述唯一入口（K3）。
