...
| W2-58 | CANARY AUTHORIZED / DEEPSEEK FIRST | W2-57 的唯一失败字段为 runner 已知、科学计分不消费的冗余 stage status；因此建立独立后继协议，保持 worlds、arms、replicates、truth、阈值、shared action index 与全部科学字段不变，只由 runner 派生 pre/post stage 并忽略 participant status 文本。两个 provider manifests 已在 provider-free contract 与 source-manifest digest 校验后复用同一 truth，qualification/public truth/roster/public-packet hashes 一致，30-cell 分母各自独立，static checks 均 ready。严格双模型 formal 汇总器及 complete/incomplete/hash-drift 正负测试已经就绪，只有双方均 `30/30`、0 failures 且 cell/world/arm/replicate 与共享 hashes 全部匹配时才输出描述性差值；runner-level 零 provider 测试也已证明输出 schema 不再包含 status，且即使模拟保留错误 status 文本也不会影响完成与计分；分析 CLI 的文件级测试进一步证明双方 canary 与 formal 完整后才会写出 JSON/Markdown closeout。当前相关 focused tests `19/19` 通过。用户已于 2026-08-31 授权 DeepSeek 后 GPT 的 3+3 canary；当前 `canary_execution_authorized=true`、`formal_execution_authorized=false`。DeepSeek 必须先 `3/3` 合格，GPT 才可启动；formal 仍需单独授权。见 `WORK_II_AS_STUDY_B3_RUNNER_DERIVED_STATUS_CROSS_MODEL_EXPERIMENT_NOTE.md`。 |
