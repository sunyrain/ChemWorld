...
当前目标是收束现有论文，并设计能检验知识决策价值的新实验。当前数据块已终态。
本轮授权是清理、稿件收束与实验规划，不是新的 provider campaign、正式证据生产或投稿授权。
...
当前论文报告描述性证据与受限信息策略比较；表示/决策器干预和新条件迁移尚未完成。
...
| W2-66 | TODO | M0：精简参与者提交表面，统一participant/truth/blind世界构造与决策读出；新数据前写短note |
| W2-67 | TODO | M1：固定证据下表示×决策器干预；完成开发设计、成本校准，再按授权冻结新cohort |
