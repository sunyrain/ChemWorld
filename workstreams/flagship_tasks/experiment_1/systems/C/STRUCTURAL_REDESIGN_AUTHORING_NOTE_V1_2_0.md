...
- 16 parent/child executions per World plus a preregistered constant-capacity scalar-null
  fit/held-out block; 84 executions per World and 252 total; each execution requires
  tolerance-zero replay;
...
9. fit the best constant-capacity parent-law multiplier over the continuous bounded interval
   `[1, 6]` with a deterministic 12-iteration golden-section optimizer. Endpoints and both initial
   interior points are included, giving 16 fit-only multiplier evaluations. Selection uses only
   `s0-t0-d0`, `s0-t1-d1`, `s1-t0-d1`, `s1-t1-d0` and noise-normalized squared residuals for
   purity, yield, CSD quality and fines. On the disjoint four held-out cells, at least two cells
   must retain a residual of at least `2.0` declared final-assay standard deviations.
...
Values `2`, `4` and `6` correspond to `0.04`, `0.08` and
`0.12 mol impurity / mol target` before the supersaturation factor and document the scientific
rationale for the bounded authoring range. They are not a separate executable sensitivity block
and are not in the frozen denominator. Any later sensitivity report requires a new preregistered
note and denominator. The surface half-saturation concentration is frozen at `0.010 mol/L`.
