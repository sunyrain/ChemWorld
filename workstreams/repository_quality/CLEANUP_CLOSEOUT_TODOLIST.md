...
### CD-38：退役未闭合的 Mechanism RC30 审计链 — DONE

RC30 原计划在 Work II runtime/formal-gate 修复后再次重建 Gate A current-source 资格，但只生成 protocol、plan、
participant candidate、relation graph、sample-size、semantics 与 design audit 七个零 provider 投影。它自己声明
必需的 preregistration、release qualification、online attainability、A2/A3 structural receipts 与 public
decision 均不存在；没有 provider 数据、formal result 或 terminal failure。七个文件也没有 current registry、
runner、报告路径、byte hash 或 canonical hash consumer；current evidence DAG 始终明确指向完整 rc29 链。

Work II 当前 development-first 规则已明确：功能持续变化期间，旧 Gate-A/source 证书标为 historical/stale，
只在最终 release freeze 重建一次，不在每次修复后追逐新 rc。现删除 RC30 七个未闭合投影，保留原 experiment
note 并追加 closeout，说明精确内容可从 Git 恢复且不得继续补跑成另一套 current authority。RC15-RC29 的真实
...
### CD-40：退役误导性的 tracked-tree dirty 兼容接口 — DONE

共享 provenance 模块一度同时公开 `git_worktree_dirty` 与 `git_tracked_tree_dirty`。后者只是前者的逐参数转发，
实际同样读取 `git status --untracked-files=all` 并把未跟踪文件算作 dirty；除自身导出和同一测试文件外，没有
runner、release gate、实验记录或外部模块消费者。这个名字既没有提供兼容价值，又容易让新控制误以为它只检查
tracked tree，继而在别名之上重复添加 untracked/clean gate。

现删除该别名、公开导出和别名专属断言，把原有 Git 临时仓库行为测试统一到唯一明确接口
`git_worktree_dirty`。保留全工作树 dirty 检测、精确路径/前缀排除、source commit、canonical/file hashes 与
...
### CD-42：将全包 coverage 从默认 gate 改为显式诊断 — DONE

pytest 原先通过 `pyproject.toml` 对每一次测试调用自动追加 `--cov=chemworld --cov-report=term-missing`。仓库没有 CI
workflow、`cov-fail-under`、coverage XML/HTML 消费者或任何按覆盖率作出的发布判定；因此即使只跑 5 个
provenance helper 测试，也会扫描 71k 行 package、输出数百行全包缺失表。开发 smoke 已不得不额外传
`--no-cov` 来抵消仓库默认值，这表明控制方向已经倒置。

现删除 pytest 全局 coverage addopts，让普通及聚焦测试只执行被选择的行为；保留 `pytest-cov` 开发依赖，并在
开发指南中提供显式的相关面 coverage 命令。相同 5-test 基线由带默认 coverage 的 9.74 秒、巨量输出降为
