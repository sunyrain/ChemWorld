...
- **DOING / Claim**：Codex `/root` — **B02** — 实现执行前兼容性检查与缺失依赖、冲突状态所有者、单位、参数、资源和生命周期诊断。
- **TODO**：兼容语法、覆盖生成、未见组合、模块参考验证和新增使用案例。
...
| B02 | DOING | 实现兼容性检查 | 合法组合可编译；缺失依赖、冲突模块、单位不匹配和无效参数在执行前被清楚拒绝 |
