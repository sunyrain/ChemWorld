...
- **DOING / Claim**：Codex `/root` — **B03** — 实现覆盖引导的组合生成，包括离散 covering array、连续空间填充样本和关键有序操作交互覆盖。
- **TODO**：覆盖生成、未见组合、模块参考验证和新增使用案例。
...
| B03 | DOING | 实现覆盖引导的组合生成 | 离散轴采用 covering array，连续轴采用空间填充采样，操作流程覆盖关键有序交互；生成规模由覆盖目标决定而非声称穷举 |
