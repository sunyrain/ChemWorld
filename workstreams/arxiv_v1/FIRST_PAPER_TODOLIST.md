...
- **DOING / Claim**：Codex `/root` — **C00** — 冻结 C01--C08 与 D01--D04 的单份组合资格实验说明，明确问题、独立单位、覆盖设计、测量、通过/失败规则和预期输出。
- **TODO**：未见组合、模块参考验证和论文新增使用案例。
...
| ID | 状态 | 工作 | 主要读出 |
| --- | --- | --- | --- |
| C00 | DOING | 冻结组合资格实验说明 | 在任何新数据生成前固定问题、测试单位、覆盖、测量、pass/failure 规则和机器可读输出 |
...
| U06 | TODO | 参考案例库 | 连续流、电化学、蒸馏、分配、结晶等更多样例放附录/公开文档，不逐个写成性能实验 |
...
| U02 | 在极小操作与样品预算下，哪些测量足以表征一个有界平衡状态？使用 B04 的单一过程模块示例 `phase + observation`。 | 只公开 `measure` 与 `terminate`，仪器为 pH meter、UV--Vis 和 final assay；无反应或热操作表面。 | 冻结为 3 次操作、0.001 L 样品上限、3 次仪器使用和 1 次终检；参考轨迹为 pH 测量、显式终止、final assay，UV--Vis 保留为未选替代仪器。 |
| U03 | 无效前置条件或资源约束触发后，事务是否保持原子性，并允许从已提交状态继续恢复？使用 B04 的 `reaction + thermal + phase + separation + observation` 多阶段世界。 | 公开反应、分相、洗涤、干燥、浓缩、转移、过程测量、终止和终检；失败返回结构化原因，不自动改写动作。 | 20 次操作、0.001 L 样品、3600 s、2 次仪器使用和 1 次终检；参考轨迹先提交一次故意过早的 `separate_phase` 失败探针，验证无幽灵状态后执行合法反应--分离--终检恢复路径。 |
