...
- **DONE / 平台预算修复**：正式 U05 审计确认旧组合资格说明中的 `time_s` 是手工 pattern cap，未从必要阶段、明确重复次数和 quench/transfer 余量推导；现已新增按 pattern 推导的 `process_time_policy` 与提交前硬预检，资源拒绝动作会在进入物理 runtime 前 fail closed，仍计 attempt 但不提交物理状态。C01--C08 / D01--D04 已从首个 case 完整复跑并通过：64/64 参考单元、1,786/1,786 recipes、192/192 negative probes、52/52 generated cases，零失败；受影响的 8 个确定性案例亦完成 89/89 动作复跑与 exact replay。旧结果保留在 Git 历史中，不作为修复后 current binding。
