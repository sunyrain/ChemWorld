...
PARENT_PLAN_PATH = ROOT / "configs/methods/rl_v0.4/ppo_v0411_preflight_plan.json"
PLAN_PATH = ROOT / "configs/methods/rl_v0.4/ppo_v0412_preflight_plan.json"
NEGATIVE_REPORT_PATH = (
    ROOT / "workstreams/benchmark_v1/reports/rl-ppo-v0411-preflight-v0.4.json"
)
...
def test_crystallization_domain_retry_is_frozen_and_namespaced() -> None:
    ...
    assert plan["schema_version"] == preflight.CRYSTALLIZATION_DOMAIN_PLAN_VERSION
    assert preflight.scientific_contract_sha256(
        parent
    ) == preflight.scientific_contract_sha256(plan)
    ...
    assert plan["adapter_reattestation"][
        "conditional_hybrid_action_schema_version"
    ].endswith("0.4")
    assert "crystallization-domain-v0412" in plan["writer_gate_path"]
    assert evidence["ppo_v0411_negative_report"]["full_matrix_allowed"] is False
    ...
    assert full_plan["current_contract_preflight"]["required_status"] == (
        "ppo_v0412_preflight_passed_full_matrix_allowed"
    )


def test_crystallization_domain_retry_rejects_tampered_negative_report(
    ...
    def tampered_load(path: Path, label: str) -> dict[str, Any]:
        ...
        if path.resolve() == NEGATIVE_REPORT_PATH.resolve():
            ...
            payload["status"] = "ppo_v0411_preflight_passed_full_matrix_allowed"
        return payload
...
def test_crystallization_domain_retry_uses_v0412_gate_status() -> None:
    plan = _load(PLAN_PATH)
    jobs = []
    ...
    assert assessment["status"] == "ppo_v0412_preflight_passed_full_matrix_allowed"
