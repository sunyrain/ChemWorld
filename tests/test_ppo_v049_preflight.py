...
PLAN_PATH = ROOT / "configs/methods/rl_v0.4/ppo_v049_preflight_plan.json"
...
def _condition(*, behavior: int, endpoint: float | None) -> dict[str, Any]:
    return {
        "exact_replay": True,
...
def test_post_affordance_ppo_plan_is_preregistered_and_namespaced() -> None:
    ...
    assert plan["execution"]["report"].endswith("rl-ppo-v049-preflight-v0.4.json")
    assert plan["source_binding"]["require_stable_head_until_report"] is True
    assert plan["evidence_boundary"]["v048_negative_result_remains_immutable"] is True


def test_post_affordance_ppo_gate_uses_v049_status_without_relaxation() -> None:
    plan = _load(PLAN_PATH)

    assessment = preflight.assess_gate(plan, _jobs(plan))

    assert assessment["passed"] is True
    assert assessment["status"] == "ppo_v049_preflight_passed_full_matrix_allowed"
    assert plan["gate"]["minimum_tasks_with_learning_signal"] == 2
    assert plan["gate"]["runtime_domain_failure_count_required"] == 0


def test_post_affordance_ppo_report_rejects_source_or_tree_drift(
    ...
    with pytest.raises(preflight.PPOPreflightError, match="commit changed"):
    ...
    with pytest.raises(preflight.PPOPreflightError, match="tree changed"):
        preflight.source_state_before_report(tmp_path, start)
