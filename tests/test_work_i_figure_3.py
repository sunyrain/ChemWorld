...
def test_manifest_preserves_the_result_independent_pending_slots() -> None:
    manifest = _manifest()
    assert manifest["manifest_sha256"] == manifest_sha256(manifest)
    assert manifest["status"] == "frozen_structure_pending_latent_results"
    assert manifest["pending_result_panels"] == ["C", "D"]
...
def test_manifest_binds_sources_and_publication_outputs() -> None:
    ...
    assert manifest["owner_task"] == "W1-P04"
