...
| DEEP-D4A | whilesunny | Claimed | `thermo` UNIQUAC activity-coefficient implementation notes, standard UNIQUAC combinatorial/residual equations, existing ChemWorld activity-model API | UNIQUAC structural parameters, binary interaction parameters, combinatorial and residual activity-coefficient terms, validation reports, and failure diagnostics | caller-supplied local r/q/tau parameters only; explicit units, component ids, temperature, and provenance metadata; no copied parameter tables or reference code | ideal-like tau=1 sanity case, asymmetric binary interaction response, ternary composition closure, missing structural parameter failure, invalid composition failure, model-card validation | nonpositive structural parameter, missing tau pair, invalid component composition, unsupported parameter shape, overflow/underflow in residual term | `src/chemworld/physchem/equilibrium.py`, `src/chemworld/physchem/equilibrium_cards.py`, `src/chemworld/physchem/__init__.py`, `tests/test_phase_equilibrium.py`, docs | LLE/VLE and extraction tasks can use UNIQUAC activity coefficients through the same gamma-phi and phase-equilibrium reporting path | claim pushed before implementation; code, tests, model card/docs, reference-reading note, validation examples, public API exports, and local gates | claim commit |
