...
| DEEP-D10A | whilesunny | Claimed | Gymnasium/Minari metadata conventions, Safety-Gymnasium cost-signal separation, existing ChemWorld task maturity and baseline-report code | model-maturity gate audit for trajectory, task, baseline, and leaderboard outputs; per-task consistency checks for proxy/lite/professional metadata | no external data; maturity metadata must be generated from task/model cards and recorded in JSON-friendly artifacts | task-info maturity exposure, trajectory maturity keys, same-task mixed-maturity rejection, baseline report maturity summary, proxy-allowed task manifest | missing maturity keys, silent same-task proxy/professional mixing, proxy task without allowed tag, stale docs/TODO status | `src/chemworld/tasks.py`, `src/chemworld/eval/baseline_report.py`, `tests/test_maturity.py`, `tests/test_baselines.py`, docs | benchmark results cannot silently mix proxy and professional kernels; reports expose maturity metadata and proxy permission | audit existing implementation, synchronize TODO/docs, run gates | claim commit |
