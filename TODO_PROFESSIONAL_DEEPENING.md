...
| DEEP-D2A | whilesunny | Claimed | `chemicals` vapor-pressure families, `thermo` vapor-pressure method governance, existing ChemWorld curated property correlations | Antoine and Wagner/DIPPR vapor-pressure families, analytic temperature derivatives, validity-domain enforcement, sublimation extension where data exists | curated coefficients with provenance and declared units; no copied reference code or bulk data vendoring | Antoine water/ethanol analytical checks, DIPPR derivative finite-difference check, invalid temperature hard-fail/warning policy, monotonic pressure sanity cases | unsupported component, missing coefficient set, invalid temperature unit/range, nonpositive pressure, derivative outside validity range | `src/chemworld/physchem/properties.py`, `src/chemworld/physchem/curated_properties.py`, `tests/test_physchem_properties.py`, docs | flash/distillation/safety tasks can request vapor pressure with explicit method, derivative, range status, and model-card provenance | claim pushed before implementation; code, tests, model card/docs, reference-reading note, validation examples | pending push |
