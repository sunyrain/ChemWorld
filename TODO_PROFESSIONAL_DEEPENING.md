...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| DEEP-D10B | whilesunny | Done | Minari dataset-card/replay metadata patterns, Gymnasium metadata conventions, current ChemWorld JSONL/Parquet export and replay verifier | schema-versioned dataset cards, protocol-hash manifests, replay verification summaries, JSONL/Parquet export metadata, privacy/anonymization declarations | ChemWorld trajectory records only; no external dataset vendoring; dataset card records replay validity and protocol hash sets | JSONL export preserves records, Parquet flattening carries protocol hashes, dataset card includes replay summary and hash sets, invalid trajectory fails export | missing trajectory schema field, replay verification failure, mixed protocol hashes, invalid privacy status, Parquet dependency unavailable | `src/chemworld/data/datasets.py`, `tests/test_world_architecture.py`, docs | offline agent-training and paper-artifact datasets can prove task/profile/mechanism/scoring/observation protocol identity and replay status | code, tests, docs, reference-reading note, replay summary, protocol hash manifest, and local gates are complete | this commit |
...
| DEEP-D5C | whilesunny | Done | Cantera YAML mechanism schema, RMG reaction/species schema notes, existing ChemWorld mechanism YAML and runtime compiler | schema-versioned mechanism manifest, species/reaction/rate-law/observable/score/initial-state contracts, deterministic mechanism hash and validation report | local ChemWorld JSON schema and manifest only; no copied mechanism parser or third-party schema code | all bundled mechanisms validate, unknown schema/version fails, missing observable/score binding fails, unsafe/eval-style rate law rejected, manifest JSON round-trip | duplicate species, unbalanced reaction, unsupported rate law equation id, unknown species reference, missing target/impurity/initial species, mutable hash drift | `src/chemworld/runtime/mechanisms.py`, `src/chemworld/schemas/`, `configs/mechanisms/`, tests/docs | benchmark scenarios can expose a machine-readable mechanism manifest for replay, submission audit, and future observation/score/instrument binding | code, tests, docs, manifest export, reference-reading note, and local gates are complete | this commit |
| DEEP-D3B | whilesunny | Done | `chemicals` saturation solvers, `thermo` vapor-pressure method governance, CoolProp saturation API notes, existing ChemWorld vapor-pressure and cubic-EOS code | pure-fluid saturation pressure/temperature solve, boiling-point inverse solve, critical-region warnings, triple/critical bound checks, residual diagnostics, reference-backend comparison hooks | local equations and curated coefficients only; explicit units, validity ranges, root status, and provenance; no copied reference code or bulk data | vapor-pressure forward/inverse closure, normal boiling point solve, critical-region warning, out-of-range hard-fail, missing coefficient failure, model-card validation | nonpositive temperature/pressure, temperature outside validity, no bracketing pressure root, pressure above critical, missing critical constants, derivative singularity | `src/chemworld/physchem/saturation.py`, `src/chemworld/physchem/vapor_pressure.py`, `src/chemworld/physchem/__init__.py`, `tests/test_physchem_properties.py`, docs | flash/distillation/scenario generators can request auditable pure-fluid saturation reports instead of unlabeled vapor-pressure proxies | code, tests, model card/docs, reference-reading note, validation examples, public API exports, and local gates are complete | this commit |
| DEEP-D3C | whilesunny | Done | `chemicals` Rachford-Rice and flash notes, `thermo` ideal bubble/dew/flash workflows, existing ChemWorld equilibrium and saturation APIs | mixture bubble-temperature solve, dew-temperature solve, K-value initialization from pure saturation reports, Rachford-Rice diagnostic report, nonconvergence/invalid-bracket diagnostics | local equations and curated correlations only; explicit pressure, composition, K-value, residual, bracket, and provenance metadata; no copied flash solver code or data tables | ideal binary bubble/dew temperature ordering, bubble/dew pressure closure, Rachford-Rice vapor fraction closure, invalid composition failure, no-bracket failure, model-card validation | nonpositive pressure, invalid composition, missing vapor pressure, bad bracket, K-values all on one side, nonconvergence, negative phase composition | `src/chemworld/physchem/equilibrium.py`, `src/chemworld/physchem/saturation.py`, `src/chemworld/physchem/__init__.py`, `tests/test_phase_equilibrium.py`, docs | distillation, evaporation, and flash tasks can expose auditable mixture phase-boundary and TP flash diagnostics instead of unlabeled VLE splits | code, tests, model card/docs, reference-reading note, validation examples, public API exports, and local gates are complete | this commit |
| DEEP-D3D | whilesunny | Done | `thermo` gamma-phi flash/property-package workflows, cubic-EOS fugacity coefficient notes, azeotrope-detection examples in phase-equilibrium references, existing ChemWorld EOS/activity/bubble-dew APIs | gamma-phi K-value report, vapor-fugacity coefficient contract, binary azeotrope residual scan, relative-volatility crossing diagnostics, public/private scenario parameter notes | local equations and curated correlations only; explicit gamma, phi, Psat, K-value, composition grid, crossing bracket, residual, and provenance metadata; no copied flash solver code or parameter tables | ideal phi=1 regression, nonideal phi shifts K-values, binary azeotrope residual crossing, no-crossing diagnostic, invalid phi/composition/bracket failures, model-card validation | nonpositive pressure, invalid vapor fugacity coefficient, missing vapor pressure, bad composition grid, no relative-volatility crossing, nonbinary azeotrope request, nonconvergence | `src/chemworld/physchem/equilibrium.py`, `src/chemworld/physchem/equilibrium_cards.py`, `src/chemworld/physchem/__init__.py`, `tests/test_phase_equilibrium.py`, docs | distillation, flash, and generalization tasks can expose gamma-phi K-value provenance and azeotrope-risk diagnostics before rigorous EOS flash arrives | code, tests, model card/docs, reference-reading note, validation examples, public API exports, and local gates are complete | this commit |
| DEEP-D4A | whilesunny | Done | `thermo` UNIQUAC activity-coefficient implementation notes, standard UNIQUAC combinatorial/residual equations, existing ChemWorld activity-model API | UNIQUAC structural parameters, binary interaction parameters, combinatorial and residual activity-coefficient terms, validation reports, and failure diagnostics | caller-supplied local r/q/tau parameters only; explicit units, component ids, temperature, and provenance metadata; no copied parameter tables or reference code | documented thermo binary gamma example, asymmetric binary interaction response, ternary composition closure, missing structural parameter failure, invalid composition failure, model-card validation | nonpositive structural parameter, missing tau pair, invalid component composition, unsupported parameter shape, overflow/underflow in residual term | `src/chemworld/physchem/equilibrium.py`, `src/chemworld/physchem/equilibrium_cards.py`, `src/chemworld/physchem/__init__.py`, `tests/test_phase_equilibrium.py`, `tests/reference/test_optional_reference_backends.py`, docs | LLE/VLE and extraction tasks can use UNIQUAC activity coefficients through the same gamma-phi and phase-equilibrium reporting path | code, tests, model card/docs, reference-reading note, validation examples, public API exports, and local gates are complete | this commit |
...
| DEEP-D10A | whilesunny | Done | Gymnasium/Minari metadata conventions, Safety-Gymnasium cost-signal separation, existing ChemWorld task maturity and baseline-report code | model-maturity gate audit for trajectory, task, baseline, and leaderboard outputs; per-task consistency checks for proxy/lite/professional metadata | no external data; maturity metadata is generated from task/model cards and recorded in JSON-friendly artifacts | task-info maturity exposure, trajectory maturity keys, same-task mixed-maturity rejection, baseline report maturity summary, proxy-allowed task manifest | missing maturity keys, silent same-task proxy/professional mixing, proxy task without allowed tag, stale docs/TODO status | `src/chemworld/tasks.py`, `src/chemworld/eval/baseline_report.py`, `tests/test_maturity.py`, `tests/test_baselines.py`, docs | benchmark results cannot silently mix proxy and professional kernels; reports expose maturity metadata and proxy permission | existing code, tests, docs, baseline-report gate, task manifest, and local gates are complete | this commit |
| DEEP-D11A | whilesunny | Done | Cantera electrochemical examples, Newman-style electrolyte resistance equations, existing ChemWorld Nernst/Butler-Volmer electrochemistry core | solution resistance from conductivity/path/area, uncompensated ohmic drop, measured vs interfacial potential, energy-loss accounting, and control-voltage failure flags | local equations only; explicit geometry/conductivity units; no copied electrochemistry reference code | zero-current no-drop case, Ohm-law voltage-drop case, resistance scaling with conductivity/path/area, run_electrolysis energy-loss report, invalid geometry/conductivity failures | nonpositive conductivity/area/path, finite-current requested beyond voltage window, sign mistakes in interfacial potential, silent negative energy loss | `src/chemworld/physchem/electrochemistry.py`, `src/chemworld/physchem/electrochemistry_cards.py`, `src/chemworld/runtime/electrochemical_services.py`, tests/docs | electrochemical tasks can expose measured cell voltage, interfacial potential, ohmic loss, and electrolyte-resistance metadata rather than only ideal electrode potential | code, tests, model card/docs, reference-reading note, validation examples, task-facing runtime metadata, and local gates are complete | this commit |
...
- [x] `DEEP-D11A` ohmic-drop and electrolyte-resistance slice:
