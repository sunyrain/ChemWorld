...
| PRO-RUNTIME-A mechanism-owned initial-state generator and legacy fallback removal | whilesunny | Implementing | Cantera mechanism/species initialization patterns, ChemWorld mechanism YAML cards | `configs/mechanisms/`, `configs/scenarios/`, `src/chemworld/world/scenario.py`, `src/chemworld/world/state_factory.py`, `src/chemworld/runtime/species.py`, tests/docs | initial `WorldState` species ledgers now come from compiled mechanisms; next replace the remaining lite seven-slot reaction backend with compiled stoichiometry/rate-law integration so fallback constants can be deleted | this commit |
| PRO-RUNTIME-A compiled reaction integrator slice | whilesunny | Review | Cantera reaction-network integration patterns, ChemWorld mechanism YAML cards | `src/chemworld/runtime/mechanisms.py`, `src/chemworld/world/reaction_kernel.py`, `src/chemworld/runtime/reaction_thermal_services.py`, tests/docs | compiled heat/wait integrator is implemented; next remove the old seven-slot fallback function and move any remaining reference tests to explicit fixtures | this commit |
| PRO-RUNTIME-A runtime compiled mechanism required | whilesunny | Review | Cantera mechanism/species initialization patterns, ChemWorld runtime v2 architecture | `src/chemworld/runtime/reaction_thermal_services.py`, `src/chemworld/world/reaction_kernel.py`, `src/chemworld/runtime/species.py`, tests/docs | runtime reaction advancement now requires compiled mechanisms; next delete or quarantine the old seven-slot `integrate_reaction_ode` reference fixture and remove remaining generic legacy species defaults | this commit |
| PRO-RUNTIME-A seven-slot reference quarantine | whilesunny | Review | Cantera mechanism-driven runtime architecture and ChemWorld golden runtime tests | `src/chemworld/world/reaction_kernel.py`, `src/chemworld/world/reaction_reference.py`, tests/docs | fixed A/P/B/D/E ODE is quarantined as an explicit reference fixture; runtime/domain services are guarded against importing it | this commit |
| PRO-RUNTIME-A compiled mechanism required across runtime services | whilesunny | Review | Cantera mechanism-driven runtime architecture and ChemWorld role mapping tests | `src/chemworld/runtime/`, tests/docs | env/runtime/eval now have no `LEGACY_*` species defaults and runtime/domain/observation services require compiled mechanisms; next split mechanism role validation into schema-level checks | this commit |
| PRO-RUNTIME-A mechanism role validation at compile time | whilesunny | Review | Cantera mechanism schema validation patterns and ChemWorld mechanism role tests | `src/chemworld/runtime/mechanisms.py`, mechanism tests/docs | compile-time validation now separates base mechanism-library checks from stricter runtime scenario role contracts for target, impurity, initial species, and observable mappings | this commit |
| PRO-RUNTIME-B transaction record consistency | whilesunny | Claimed | Safety-Gymnasium cost-channel patterns, transactional runtime architecture | `src/chemworld/runtime/kernels.py`, `src/chemworld/runtime/transactions.py`, tests/docs | make operation records, state patches, rollback events, and returned state describe the same committed or rolled-back transaction result | pending |
