...
| PRO-RUNTIME-A mechanism-owned initial-state generator and legacy fallback removal | whilesunny | Done | Cantera mechanism/species initialization patterns, ChemWorld mechanism YAML cards | `configs/mechanisms/`, `configs/scenarios/`, `src/chemworld/world/scenario.py`, `src/chemworld/world/state_factory.py`, `src/chemworld/runtime/species.py`, tests/docs | scenario initial states now come from compiled mechanism species, roles, and initial-amount policies; remaining fixed-slot logic is quarantined as explicit reference code | this commit |
| PRO-RUNTIME-A compiled reaction integrator slice | whilesunny | Done | Cantera reaction-network integration patterns, ChemWorld mechanism YAML cards | `src/chemworld/runtime/mechanisms.py`, `src/chemworld/world/reaction_kernel.py`, `src/chemworld/runtime/reaction_thermal_services.py`, tests/docs | heat/wait and flow reaction advancement now use compiled stoichiometry, rate laws, and reaction enthalpies | this commit |
| PRO-RUNTIME-A runtime compiled mechanism required | whilesunny | Done | Cantera mechanism/species initialization patterns, ChemWorld runtime v2 architecture | `src/chemworld/runtime/reaction_thermal_services.py`, `src/chemworld/world/reaction_kernel.py`, `src/chemworld/runtime/species.py`, tests/docs | reaction/thermal runtime now requires compiled mechanisms and guards against missing compiled-mechanism calls | this commit |
| PRO-RUNTIME-A seven-slot reference quarantine | whilesunny | Done | Cantera mechanism-driven runtime architecture and ChemWorld golden runtime tests | `src/chemworld/world/reaction_kernel.py`, `src/chemworld/world/reaction_reference.py`, tests/docs | fixed A/P/B/D/E ODE is quarantined as an explicit reference fixture; runtime/domain services are guarded against importing it | this commit |
...
| PRO-RUNTIME-F precondition rollback transaction | whilesunny | Done | Safety-Gymnasium cost-channel patterns, transactional runtime architecture | `src/chemworld/runtime/`, `src/chemworld/envs/`, tests/docs | operation precondition failures now enter Runtime v2 rollback events and process-only penalty patches; schema/task/payload failures remain env-level validation failures | this commit |
