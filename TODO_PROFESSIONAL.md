...
| PRO-RUNTIME-A mechanism-owned initial-state generator and legacy fallback removal | whilesunny | Planned | Cantera mechanism/species initialization patterns, ChemWorld mechanism YAML cards | `configs/mechanisms/`, `configs/scenarios/`, `src/chemworld/world/scenario.py`, `src/chemworld/world/state_factory.py`, `src/chemworld/runtime/species.py`, tests/docs | compile initial species amounts directly from mechanism/scenario specs, then remove `A/P/B/D/E` fallback constants from runtime species resolution | not started |
