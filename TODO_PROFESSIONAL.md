...
| PRO-RUNTIME-A compiled reaction integrator slice | whilesunny | Claimed | Cantera reaction-network integration patterns, ChemWorld mechanism YAML cards | `src/chemworld/runtime/mechanisms.py`, `src/chemworld/world/reaction_kernel.py`, `src/chemworld/runtime/reaction_thermal_services.py`, tests/docs | route heat/wait through compiled stoichiometric matrices and rate-law evaluators, keeping the old seven-slot ODE only as removable reference until tests pass | this claim |
