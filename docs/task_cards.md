...
| `partition-discovery` | 学习未知溶剂/萃取体系的分配规律 | `public-test` | 48 | `0,1,2` | `campaign` | 0.60 | `proxy` |
...
| --- | --- |
| `reaction-to-assay` | `4e1217427b9f0d4d73330c7d21072233806196fa8af76ead4178fb5e0400c34a` |
| `reaction-to-purification` | `7a2115ada2b879a763aae2ab6924256fa170dd987b7494bcb91d8e15758aac34` |
| `partition-discovery` | `4f5b07b32d223b1be5ea36f7e9c6ff94a93f07f0496ba12fe27d03e5a1b7d20d` |
...
- settle 时间和 entrainment 会影响 mass balance；
- agent 应通过少量测量形成局部分配模型，而不是只追求单次 final assay score。

## 发布规则

- pre-release benchmark 只引用上面三个任务。
- 任务合同版本：`chemworld-task-contract-0.2`。
- 每个 task card 必须暴露 `task_contract_hash`、`kernel_maturity`、`physics_maturity`
  和 `proxy_allowed`。
- `reaction-to-purification` 和 `partition-discovery` 仍含 proxy-maturity downstream
  components；报告、排行榜和论文 artifact 必须显示这一点。
...
目前 baseline 表仍在 `P0-BENCH-03` 中待生成。
