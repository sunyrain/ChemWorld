...
- `TaskObservationContract` 将 `success_metrics`、评分合同、可用仪器和机制 observable mapping 编译成可序列化的观测合同，避免所有任务继承全局仪器的完整输出字段。
...
任务边界和运行时 profile 也进入合同层。`TaskSpec.contract_hash` 锁定 task id、scenario、budget、allowed operations、allowed instruments、success metrics 和 safety limit；`TaskRuntimeProfile.profile_hash` 锁定当前任务实际需要的 kernels、domain services、capabilities 和 instruments。
...
当前 observation kernel 会读取 task-specific observation contract：反应任务只暴露 yield、selectivity、conversion、byproduct、degradation 等反应诊断；纯化、分配、结晶、蒸馏、连续流和电化学任务只额外暴露各自评分需要的观测键。这样 HPLC、GC、UV-vis 和 final assay 仍共享同一套仪器模型，但不会把无关任务指标作为全局字段泄露给 agent。raw signal 在环境运行路径中也只使用 task-visible 的公开角色聚合量，例如 `A_public`、`P_public`、`B_public`、`D_public`，不会把 full hidden species ledger 或机制内部物种名直接交给 agent。
...
同时，verifier 会比对 `task_contract_hash`、`runtime_profile_hash`、`mechanism_hash`、`scoring_contract_hash` 与 `observation_contract_hash`；只要任务边界、运行时 profile、机制、评分合同或观测合同发生变化，重放就会明确失败，而不是生成一个看似可比但语义不同的结果。
...
- 机制驱动广度：score、processed observation 和环境 raw signal 已经有 task-level contract；下一步需要把仪器校准、谱峰归属、检测限和方法条件进一步绑定到 mechanism/species/instrument card。
