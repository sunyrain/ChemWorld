...
ChemWorld-Bench 是一个面向 AI4Science、化工教育和闭环实验决策研究的虚拟物理化学交互环境。它不是一个真实反应预测软件，也不是许多互不相关小游戏的集合；它的核心目标是在同一套物理化学世界规律下，为学生、LLM agent、贝叶斯优化器、强化学习 agent 和人机协作系统提供可交互、可复现、可提交、可验证、可评测的实验任务。
...
这里的 `ChemWorld` 是一个统一世界环境，`task_id` 是同一世界中的任务切片。不同任务共享 ontology、physical constitution、operation language、instrument contracts、runtime transaction、trajectory schema 和评测协议。
...
├── schemas         # action, recipe, trajectory, manifest, task, scenario, mechanism schemas
...
- `eval`、`data` 和 schemas 让轨迹可复现、可回放、可审计、可比较。
...
因此 `reaction-optimization-standard`、`reaction-to-purification`、`reaction-to-distillation`、`flow-reaction-optimization`、`electrochemical-conversion` 等任务都是同一个物理化学世界的不同切片。
...
`manifest` 内含 validation report、rate-law families、score spec 和 initial amount policy。`task_info()` 会暴露 `mechanism_manifest`，trajectory 和 verifier 会记录 `mechanism_id` 与 `mechanism_hash`。如果 mechanism 文件发生变化，replay verifier 应该失败，而不是悄悄比较不同隐藏世界产生的轨迹。

运行时已经不再依赖固定五物种网络。旧的 `A/P/B/D/E` 只应出现在具体 mechanism fixture 或显式 reference code 中，不应成为通用 runtime、observation、scoring 或 phase bookkeeping 的隐含前提。
...
当前仍保留一层 scalar state adapter，用于快速迭代期间维持旧轨迹、notebook 和部分测试稳定。长期目标是让 phase/vessel/equipment ledger 成为唯一结构化状态源。
...
当前已经具备 HPLC/GC/UV-vis/final assay，以及 IR functional-group band slice。final assay 是主要 leaderboard scoring source。
...
当前任务覆盖反应条件优化、安全约束优化、机制解释、投料到 final assay、反应到萃取纯化、分配规律发现、purity/yield/cost tradeoff、public/private generalization、low-budget characterization、flow、distillation、crystallization 和 electrochemistry 等方向。
...
- 物理模型深度：distillation、crystallization、flow、electrochemistry、phase equilibrium、safety 仍需要更严格的机理模型和验证矩阵。
