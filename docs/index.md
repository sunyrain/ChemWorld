...
截至 2026-07-29，替代静态 S0 v1.0 已在电化学和反应–结晶上各完成 10 个独立世界、每世界
20 轮探索、完整经典基线与精确回放。Codex 盲测均值分别为 **0.7150** 和 **0.5355**；电化学
相对最佳 information-matched 基线的描述性差为 +0.0991，结晶低于 LHS 的 0.5708。比较没有
预注册 superiority 阈值，因此不支持广义 SOTA 论断。

S0 材料信息实验现已完成 opaque、正确匿名属性和固定定向错误先验三臂，每任务十个配对世界。
电化学 nominal 为 **0.7874**、opaque 为 0.7150（配对差 +0.0724；双任务 familywise
97.5% 区间 [+0.0074,+0.1546]）；结晶为 **0.5615** vs 0.5355（+0.0260；
[−0.0130,+0.0630]），仍不确定。错误先验在两个任务中都改变了早期动作，但两个任务均未通过
预注册的整体恢复联合规则。60 个单元全部精确 replay。

15 个任务的完整实验适配器通过 415 个端到端案例，62 个声明指标均绑定到可执行端点；只有两个任务
具有正式多世界模型比较。RC28 Gate A 仍是冻结旧源码上的历史环境证书，当前源码绑定 stale，
`benchmark_ready=false`。旧双任务结果已撤回，不属于当前证据。
...
[探索 Showcase Worlds 与完整任务目录 →](worlds.md)

[查看哪些任务已经进入确认性基准协议 →](flagship_experiments.md)
...
LLM 交互、记忆、资源账本和谱图消融协议已经具备；替代固定世界 S0 的正式模型矩阵尚未执行，
机制变化模型矩阵也仍未完成。现有单 seed RL
