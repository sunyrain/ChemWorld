...
最新未触碰 cohort 覆盖四个任务、三种方法、20 个配对 seeds，每次 40 个完整实验。修复后的
Safe-GP 在四项任务上均通过 safety 与 cost 非劣规则，且相对 random 的目标效应均为正；但连续流
平均效应为 0.018752，未达到预注册 SESOI 0.020000。四任务联合规则因此仍为失败，不发布方法胜负
或 benchmark 主张。详情见[科学状态与证据](benchmark_release.md)。
