...
- 15 个共享 `chemworld-physical-chemistry-v0.4` 世界律的任务切片；
...
或实验机器人控制器。v0.4 已移除正式 runtime 的 proxy/fallback 路由，但所有 provider 仍只在
模型卡声明的窄域内成立；`reference_validated` 和 `professional_candidate` 不代表工业验证。
当前新合同是 backend candidate，尚无可发布的方法排名。

在引用结果前，请阅读[适用范围与限制](limitations.md)和
