...
正确匿名材料属性的 S0 v1.1 配对扩展已按负责人要求先完成每任务 seed 0–4。电化学 nominal
为 **0.7873**、opaque 为 0.6939（配对差 +0.0935）；结晶 nominal 为 **0.5507**、opaque
为 0.5173（配对差 +0.0334）。两个 95% 配对区间都跨 0，所以这是积极但不确定的五世界中期
结果，不是冻结的十世界确认性结论；seed 5–9 尚未运行。

15 个任务的完整实验适配器通过 415 个端到端案例，62 个声明指标均绑定到可执行端点；只有两个任务
具有正式多世界模型比较。RC28 Gate A 仍是冻结旧源码上的历史环境证书，当前源码绑定 stale，
`benchmark_ready=false`。旧 0.3902/0.4829 双任务结果已撤回，不属于当前证据。
