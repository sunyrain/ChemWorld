...
2026-07-27 的旧静态 S0 双任务 Participant 结果已撤回，不再进入当前模型排名或论文数字。替代
电化学协议已绑定显式材料家族与平衡效率评分；替代反应–结晶协议已绑定独立催化剂/溶剂材料家族，
并通过五世界 × 16 材料配对资格审计。两者仍是 `formal_result=false` 的开发证据。

固定世界 S0 仍是当前研究主线，但新的正式论断必须来自冻结后的经典基线与 Codex subscription
多世界运行。隐藏世界变化与机制替换已经延期，需要在现实漂移模型和独立研究问题明确后重新设计。
