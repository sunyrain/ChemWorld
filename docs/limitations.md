...
- world-family 与语义不变性控制已可执行，但 severity 网格、Train/Dev/Bench 分配和确认性方法
  实验尚未冻结，不能把不同 seed 的稳定性扩大解释为机制泛化；
- 电化学与平衡任务的主指标尚未显示稳定的自适应收益；正式经典矩阵存在非零连续风险，但安全
  阈值从未被触发，不能辨识安全约束方法。
- 历史正式经典方法只从完整实验的 final score 更新，不使用中间谱图或进行实验内闭环调整；
  live LLM、RL 和谱图消融尚无确认性 artifact。
...
在正式发布前，项目仍需冻结 provisional core 与 world-family severity、完成公开进程边界、扩展
exploit、score/replay、资源对齐的方法矩阵和第三方干净安装复现。达到这些门禁后，发布包才会
