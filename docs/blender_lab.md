...
无需打开Blender即可使用原场景的三维导出：

```powershell
uv run --no-sync python -m apps.blender_lab --api-only --port 8878
```

打开 **http://127.0.0.1:8878/explore**。可以旋转、缩放、点选36个设备，切换全景/反应/表征/配液/俯视，
回看固定8动作的公开实验轨迹。已测结果与尚未测量值分别呈现；拖动时间轴不会重新执行化学动作。
“实时观测”接收配置到同一端口的BlenderObserver；“运行搬运”驱动独立场景物流。

“证据图谱”展示F1–F4与M1/M3，提供模型筛选、逐世界配对效应、注册区间、精确分母和SVG/JSON导出。
全部前端依赖本地提供。研究数值通过current读取；三维场景、Core开发示例与正式论文结果标明各自用途。
该探索器用于项目/作者展示，匿名投稿仍使用独立补充包。
