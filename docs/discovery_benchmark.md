...
本页记录 2026-07-10 的历史工程 pilot。它探索经典主动学习是否利用后续实验机会，以及 DeepSeek
是否能读取公开谱图并据此进行多轮修正。相关原始 Task Lab 目录在后续清理中没有保留，因此下列
数字不再满足当前正式 evidence gate，不能用于方法排名、模型比较或论文统计结论。

!!! important "结论边界"
    这里的“发现”是对 ChemWorld 隐藏参数面的 benchmark-local 发现，不是新反应、新催化剂或
    现实实验结论。任务中的催化剂和反应物仍是匿名对象。页面保留失败模式与交互设计经验；正式
    结果必须重新运行并保留 trajectory、evaluation result、模型/prompt 和资源账本。
