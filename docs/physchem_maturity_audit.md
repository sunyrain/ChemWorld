...
| Phase equilibrium | Lite with reference-validated Wilson/NRTL/UNIQUAC gamma, mixture VLE, and gamma-phi diagnostic slices | ideal VLE, Wilson/NRTL/UNIQUAC gamma checks against `thermo`, curated bubble/dew temperature reports, gamma-phi K-value reports, binary relative-volatility crossing diagnostics, Rachford-Rice diagnostic closure tests, LLE split tests | phase stability, rigorous gamma-phi/EOS flash, azeotrope curve tracing, and nonideal VLE/LLE task cases |
...
- DEEP-D3C is implemented for mixture VLE diagnostics:
  `bubble_temperature_report()` and `dew_temperature_report()` use curated
  vapor-pressure correlations through pure saturation reports, activity
  coefficients, bracketed pressure-residual solves, and K-value closure checks.
  `rachford_rice_diagnostic_report()` reports vapor fraction, endpoint
  objectives, single/two-phase status, residuals, and warnings. Gamma-phi EOS
  flash, azeotrope detection, and rigorous phase-stability analysis remain
  future slices.
- DEEP-D3D is implemented for gamma-phi K-value and binary azeotrope-risk
  diagnostics: `gamma_phi_k_value_report()` records gamma, vapor phi,
  liquid-reference phi, Poynting, Psat, K-values, and relative volatilities;
  `binary_azeotrope_diagnostic_report()` scans `ln(alpha)` over binary
  composition and reports crossing brackets, estimated composition,
  no-crossing status, and warnings. Rigorous gamma-phi flash, azeotrope curve
  tracing, and tangent-plane stability remain future slices.
- DEEP-D4A is implemented for UNIQUAC activity coefficients:
  `uniquac_activity_report()` records structural r/q parameters, volume and
  surface fractions, tau matrices, combinatorial/residual log terms, and
  activity coefficients. Optional `thermo.uniquac` reference checks and local
  fail-fast parameter tests are in place. Parameter databases, tangent-plane
  stability, and rigorous LLE/flash coupling remain future slices.
