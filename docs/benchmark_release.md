...
的校准在线轨分开。RC23 校准轨的真值支持是 `never/6/8/10`，并冻结 `τ` 为“已完成的旧世界实验数”；
Agent 只看到总 horizon，不看到前缀、时间支持或证书状态。Reference certificate 现在要求通用结构
覆盖、held-out 旧世界预测充分性和参考新鲜度同时通过；reference failure 只从条件归因分母排除，
仍计入端到端失败。Development、A2、A3 与 private confirmation cohort 互不重叠。统一旗舰语义审计
18/18、物理设计审计 81/81 已通过；该重构不改变 RC21 历史结果，也不能在新的 untouched cohort
执行前产生 v0.3 正式结论。
