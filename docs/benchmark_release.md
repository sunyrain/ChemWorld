...
!!! info "静态 S0 v1.0 正式结果"
    2026-07-27 的旧双任务 Participant 结果已撤回，不能用于当前模型排名或论文数字。电化学替代
    与反应–结晶替代实验现已完成 10 个独立世界、每世界 20 轮、完整经典基线和精确 replay。
    Codex 均值分别为 0.7150 和 0.5355。电化学相对最佳 information-matched 基线的描述性差为
    +0.0991；结晶低于 LHS 的 0.5708。比较没有预注册 superiority 阈值或多重比较方案，因此不能
    升级为广义 SOTA、provider 因果效应或样本外泛化论断。

!!! info "S0 v1.2 匿名材料信息三臂确认性结果"
    opaque、正确匿名属性 nominal 与固定定向错误属性 misindexed 已完成每任务十个配对世界。
    电化学 nominal 为 0.7874 vs opaque 0.7150（+0.0724，familywise 97.5% 区间
    [+0.0074,+0.1546]），确认正信息价值；结晶为 0.5615 vs 0.5355（+0.0260，
    [−0.0130,+0.0630]），仍不确定。错误先验在两个任务中都改变了早期动作，但两个任务均未通过
    操纵、差分行为纠偏与性能恢复的联合规则。60 个单元全部精确 replay，共 2,280 次物理实验、
    1,260 次成功订阅调用、5 次自动重试和 0 方法失败。

!!! info "15 任务设计证据"
    15 个完整实验适配器共通过 415 个端到端案例，覆盖中点、每个坐标的低/高干预和全部离散类别；
    62 个声明指标均有可执行评估端点。三个纯化任务现覆盖完整后处理，蒸发与蒸馏控制也已拆分。
    该证据只支持设计可执行性；其余 13 个任务的正式多世界比较仍待执行。
...
!!! info "一句话状态"
    World Engine、任务合同、资源账本、回放和评价控制可以运行；双任务静态 S0 已有正式描述性结果，
    但扩展任务比较、机制适应、私有泛化与独立复现仍未共同完成。当前不支持 SOTA 或现实迁移主张。
...
| **Formal descriptive** | 冻结执行并审计，但没有确认性 superiority 规则 | 双任务十世界静态 S0 v1.0 |
| **Confirmatory** | 冻结方法在未见 cohort 上按冻结统计规则检验 | RC28 Gate A（只绑定其历史冻结源码） |
