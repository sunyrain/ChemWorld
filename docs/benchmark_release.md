...
solvent 目标不可达问题；新的 `electrolyte_profile` 设计审计和电化学 Gate A cells 通过，但
reaction-to-crystallization 的逐 family 置信下界仍低于冻结阈值，因此 Gate A 整体失败。
...
| 当前预算下所有机制 family 均可识别 | 不支持；Gate A 整体失败 |
