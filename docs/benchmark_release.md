...
## Finding 3：机制变化已经可达，但可达不等于适应

**Evidence level：Control-validated**

反应任务使用速率律与网络拓扑族；分配、电化学和平衡使用各自 Provider 消费的构成律族。在 5 个
世界 × 5 个代表性 recipe 下，9/9 任务—模式组合满足：
...
**支持的结论**：ChemWorld 能执行、区分和回放预注册隐藏规律变化。

**不支持的结论**：尚不能声称 Agent 会识别这些变化、迁移到未见 family，或适应现实机理。
...
| 六任务拥有行为可辨识的机理/构成律控制 | 支持 Control-validated 层 |
