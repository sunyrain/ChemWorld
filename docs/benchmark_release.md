...
!!! info "S0 v1.1 正确匿名材料属性：五世界中期"
    nominal-vs-opaque 配对实验已完成每任务 seed 0–4。电化学为 0.7873 vs 0.6939
    （+0.0935，95% 区间 [−0.0062,+0.2232]）；结晶为 0.5507 vs 0.5173
    （+0.0334，区间 [−0.0307,+0.0929]）。10 个单元全部精确 replay，合计 380 次物理实验、
    210 次成功订阅调用、5 次自动重试和 0 方法失败。两个区间都跨 0；该结果只完成冻结方案的一半，
    `formal_result=false`，不能宣称已证明正信息价值。seed 5–9 仍待执行。
