...
- `adaptive`（默认）：每一步发送最新公开实验报告、可用谱图目录和合法动作；模型明确请求某个
  `spectrum_request_id` 后，下一次调用才获得该谱图，不会自动收到全部历史曲线；
...
Agent Observatory 会明确区分 `MEASUREMENT OUTPUT` 与 `MODEL RETRIEVAL · DECISION N`：前者是
刚完成实验的公开输出，后者是模型主动请求后实际序列化并发送的谱图载荷。公开分析区同步展示
证据、谱图解读、假设、实验意图、历史对照、依据和不确定性，不采集或展示隐藏逐字思维链。谱图历史
选择器会保留本次页面运行中的全部测量输出和模型输入，可按任务、实验与步骤前后切换；导出结果也包含
该历史记录。

同一 experiment 内的状态是累积的：加料进入当前釜，连续加热从当前组成继续积分，中间测量会扣除
样品和成本但不会重置反应器。只有成功的 `final_assay` 才构成独立实验边界；campaign 会在记录分数后恢复新反应器，
single-experiment 则结束 episode。因此不得把同一釜内的追加加热或物料操作叙述成独立配方对照。
...
- 动作验证和可执行参数范围；
- 每种动作的累计、覆盖、取样、分流或转移语义，以及实际 `Δt`、`ΔV` 和样品消耗；
- 已用预算、剩余预算和实验轮次；
- 公开指标、安全风险、成本和最佳得分；
- 投料、热过程、取样、相分离、结晶、蒸馏、流动和电化学数字实验动画，以及学习曲线；
- 谱图坐标、公开峰指认和仪器摘要；
- 从本地 session 操作记录重建的历史谱图选择器；
- 可下载的完整实验记录。

非法动作在执行前被拒绝，不消耗预算，也不改变环境状态。
交互表单直接呈现核心 Gym 的有效范围和离散选项，不再维护网站专属的二次校验；多次晶种投加采用
累计物料与设备账本，同一实验中的溶剂、萃取剂和催化剂类别由核心状态合同锁定；萃取剂代码会实际
参与分配和风险计算，不再只是轨迹元数据；
动画只使用公开动作和事务增量，不展示未观测的真实组成。
