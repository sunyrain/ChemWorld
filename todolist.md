...
- [ ] **`benchmark-v05-rl-adapters` — PPO/SAC 正式适配、训练与 checkpoint 封存**
  ...
  - [ ] 修复并版本化 v0.4.10 新暴露的公共前置条件：`cool_crystallize` 只在扣除 seed 后仍有正目标进料时可用；`measure` 必须满足所选仪器采样体积，终止后只允许 final assay。以 v0.4.11 新 namespace/plan 重跑 PPO；SAC 只在共享门禁修复后运行，且任何阈值、split、seed、超参数和 SESOI 不得修改。
