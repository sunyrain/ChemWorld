...
| 当前 PPO | `ab62d3f` 干净 source 的 v0.4.11 preflight 已完成 4/4 任务、102,400 训练步和 16 次 exact Dev replay；4/4 任务有学习信号，行为完整实验 49→407，训练后四任务 runtime/observation domain failure 均为 0。但 crystallization step-0 仍有 10 次 runtime-domain failure，预注册 `all_tasks_operational` 因而失败 | `full_matrix_allowed=false`，20-run 矩阵未启动、0 checkpoint 入选、`ppo_method_ready=false`。只读精确复现把 10 次失败闭合为 3 次初始温度越出 solubility model 有效域、7 次正但低于最小有效粒子数的 seed population；先以 v0.4.12 公开这两项前置条件，不改 split、seed、预算、超参数、SESOI 或门槛 |
| 当前 SAC | v0.4.11 的同合同计划与 writer gate 已预注册并 source-bound ready，但 PPO v0.4.11 的共享 crystallization operational 门禁失败后未启动 SAC；因此没有 v0.4.11 SAC 训练步、checkpoint 或性能结果 | `full_matrix_allowed=false`、`sac_method_ready=false`；旧 199 万步与 v0.4.8–v0.4.10 负结果继续隔离。只有下一版 PPO 共享 operational 门禁通过后才允许消费 SAC preflight 预算，禁止在已知公共域缺口上重复训练 |
...
| RL post-affordance 执行 | v0.4.10 与 v0.4.11 均在干净且已推送的 source 上完成 4/4 PPO 任务、各 102,400 步与 16 次 exact Dev replay，单 spawned batched worker 未再触发 `WinError 1455`。v0.4.11 把 crystallization step-0/trained runtime failure 从 22/3 降到 10/0，并把 distillation step-0 从 28 降到 0；4/4 仍有学习信号、行为完整实验 49→407，但 `all_tasks_operational` 继续 fail closed | v0.4.10/v0.4.11 负报告保持不可变，SAC/full matrix 均未在已知共享缺口上启动。下一版只公开 crystallization 的 solubility 起始温度域与最小有效 seed population 两项前置条件，版本化 conditional action 合同并预注册 v0.4.12；不得放宽任何科学门槛或重抽 seed |
...
- [ ] **`benchmark-v05-rl-adapters` — PPO/SAC 正式适配、训练与 checkpoint 封存**
  ...
  - [ ] 修复并版本化 v0.4.11 剩余的两项公共结晶域：`cool_crystallize` 的起始温度必须落在 250–430 K solubility 有效域内；已有正 seed charge 必须达到固定 runtime 粒径/密度下的最小有效粒子数。预注册 v0.4.12，科学合同与门槛保持不变，并仍先运行 PPO。
  - [ ] 只有各自 preflight 通过后，才执行四任务五 seed Train/Dev、干净加载、确定性重放与 checkpoint index 封存。
  - 当前诊断：旧 PPO 的非 Markov 状态别名已经通过公开阶段位、reward contract 0.3 和任务级 `chemworld-rl-observation-contract-0.1` 修复；旧 checkpoint index 已 fail closed 清零。`ab62d3f` 的 PPO v0.4.11 执行 4 个 task ×（step-0、25,600-step）共 8 个 checkpoint，manifest/sidecar/task observation hash、干净加载、双重复放与 source stability 全部通过；4/4 任务达到学习信号，行为完整实验 49→407，训练后四任务 runtime/observation failure 均为 0。唯一门禁失败是 crystallization step-0 的 10 次 runtime-domain failure；精确 cohort/policy 复现定位为 3 次 solubility reference temperature 越界和 7 次 seed population 小于 `minimum_effective_seed_particles`。SAC 与两个 full matrix 均未启动，P3 仍未完成。
...
`fa546e5` 后的一个 v0.4.10 LLM fresh cell 已完成，v0.4.11 公开谱图审计修复也已离线就绪；当前暂停更多付费调用。operation 288-cell 与 classic 768-cell 的 v0.4.1 正式开发重认证均已通过并绑定稳定干净的 `ef202b7`，相应 P3 项已完成。RL adapter 0.2 和 Windows 单 spawned worker 已连续支撑 PPO v0.4.10/v0.4.11 两轮四任务、各 102,400 步而无页文件失败；v0.4.11 已消除训练后与蒸馏 step-0 的全部 runtime-domain failure，但结晶 step-0 仍有 3 次起始温度域和 7 次最小有效 seed population 缺口，故继续 fail closed。下一步只修复并版本化这两项 public precondition，以 v0.4.12 原门槛先复核 PPO；SAC 不在已知缺口上重复消耗。之后确定 LLM 是否退出或完成 candidate/Dev，执行 portable-release reattestation 与 method freeze。P4 必须严格按“reference evidence → base matrix → reproduction”串行；P5 的三个实验可在 P4 通过后并行；P6 最后串行审计和发布。
