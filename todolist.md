...
| 真实 LLM | prompt 0.7 的 v0.4.9 fresh cell 已在 `9ceb87c` 严格运行 1/6：16 operations、17 provider requests、97,878 input tokens、30,390 output tokens、1 retry、0.063826 USD、568.69 s；完整实验、资源账本和精确回放通过。谱图来源门禁通过，但轨迹暴露冷却 schema 与 runtime 不一致；`fa546e5` 已完成系统修复和六任务离线证明 | v0.4.8/v0.4.9 都保持 source-bound 诊断、`benchmark_claim_allowed=false`；不得续跑 v0.4.9 剩余 5 cells。下一次付费调用必须生成 v0.4.10 或等价新 runner/cache/report，绑定新 action schema/source，仍只允许 1 个 fresh cell；该单 cell 通过前不扩矩阵 |
...
当前从 `fa546e5` 新语义边界继续：先由 PPO/SAC 各自重建 source/hash-bound 的小规模 step-0 versus trained preflight，并由核心组只运行一个 v0.4.10 LLM fresh cell；三者都不得直接扩成完整矩阵。通过者再完成 P3 封存，失败者按负结果退出，随后执行 portable-release reattestation 与 method freeze。P4 必须严格按“reference evidence → base matrix → reproduction”串行；P5 的三个实验可在 P4 通过后并行；P6 最后串行审计和发布。
