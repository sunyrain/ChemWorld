...
| 当前 PPO | `099028d` 当前合同 preflight 已按预注册规则完成并以负结果关闭：4 个任务均完成 step-0/25,600-step 配对，3/4 有学习信号，行为完成数 11 → 254，但 runtime-domain failures 11 → 31，结晶任务训练后仍 0 次行为完成；102,400 训练步、8 个 checkpoint 均可干净加载且精确回放 | `full_matrix_allowed=false`，20-run 矩阵未启动、0 checkpoint 入选、`ppo_method_ready=false`。`e626474` 已修复其中一类共享冷却温度可行域错误，但这改变了动作合同；不得沿用旧 checkpoint 或只重抽 seed，必须先重新封存合同并预注册一次新的有界 preflight |
| 当前 SAC | current-contract preflight 已按与 PPO 对称的门禁完成并以负结果关闭：102,400 训练步、8 个 checkpoint、16 次 Dev evaluation 均通过干净加载与精确回放；仅 partition 1/4 任务出现学习信号，行为完整实验 21 → 9，runtime-domain failures 41 → 559，其中 flow 训练后为 501 | `full_matrix_allowed=false`，完整 20-run 矩阵未启动、0 checkpoint 入选、`sac_method_ready=false`；旧 199 万步继续隔离。先使用 `e626474` 后的新动作合同做离线/短探针，若仍无法达到预注册学习与全任务 operational 门禁，则 SAC 作为负结果退出正式方法清单 |
| 真实 LLM | prompt 0.7 的 v0.4.9 fresh cell 已在 `9ceb87c` 严格运行 1/6：16 operations、17 provider requests、97,878 input tokens、30,390 output tokens、1 retry、0.063826 USD、568.69 s；完整实验、资源账本和精确回放通过。谱图来源门禁通过，但轨迹暴露冷却 schema 与 runtime 不一致，已由 `e626474` 修复 | v0.4.8/v0.4.9 都保持 source-bound 诊断、`benchmark_claim_allowed=false`；`e626474` 改变动作语义，因此不得续跑 v0.4.9 剩余 5 cells。下一次付费调用前先生成新版本/新缓存并完成动作可行域离线探针，仍只允许 1 个 fresh cell |
...
- [ ] **`benchmark-v05-rl-adapters` — PPO/SAC 正式适配、训练与 checkpoint 封存**
  ...
  - 两个 preflight 与 LLM 轨迹共同定位了一类共享根因：`cool_crystallize` 的公开 schema 曾始终宣称 250–330 K，验证器又错误排除合法下界 250 K；体系已冷却到 260 K 时，270 K 仍会通过公开验证，之后才在 runtime 以“反向升温”失败。`e626474` 令公开上限随当前温度收窄、250 K 下界可用，并在公开验证层阻止反向冷却动作；19 个接口/原子性测试与 47 个 LLM/RL/操作基线交叉测试通过。这是动作语义修复，所有此前 PPO/SAC checkpoint 与 LLM cell 仍只属于旧 source-bound 诊断，不能直接恢复或续跑。
  - 下一步不是放宽门槛或重复抽 seed，而是在 `e626474` 后先枚举其余 runtime-domain failures 的 operation/affordance/parameter 分布，确认公开 schema 与实际 runtime 的可行域一致；然后用新 plan/hash 分别给 PPO/SAC 各一次有界 step-0 versus trained 门禁。仍失败的方法退出正式方法清单，不拖延 reference/Bench。
...
- [ ] **`benchmark-v05-live-llm-adapters` — 真实 LLM 双角色开发与提示冻结**
  ...
  - v0.4.9 同时暴露的两个非法动作来自共享可行域错误而非模型未知私有物理：公开 schema 排除了自身标明的 250 K 下界，并在当前 260 K 时仍允许用 `cool_crystallize` 指定 270 K。`e626474` 已修复并通过接口与交叉回归，但这也使 v0.4.9 的剩余 5 cells 失去同源可比性。
  - 下一门禁：v0.4.8/v0.4.9 私有缓存和公开报告都保留为不可续跑的 source-bound 诊断。下一次调用必须先把 runner/cache/report 升为新版本，绑定 `e626474` 之后的干净提交，并离线证明所有公开参数界限与 runtime domain 一致；之后仍使用 `--stop-after-new-terminals 1` 只运行一个 fresh cell。只有来源、动作有效性、账本、回放和投影资源均通过，才允许同一新源码继续其余 5 cells。单实验 `candidate_screen` 只检验可执行性、成本和失败模式；跨实验学习必须在后续 2-experiment `live_pilot` 和 4-experiment Dev 阶段单独验证。
