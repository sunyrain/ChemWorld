...
| `benchmark-vnext-agent-interaction-contract` | Agent/环境 | 冻结 campaign→experiment→operation 三层预算；允许实验内读谱、更新假设和追加操作；保留结构化 hypothesis/evidence/uncertainty/decision，不记录私有思维链 | task validity、world-family axes |
| `benchmark-vnext-evaluation-identifiability` | 核心评测 | 评价系统分离 task primary、在线 shaping、risk/cost constraint、regret 与资源效率；逐任务验证信号非退化、方向正确、可由轨迹重算 | task validity、risk-cost signal |
...
1. 并行认领 `benchmark-vnext-task-validity`、`wf-vnext-risk-cost-signal`、
   `wf-vnext-mechanism-families`、`benchmark-vnext-agent-interaction-contract` 和
   `benchmark-vnext-evaluation-identifiability`。
