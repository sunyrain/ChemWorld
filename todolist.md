...
| 机理族 | 结晶前反应、蒸馏前反应和 PFR 三个真实消费 ReactionNetworkSpec 的任务已支持速率律族与拓扑族；hash 与实际任务响应同步改变 | 可达任务控制层通过；其它三任务应使用专属 constitutive-law family |
...
| 评价分层 | vNext 已分离 final-assay objective、task primary、在线 shaping、constraint、resource 与 validity；缺失终点主指标失败关闭 | 控制层通过；安全阈值校准阻塞可辨识性 |
| 方法交互覆盖 | v0.1 正式 5 方法仍只在实验间更新完整 recipe；vNext runner 已提供逐操作公共上下文、谱图递交、能力声明和结构化决策证据，信息流探针通过 | 控制层通过；经典/RL/真实 LLM 重跑未完成 |
...
| `wf-vnext-mechanism-families` | 世界基座团队 | ReactionNetwork 可达三任务的速率律族、反向竞争通道、opaque hash 与响应探针已完成；电化学、分配、平衡应在各自专用 provider 中建设 constitutive-law family | 控制实现完成 |
...
| `benchmark-vnext-evaluation-identifiability` | 核心评测 | 六层评价合同、实验峰值风险和 total/process/measurement 三账已完成；旧方法尚未接收 vNext policy，主指标方向/灵敏度仍待 task validity | 控制实现完成；方法重跑待 method protocol |
...
| `benchmark-vnext-llm-baselines` | LLM agent | 至少两类真实模型，多轮读谱/实验/更新；只保存结构化 hypothesis/evidence/uncertainty/decision summary，不要求私有 chain-of-thought | method protocol、public harness、agent interaction contract |
...
- [x] `benchmark-vnext-evaluation-identifiability`：新增不覆盖 v0.1 的分层 evaluator，终点 objective、
  task primary、在线 shaping、约束、资源与有效性独立重算；控制审计因安全约束零命中继续阻塞科学主张。
- [x] `wf-vnext-mechanism-families`：仅对真实执行 ReactionNetworkSpec 的三个任务启用速率律族与
  拓扑族；派生 mechanism hash、reaction graph、任务响应和 opaque 公共标识均通过机器探针，未对不可达任务伪造机理变化。
...
- [x] `benchmark-vnext-method-protocol`：经典、RL 与真实 LLM 共用的累积资源账本已接入正式 runner；
  ...
  GP-PI/UCB/RF-EI 也须先改为 typed categorical encoding，故正式方法矩阵继续关闭。
...
1. 冻结 provisional core-4 的 task-specific SESOI，并分别建设分配、电化学、平衡任务的 constitutive-law family。
2. 冻结机制、交互、评价、风险—成本和 world-family 新合同，运行 core-4 confirmatory round。
3. 冻结严重度网格与 Train/Dev/Bench 世界分配，再完成 semantic invariance、public harness、
   exploit matrix 和 score/replay。
4. method protocol 控制已冻结；完成 typed classic、RL 和 live LLM adapter 后再并行正式重跑，
   先跑方法后改任务的结果全部作废。
