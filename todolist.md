...
| RL post-affordance 执行 | `9e2339c` 干净 source 的 SAC v0.4.9 已完成 4/4 任务、102,400 训练步与 16 次精确可重放 Dev evaluation，source 起止稳定干净；0/4 学习信号、行为完整实验 13→0、runtime-domain failures 47→428，`full_matrix_allowed=false`。这不是纯 SAC 算法结论：step-0 在 3/4 任务已有 runtime failure，审计确认共享 RL decoder 只 mask 操作类型，却未投影公开 action schema 的动态 choices/bounds/cross-field constraints。PPO v0.4.9 另在 0 训练步、0 Dev outcome 时因并发 PyTorch 子进程触发 `WinError 1455`，两类失败均已保留 | 已升级共享 conditional adapter 0.2：只消费公开 `action_schema`/`validate_action`，动态映射锁定物料、可用 instrument、状态数值边界及冷却/电压约束，且精确 `-1/+1` latent 端点使用 dtype-aware 可表示边界。Windows subprocess 保持 8 个有序 vector slots，但改由单个 spawn worker 承载，实际训练 smoke 与 DummyVecEnv 固定 seed 逐步同值并避免每 slot 重复加载 SciPy/Torch。PPO/SAC v0.4.10 保持原 split/seed/超参数/SESOI/门槛并进入新 namespace，134 项联合回归通过。下一步在干净 source 签发 writer gate，先单独运行一个 PPO task 验证进程与新 adapter，再按冻结计划完成四任务门禁 |
...
- [ ] **`benchmark-v05-rl-adapters` — PPO/SAC 正式适配、训练与 checkpoint 封存**
  ...
  - [ ] 在新干净 source 签发 adapter 0.2 writer gate；所有旧 RL 进程退出后先单独执行 PPO v0.4.10 的一个预注册 task，确认无 `WinError 1455` 且新 checkpoint/action hash 正确，再完成 PPO/SAC 四任务门禁。
...
`fa546e5` 后的一个 v0.4.10 LLM fresh cell 已完成，v0.4.11 公开谱图审计修复也已离线就绪；当前暂停更多付费调用。operation 288-cell 与 classic 768-cell 的 v0.4.1 正式开发重认证均已通过并绑定稳定干净的 `ef202b7`，相应 P3 项已完成。RL v0.4.9 已把两个问题拆开：PPO 为零 outcome 的并发进程基础设施失败，SAC 为完整但受共享动态 payload adapter 缺口污染的负面诊断；两者均不得作为算法排名。下一步先提交并签发 adapter 0.2，在无其他 RL 进程时运行 PPO v0.4.10 单任务，随后按未放宽的四任务门禁完成 PPO/SAC；通过者才执行 20-run Train/Dev，失败者按负结果退出。之后确定 LLM 是否退出或完成 candidate/Dev，执行 portable-release reattestation 与 method freeze。P4 必须严格按“reference evidence → base matrix → reproduction”串行；P5 的三个实验可在 P4 通过后并行；P6 最后串行审计和发布。
