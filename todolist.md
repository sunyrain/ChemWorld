...
| RL post-affordance 执行 | `4c34e8c` 干净 source 的 PPO v0.4.10 已完成 4/4 任务、102,400 训练步与 16 次精确可重放 Dev evaluation；8 个有序 vector slots 由单个 spawned worker 承载，未再触发 `WinError 1455`。4/4 任务均有学习信号，行为完整实验 36→408，但预注册 `all_tasks_operational` 失败：crystallization step-0/trained 分别有 22/3 次 runtime-domain failure，distillation step-0 有 28 次，故 `full_matrix_allowed=false` | v0.4.10 负报告保持不可变。v0.4.11 已实现公共 precondition/affordance 修复：`cool_crystallize` 要求扣除 seed 后仍有正目标进料，`measure` 要求所选仪器样品体积可用且终止后仅允许 final assay；conditional hybrid action 合同升至 0.3。PPO/SAC 新 namespace、同科学合同 plan 与 full-matrix fail-closed plan 已预注册，148 项扩展回归和 26 项新旧 preflight/adapter 合同测试通过。下一门禁是把该干净提交推到 `origin/main`、重签 writer gate，并先运行 PPO v0.4.11；SAC 继续等待 PPO 的共享门禁结果 |
...
- [ ] **`benchmark-v05-rl-adapters` — PPO/SAC 正式适配、训练与 checkpoint 封存**
  ...
  - [ ] 在干净且已推送的 source 上重签 writer gate 并先运行 PPO v0.4.11；SAC 只在 PPO 共享 operational 门禁通过后运行，PPO 失败则固化负结果并禁止继续消耗 SAC/full-matrix 预算。
...
`fa546e5` 后的一个 v0.4.10 LLM fresh cell 已完成，v0.4.11 公开谱图审计修复也已离线就绪；当前暂停更多付费调用。operation 288-cell 与 classic 768-cell 的 v0.4.1 正式开发重认证均已通过并绑定稳定干净的 `ef202b7`，相应 P3 项已完成。RL adapter 0.2 和 Windows 单 spawned worker 已在 `4c34e8c` 的 PPO v0.4.10 四任务、102,400 步中得到实证，4/4 学习信号且无页文件失败；但公共 validator 仍错误放行零目标进料结晶与采样体积不足的 measurement，预注册 operational 门禁因此 fail closed，20-run 矩阵未启动。下一步先封存该负报告，修复并版本化共享 public precondition/affordance，再以 v0.4.11 新计划复核 PPO；SAC 不在已知缺口上重复消耗。之后确定 LLM 是否退出或完成 candidate/Dev，执行 portable-release reattestation 与 method freeze。P4 必须严格按“reference evidence → base matrix → reproduction”串行；P5 的三个实验可在 P4 通过后并行；P6 最后串行审计和发布。
