...
| 方法交互覆盖 | recipe-search / operation-open-loop / operation-closed-loop 已分层；官方 live-LLM adapter 的 assigned/masked 消融只删除谱图字段并保持质量衡算、成本、约束等非谱图证据，支持跨实验记忆、失败计费且不代办收尾；单 seed 100k SAC 通过 Dev 门槛但 checkpoint 非单调 | 控制层通过；本地无 DeepSeek 凭证，真实 LLM 运行、RL 多 seed/全任务与同层正式比较未完成 |
...
| `benchmark-vnext-llm-baselines` | LLM agent | 官方逐操作 adapter、因果隔离的公开谱图/遮蔽配对、跨实验记忆、结构化公开审计、token/费用/重试账本与失败保留已实现并通过 fake-client replay；官方文档确认 Pro/Flash 与 high/max thinking 有效；真实矩阵仍缺本地凭证 | 提供 `DEEPSEEK_API_KEY` 后运行与独立复现 |
