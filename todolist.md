...
| 机理族 | 反应任务的速率律/拓扑族与分配任务的 constitutive-law 族已完成 5 seeds × 5 配方探针校准；7/7 任务-模式组合可区分、非灾难且守恒；轨迹只公开 opaque hash，精确干预上下文由维护者回放 | 可达任务控制层通过；电化学与平衡仍需专属 constitutive-law family，agent 机理识别/迁移未测 |
...
| `wf-vnext-mechanism-families` | 世界基座团队 | ReactionNetwork 三任务与分配 constitutive-law family 已完成多 seed/多配方强度校准、opaque hash 和精确回放绑定；下一步为电化学/平衡 family 与 Train/Dev/Bench 机理迁移实验 | 控制实现完成 |
...
- [x] `wf-vnext-mechanism-families`：ReactionNetwork 三任务与分配 constitutive family 通过
  5 seeds × 5 配方的可区分/非灾难/守恒校准；干预版本与 opaque hash 入轨迹，精确上下文缺失或篡改时回放失败关闭。
...
1. 冻结 provisional core-4 的 task-specific SESOI；分配 family 已完成，继续建设电化学与平衡 constitutive-law family。
