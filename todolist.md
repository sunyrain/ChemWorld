...
| `benchmark-vnext-evaluation-identifiability` | 核心评测 | 六层评价合同与轨迹重算器已完成；当前正式证据约束零命中，信号校准与逐任务方向/灵敏度冻结仍待 task validity、risk-cost signal | 控制实现完成 |
...
1. 并行认领 `benchmark-vnext-task-validity`、`wf-vnext-risk-cost-signal`，并分别建设分配、
   电化学、平衡任务的 constitutive-law family。
