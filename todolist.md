...
当前配置入口统一见 `configs/current.json`。历史协议与负结果继续保留以支持审计，但不再依赖文件名中的
“vnext”或较大的版本号推断当前状态。
...
| 当前 PPO | `89dff507` 干净 source 的 v0.4.12 preflight 已完成 4/4 任务、102,400 训练步和 16 次 exact Dev replay；4/4 任务 operational 且有学习信号，行为完整实验 50→407、trained 主指标观测 414，四任务 step-0/trained runtime/observation domain failure 均为 0，原 crystallization 3+7 缺口已在正式 evaluator 中清零。exact-source 20-run full matrix 已于 2026-07-16 22:55 启动，首个 `ppo-partition-discovery-seed0` attempt 已绑定 source/plan hash | `full_matrix_allowed=true` 只表示当前 PPO full-matrix gate 已解锁，不是方法封存或 Bench 结果；当前等待全部 20 runs、checkpoint evaluation/index、资源账本与确定性重放完成，完成前 `ppo_method_ready=false` |
...
| RL post-affordance 执行 | v0.4.12 PPO/SAC 均在干净且已推送的 source 上完成 4/4 tasks、各 102,400 步与 16 次 exact Dev replay；PPO 4/4、SAC 3/4 learning signal，行为完整实验分别 50→407 与 57→155。公开 crystallization 的 solubility 起始温度域与最小有效 seed population 后，两算法 step-0/trained 的四任务 runtime/observation failure 全部为 0。PPO full matrix 已在独立 exact-source clean worktree 启动 | v0.4.10/v0.4.11 负报告保持不可变，v0.4.12 两个 `full_matrix_allowed=true` 仅解锁各自后续执行。PPO 当前运行，SAC 为避免资源账本和算力争用而顺序排队；两者完成前不得把 preflight 或中间产物冒充方法结果 |
...
- [ ] **`benchmark-v05-rl-adapters` — PPO/SAC 正式适配、训练与 checkpoint 封存**
  ...
  - [x] 固化并推送 SAC v0.4.12 正报告；最终证据绑定 `c4497586`，门禁为 `sac_v0412_preflight_passed_full_matrix_allowed`。
  - [ ] 在各自 exact source-bound preflight report 与 clean tree 上依次执行 PPO/SAC 四任务五 seed full matrix，封存 checkpoint index、资源账本和确定性重放。PPO 已于 2026-07-16 22:55 从 `89dff507` 启动首个 `ppo-partition-discovery-seed0` attempt；SAC 顺序排队，未完成前 `ppo_method_ready=false`、`sac_method_ready=false`。
  - [ ] 只有各自 preflight 通过后，才执行四任务五 seed Train/Dev、干净加载、确定性重放与 checkpoint index 封存。
  - 当前诊断：旧 PPO 的非 Markov 状态别名已经通过公开阶段位、reward contract 0.3 和任务级 `chemworld-rl-observation-contract-0.1` 修复；旧 checkpoint index 已 fail closed 清零。`89dff507` PPO 与 `c4497586` SAC 的 v0.4.12 preflight 均执行 4 个 task ×（step-0、25,600-step）共 8 个 checkpoint，manifest/sidecar/task observation hash、干净加载、双重复放与 source stability 全部通过；PPO 4/4、SAC 3/4 任务达到学习信号，行为完整实验分别 50→407、57→155，两算法四任务 step-0/trained runtime/observation failure 均为 0。v0.4.11 crystallization step-0 的 3 次 solubility reference temperature 越界和 7 次低有效 seed population 已由公开前置条件在 runtime dispatch 前关闭。PPO full matrix 已在 `89dff507` exact-source worktree 运行，SAC 尚未启动，P3 仍未完成。
