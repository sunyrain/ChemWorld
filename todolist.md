...
| 当前 SAC | v0.4.12 同合同计划与 `89dff507` source-bound writer gate 已就绪；PPO v0.4.12 共享 operational 门禁现已通过，因此可以按预注册顺序消费 SAC preflight 预算 | `sac_method_ready=false`，SAC v0.4.12 尚有 0 正式训练步；旧 199 万步与 v0.4.8–v0.4.10 负结果继续隔离。只有 SAC 自身 preflight 通过后才允许 SAC full matrix |
...
| RL post-affordance 执行 | v0.4.10–v0.4.12 均在干净且已推送的 source 上完成 4/4 PPO 任务、各 102,400 步与 16 次 exact Dev replay，单 spawned batched worker 未再触发 `WinError 1455`。v0.4.12 公开 crystallization 的 solubility 起始温度域与最小有效 seed population 后，step-0/trained 的四任务 runtime/observation failure 全部为 0；4/4 operational 且有学习信号，行为完整实验 50→407 | v0.4.10/v0.4.11 负报告保持不可变，v0.4.12 `full_matrix_allowed=true` 仅解锁后续执行。先完成同合同 SAC preflight；PPO/SAC full matrix 仍各自受当前报告与正式训练入口约束，不得把 preflight 冒充方法结果 |
...
- [ ] **`benchmark-v05-rl-adapters` — PPO/SAC 正式适配、训练与 checkpoint 封存**
  ...
  - [ ] 固化并推送 PPO v0.4.12 正报告后运行同合同 SAC step-0 versus trained preflight；只有 SAC 自身通过才解锁 SAC full matrix。
  - [ ] 只有各自 preflight 通过后，才执行四任务五 seed Train/Dev、干净加载、确定性重放与 checkpoint index 封存。
  - 当前诊断：旧 PPO 的非 Markov 状态别名已经通过公开阶段位、reward contract 0.3 和任务级 `chemworld-rl-observation-contract-0.1` 修复；旧 checkpoint index 已 fail closed 清零。`89dff507` 的 PPO v0.4.12 执行 4 个 task ×（step-0、25,600-step）共 8 个 checkpoint，manifest/sidecar/task observation hash、干净加载、双重复放与 source stability 全部通过；4/4 任务 operational 且达到学习信号，行为完整实验 50→407，四任务 step-0/trained runtime/observation failure 均为 0。v0.4.11 crystallization step-0 的 3 次 solubility reference temperature 越界和 7 次低有效 seed population 已由公开前置条件在 runtime dispatch 前关闭。SAC 与两个 full matrix 尚未启动，P3 仍未完成。
...
`fa546e5` 后的一个 v0.4.10 LLM fresh cell 已完成，v0.4.11 公开谱图审计修复也已离线就绪；当前暂停更多付费调用。operation 288-cell 与 classic 768-cell 的 v0.4.1 正式开发重认证均已通过并绑定稳定干净的 `ef202b7`，相应 P3 项已完成。RL adapter 0.2 和 Windows 单 spawned worker 已连续支撑 PPO v0.4.10–v0.4.12 三轮四任务、各 102,400 步而无页文件失败；`89dff507` 的 v0.4.12 已在原冻结门槛下关闭结晶 3+7 公共域缺口并通过 PPO gate。下一步固化正报告并运行同合同 SAC preflight；两个 full matrix 仍须分别由当前报告解锁。之后确定 LLM 是否退出或完成 candidate/Dev，执行 portable-release reattestation 与 method freeze。P4 必须严格按“reference evidence → base matrix → reproduction”串行；P5 的三个实验可在 P4 通过后并行；P6 最后串行审计和发布。
