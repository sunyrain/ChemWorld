...
try {
    ...
    $casePresentation.Slides.Item(1).Export((Join-Path $caseTemp 'figure2-full.png'),'PNG',4320,5700)
...
Push-Location $caseRoot
try {
    & uv run --no-sync python -c "from PIL import Image; from pathlib import Path; import tempfile; t=Path(tempfile.gettempdir())/'chemworld-case-typography'; im=Image.open(t/'figure2-full.png').crop((0,0,4320,5475)); im.save('paper/figures/final-ppt/figure02-research-paths-typography.png'); im.thumbnail((1440,1900)); im.save(t/'preview.png')"
