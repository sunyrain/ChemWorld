...
def inject_archive_metadata(text: str, metadata: Mapping[str, Any]) -> str:
    ...
    availability_pattern = re.compile(
        ...
        r"|"
        r"(?P<current_public>"
        r"A frozen(?:, versioned)? (?:public|software-and-evidence) release.*?"
        r"(?:private seeds and local\s+run caches are not included\."
        r"|The article and its submission source bundle are maintained separately "
        r"from the software release\.)"
        r")"
        r"|"
