...
更新：2026-09-06。本文件负责作者侧的完整论证，不管理实验执行。
...
> 在有界化学模拟任务中，提交规律可能遗漏Agent已输出的预测信息，却也能在特定设置下独立帮助新决策；预测、知识与行动需要分别验证。
> 四层测量组织这些不同读出，M1/M3提供观察到的成功设置及其边界，没有识别一套统一的成功机制。

“四层断裂”保留为研究起点与问题框架。当前结论并非四个转换都已被证明失败：F1是预设纠错
标准未获支持，F2仍有公开信息充分性缺口，F3测到提交规律的保真损失，F4测到规律与实际选择
...
| F2 数值预测→结构识别 | 最小接口120会话117有效，预测平均改善；未知/错误先验0/80结构恢复，17次成功均为正确先验保持 | 5复用worlds，工具主差+1.67个百分点、区间跨零；公开信息独立可识别性未证明 |
...
B3承担F2条件性读出的主证据，不承担信息充分时的能力缺陷论证；B2欠识别、schema容量细节和evaluator错位主要放补充材料。
...
因此F2继续表述为条件性的数值/结构输出分离。上述差异不是信噪比或未见预测基线，
也不证明真实模拟器不可识别。具体来源由current的w2_79_public_information解析。

Spotlight最优先的科学增量是可预测的条件性结果：先使公开观测映射明确，再固定世界、
证据、预算与读出，单独操纵一个表示或信息因素，在未用于提出假设的world上复核。
第三模型用于最强分离的独立验证；不复制全矩阵。该新干预尚未执行，不能写成已发现的修复机制。
...
图1明确四处测量关系；最后B3诊断图作为F2核心证据进入主文，B2和evaluator错位留补充；
各图显示逐world差异、scheduled/available分母和失败，不把四层汇成一个分数或全红失败示意图。

当前九张共享图对应完整收束：匿名主文使用四层设计、纠错、压缩、最后结构诊断和M3图；
M1、历史B2/B3及行动策略图置于附录。稿件与图件源已同步，本轮文字与PDF同步由W2-86管理。
