# Work II 完整故事：科学Agent的四层测量、分离与成功边界

更新：2026-09-07。本文件负责作者侧的完整论证，不管理实验执行。
...
> 在有界化学模拟任务中，补全观测映射能显著改善结构识别，但结构恢复不保证行动收益；提交规律可能丢失预测信息，也能在特定设置下独立帮助新决策。
> 四层测量组织这些不同读出，M1/M3提供观察到的成功设置及其边界，没有识别一套统一的成功机制。

“四层断裂”保留为研究起点与问题框架。当前结论并非四个转换都已被证明失败：F1是预设纠错
标准未获支持，F2在新映射干预中获得双模型正效应，同时保留旧协议的信息缺口，F3测到提交规律的保真损失，F4测到规律与实际选择
不一致。四层不能合并为内部因果链，也不能外推为所有Agent的共同缺陷。

| 转化位置 | 已有支持 | 解释强度 |
| --- | --- | --- |
| F1 实验搜索/进步→针对性纠错 | C2预测平均改善，注册selective-correction未获支持；A-P有条件性packet后响应 | 搜索进步不识别反馈学习；packet与额外turn尚未隔离 |
| F2 数值预测→结构识别 | 补全观测映射：GPT恢复6/20→20/20、DeepSeek3/20→17/20；两者均+70个百分点 | 共享10世界、同族参数覆盖；旧协议信息缺口保留，结构恢复不保证行动改善 |
...
三种先验（不提供、正确、错误）是提供给Agent的信息条件；实体、参数、结构是先验改变的三种位置。
四层则是四个评价问题，这些分类不能混为一谈。各组在同一世界中使用相同物理规则和公开接口，
测试真值由评价器持有。不同实验块按各自预算执行，不将两模型比较解释为等算力排名。

| 转换 | 实际对照 | 主要测量 | 分母与解释边界 |
| --- | --- | --- | --- |
| F1 实验→针对性纠错 | 三种先验下自主实验；比较错误与正确先验组的实验前后预测改善 | 错误组的误差下降减去正确组的误差下降，结合预设纠错门槛 | 每模型9个任务—干预组合×5世界×3先验＝135会话；门槛受初始误差空间影响，不通过不等于完全不会纠错 |
| F2 预测→结构识别 | 同一世界先给各组相同证据；后继实验只改变是否补全观测映射 | 留出预测误差、家族正确且指数误差≤0.10的联合恢复；正确先验保持单列 | 新块10世界×3先验×2说明×2模型＝120会话；旧包信息不足，不能作为充分信息下的能力缺陷证明 |
| F3 预测→可执行规律 | 将同一最终提交的逐点预测和公式在相同测试条件上对齐 | 公式误差减去直接预测误差，并测两份输出自身的不一致 | DeepSeek/GPT为135/129份可评价规律；合法公式拟合只验证已测域容量，不证明新条件泛化 |
| F4 规律→未见行动 | 自主实验后给8个完整候选，比较公式推荐、Agent选择和实际最优 | Top-1、选择一致率及错选造成的收益损失 | 原45会话/42排序为DeepSeek-only；M1替换规律与选择器，M3隔离规律与原始证据，分别检验干预和独立知识效用 |

两个实测案例可帮助读者理解F2和F3，均为模拟环境记录中的示例，不作为新增统计检验。

- **F2萃取：** 一个错误先验会话收到的8个有机相产物读数为0.792–0.821，DeepSeek将其概括为约0.80的固定回收水平，
  并把其他查询也预测在该值附近，却没有表达设定的1.75次幂分配关系。旧公开表面存在精确linear/power alias，
  因而这说明低预测误差不充分证明结构识别，也暴露了实验信息不足；不能把它归为纯Agent内部失败。
  [公开回答与信息边界](../workstreams/flagship_tasks/reports/work-ii-w2-64-publication-reanalysis-v0.1.json)。
- **F3电化学：** 一次完成8个实验的会话，在同一0.8 V、100 mA测试条件下，直接预测能量效率0.55；
  同次提交的公式为 $\eta=0.5+0.4V-1.2V^2$，执行后却得到0.052，模拟器真值为0.61619。
  这暴露的是同一提交中答案与公式的不一致。该会话全部12个query–metric项的直接预测/公式误差为0.09850/0.23847；
  全体135例平均压缩损失0.0686是另一统计层级，不是这个单例的误差。
  [原始提交](../runs/formal/work-ii-deepseek-c2-public-v0.2-20260814/cells/A_E_public--electrochemical-conversion--seed934334899--opaque/summary.json)。
...
每个task的五个world共用一套候选坐标，实测最优也位于相同坐标；该结果尚未显示决策必须依赖世界特异的规律识别。
...
### 第五步：信息补全直接改善结构，行动仍需单独检验

新的信息完整性干预在10个独立选定世界中，只改变post阶段是否披露完整观测映射；固定观测、先验、候选、接口和工具。
GPT/medium与DeepSeek Flash/low各60会话，120/120终态、112有效、8失败/中断。
未知/错误先验恢复分别6/20→20/20、3/20→17/20；合并9/40→37/40，提升70个百分点，近似world 95%区间[+52.5,+85]。
正确先验保持分别8/10→10/10、6/10→10/10，不混入恢复。模型和先验嵌套于共享10世界，不扩成20世界。

行动结果没有一致方向：GPT Top-1 5/20→12/20、regret 0.07097→0.00748；DeepSeek Top-1 6/20→4/20、regret 0.15090→0.24263。
后者只是描述性下降，不能宣称显著恶化。信息长度与配对噪声属于干预设置，历史B3并未因此得到重新解释或回填。
这把故事从仅报告分离推进到检验具体成功条件：信息补全帮助结构恢复，结构恢复本身不能代替行动测量。
用户另授权的8个失败补跑已完成：8/8有效，其中5例未知/错误先验恢复、2例正确先验保持、1例仍答错。一次补跑敏感性为DeepSeek恢复5/20→20/20、Top-1 7/20→5/20，两个方向保留；原120会话主结果及全部失败不替换。

## 4. 贯穿全篇的逻辑

最后的B3补强已完成：120/120尝试、117有效、3失败，240轮全部计账。
工具可用性的主差+0.01667，95%区间[−0.08333,+0.10000]，不支持明确改善，也不证明等价。
未知/错误先验80会话中78有效、0恢复；正确先验下GPT工具off/on保持6/10、7/10，DeepSeek均2/10。
GPT开放工具30会话均未使用，DeepSeek20/30使用，不能把该对比称为强制数值计算的效果。
这补强F2的条件性观测，没有证明纯内部算术故障、随机schema修复或公开信息一定足够。
完整规律压缩和纵向F4继续由原协议支持；新块不替代其分母。
...
B2的单对观测存在精确linear/power alias，属于表达和可识别性诊断；B3检验有界函数族和指数读出，
其特权参考资格不证明公开信息充分性。
...
新映射干预承担F2的主文证据；历史B3、工具可用性、B2欠识别、schema容量和evaluator错位保留在补充材料，不承担信息充分时的普遍能力缺陷论证。
A-P packet与额外turn捆绑；best-minus-first描述搜索；low reasoning不是thinking-off。
它们不共同构成一条已识别的内部能力因果链。

## 6. Spotlight空间与下一步

2026-09-06公开信息复核完成：全部5个world、40对证据、160个metric对，无缺失。
水相含量与相比例各40/40对完全相同；有机相含量与score平均绝对差为0.005873/0.004992。
公开包缺少完整观测方程、过程因子定义与噪声模型；同信息参考结构恢复仍未建立。
因此旧协议的F2只支持条件性的数值/结构输出分离。上述差异不是信噪比或未见预测基线，
也不证明真实模拟器不可识别。具体来源由current的w2_79_public_information解析。

优先的信息完整性干预现已完成：共享10个新world、两配置均提升70个百分点，行动方向却不一致。
这补强了先前最薄弱的F2条件证据，比继续堆模型或重复负结果更有解释价值；但同族参数、披露长度和共用harness仍限制外推。
Spotlight空间来自把信息条件、结构恢复和行动边界连成清楚且可反驳的叙述，不能据此宣布普遍修复机制或录用概率。

Blender探索器用于讲清实验过程、已测量值与论文证据：三维场景、公开轨迹和四层图表联动。
它不增加研究样本，不把场景合成读数混入Core，也不把视觉逼真度作为物理或机制效度。
...
诊断论文的增量应是四处转换问题的分别测量、可复核的差异和成功边界；超过nearest只约束方法
优势主张，不作为论文成立或结束实验的条件。M1/M3没有在相同协议下推翻历史差异，也没有识别
哪个接口或证据变化修复了差异。不得将一组单独实验包装为同一内部因果链。

本轮三位独立AI审稿支持按有界诊断论文停止新增实验；故事审稿认为普通接收有空间，但新颖性、
F2测量效度和主线分散仍是审稿风险，当时证据的spotlight竞争力有限；该评阅早于新信息干预，不能视为对当前版本的再次审稿。这是内部评阅判断，
不是真实会审或录用预测，详见[独立评估](../workstreams/flagship_tasks/reports/work-ii-independent-review-20260906.md)。
投稿前最后补强块已完成，现在只整合实际结果、图表与稿件，不追加第三模型；8例获授权的一次补跑单列敏感性分析，主分母不替换。
未来增加模型也不能自动补齐完整纵向F4或所有四层的跨模型证据。设计和终态见
...
第三模型家族与独立框架是后续外部复核方向，本轮暂缓，不加入已完成的120会话正式块。
未来另立实验时应固定数值工具对照，不让复核模型参与提示、任务或阈值调优，也不按结果换模型。
当前范围和停止点由[实验矩阵](../workstreams/flagship_tasks/WORK_II_EXPERIMENT_MATRIX.md)管理。
跨模型接收同一封存规律只能检验接收侧，来源模型与接收模型的泛化应分开；
同世界上增加模型与会话，也不增加独立世界数。

## 7. 稿件和资源如何呈现

主文现按“F1纠错 → F2结构识别 → F3规律压缩 → F4未见行动 → M1/M3成功边界”组织。
图1明确四处测量关系；双模型映射补全图作为F2核心证据进入主文，旧工具可用性图移入附录，B2和evaluator错位留补充；
各图显示逐world差异、scheduled/available分母和失败，不把四层汇成一个分数或全红失败示意图。

当前图件收束加入双模型结构/行动比较：匿名主文使用四层设计、纠错、压缩、映射干预和M3图；
M1、历史B2/B3及行动策略图置于附录。本轮实验讲解、图件改版及稿件同步由W2-90管理；原W2-87/W2-89实验结果保持终态。
