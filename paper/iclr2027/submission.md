...
ChemWorld varies the supplied initial description while holding the executable world fixed, then evaluates
...
A next experiment would hold evidence fixed and independently change the artifact and decision rule,
followed by a fresh-context test. The present study does not claim those effects or a validated repair
method. It establishes empirical gaps and measurement conditions that such tests must address.
