...
## F2: Structure recovery depends on the disclosed mapping

Matched packets expose numerical--expression dissociation, but B2's single-pair linear/power
alias makes it an underidentifying surface. Historical B3 retained joint recovery of 5/30 for
GPT and 0/30 for DeepSeek, including 13 DeepSeek schema failures. A later minimal-interface
diagnostic completed 117/120 sessions but recovered structure in 0/80 unknown/wrong-prior
cases. Optional numerics changed success by +1.67 percentage points (95% interval
[-8.33, 10.00]); GPT used the tool in 0/30 enabled sessions and DeepSeek in 20/30.
The public mapping was incomplete, so these results do not establish failure on an identifiable
task. Full historical denominators and controls remain in the appendix.
...
states at mean MAE $4.25\times10^{-13}$: the schema can express these already-observed prediction
vectors. This same-domain fit does not test generalization. Blind incumbent replay completed
726 executions for 121 cells, with
...
evidence under operational constraints and submits a reusable knowledge artifact. We measure both
the fidelity of that artifact to the agent's own predictions and its agreement with actual choices,
then deliver it separately from source observations to test recipient decision utility. These are
distinct empirical tests; we do not introduce a decision-focused training algorithm.
...
These studies distinguish information availability, artifact fidelity and decision value.
Completing observation mapping improves structure in both configurations, while action gains
differ; structural accuracy cannot substitute for measuring the resulting decision.
...
reuse does not add independent replication worlds. Each task shares a candidate grid across its
five worlds, and the measured optimum has the same coordinates within task. Effects differ by task. The
...
The disclosure effect covers ten shared worlds in one power family, with paired noise
and additional prompt length. DeepSeek/low and GPT/medium are study-specific configurations,
not equal-compute competitors; calendar and concurrency amendments are disclosed.
