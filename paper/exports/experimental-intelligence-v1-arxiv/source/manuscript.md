...
\caption{\textbf{Programmable construction and frozen coverage design.}
...
Coverage determines the finite qualification sample without defining the extent of the world-construction space; qualification outcomes are reported in Section 4 and Appendix Table 8.}
...
\caption{\textbf{Workflow diversity and planned rollback recovery.}
\textbf{A,} Eight frozen cases span single-process, multistage and reference-library workflows.
\textbf{B,} One protocol-frozen rollback is followed by 18 subsequent commits inside the same complete census. The failed candidate is not installed; recovery continues from the last committed state.}
\label{fig:use-cases}
\end{figure*}
```
...
intervention under fixed actions and noise identity (Fig. 4A,B).
...
\caption{\textbf{Controlled private-law interventions under an invariant public contract.}
\textbf{A,} Parent and child worlds share $W_{\mathrm{pub}}$, the public contract and action sequence while one evaluator-owned constitutive or material law in $\theta$ changes.
...
The selected protocol-frozen non-reference reaction--distillation world was evaluated
